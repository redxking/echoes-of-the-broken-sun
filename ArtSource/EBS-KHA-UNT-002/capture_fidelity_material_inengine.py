# Author: Angelis Pseftis. Isolated production material and evidence capture.
...
DEST = "/Game/Echoes/RiftProduction/Textures"
MAT_DIR = "/Game/Echoes/RiftProduction/Materials"
TICKS = [0, 20, 40, 60, 80]
WINDOW = 80.0
...
def connect_prop(src, src_pin, prop):
    ...
    return ok
full = f"{MAT_DIR}/M_EBS_KHA_Master"
if unreal.EditorAssetLibrary.does_asset_exist(full):
    unreal.EditorAssetLibrary.delete_asset(full)
mat = tools.create_asset("M_EBS_KHA_Master", MAT_DIR, unreal.Material, unreal.MaterialFactoryNew())
...
def tex_node(key, x, y, sampler=None):
    n = lib.create_material_expression(mat, unreal.MaterialExpressionTextureSampleParameter2D, x, y)
    n.set_editor_property("texture", textures[key])
    n.set_editor_property("parameter_name", key+"Texture")
...
team.set_editor_property("default_value", unreal.LinearColor(0.86, 0.86, 0.80, 1.0))
...
connect(base, "", skinned, "A")
connect(core_tone, "", skinned, "B")
connect(window_soft, "", skinned, "Alpha")
# Membership channels drive adaptation-specific mineral tones; team is applied LAST.
core_mask = tex_node("MoltBlend", -1400, 1350, unreal.MaterialSamplerType.SAMPLERTYPE_MASKS)
def scalar(name, value):
    n=lib.create_material_expression(mat, unreal.MaterialExpressionScalarParameter)
    n.set_editor_property("parameter_name",name);n.set_editor_property("default_value",value);return n
def mul(a,ap,b,bp):
    n=lib.create_material_expression(mat, unreal.MaterialExpressionMultiply)
    connect(a,ap,n,"A");connect(b,bp,n,"B");return n
active=scalar("MoltActive",0.0)
localized=mul(window_soft,"",core_mask,"G")
localized=mul(localized,"",active,"")
connect(localized,"",skinned,"Alpha")
carapace_tone=lib.create_material_expression(mat,unreal.MaterialExpressionConstant3Vector)
carapace_tone.set_editor_property("constant",unreal.LinearColor(1.10,1.06,.98,1))
carapace_mix=lib.create_material_expression(mat,unreal.MaterialExpressionLinearInterpolate)
connect(skinned,"",carapace_mix,"A");connect(mul(skinned,"",carapace_tone,""),"",carapace_mix,"B");connect(vc,"G",carapace_mix,"Alpha")
striker_tone=lib.create_material_expression(mat,unreal.MaterialExpressionConstant3Vector)
striker_tone.set_editor_property("constant",unreal.LinearColor(.90,.88,.86,1))
striker_mix=lib.create_material_expression(mat,unreal.MaterialExpressionLinearInterpolate)
connect(carapace_mix,"",striker_mix,"A");connect(mul(carapace_mix,"",striker_tone,""),"",striker_mix,"B");connect(vc,"B",striker_mix,"Alpha")
damage=scalar("Damage01",0.0)
dark=lib.create_material_expression(mat,unreal.MaterialExpressionConstant3Vector)
dark.set_editor_property("constant",unreal.LinearColor(.02,.02,.02,1))
damaged=lib.create_material_expression(mat,unreal.MaterialExpressionLinearInterpolate)
connect(striker_mix,"",damaged,"A");connect(dark,"",damaged,"B");connect(damage,"",damaged,"Alpha")
clamped=lib.create_material_expression(mat,unreal.MaterialExpressionClamp)
clamped.set_editor_property("min_default",.02);clamped.set_editor_property("max_default",.07)
connect(damaged,"",clamped,"");connect(clamped,"",teamed,"A")
connect_prop(teamed, "", unreal.MaterialProperty.MP_BASE_COLOR)
transmission = lib.create_material_expression(mat,unreal.MaterialExpressionOneMinus)
connect(localized,"",transmission,"")
connect_prop(transmission,"",unreal.MaterialProperty.MP_OPACITY)
connect_prop(mul(core_tone,"",localized,""),"",unreal.MaterialProperty.MP_SUBSURFACE_COLOR)
report["material_scope"]={"core":"opaque-pass subsurface transmission controlled by 512px MoltBlend.G and MoltActive; not see-through geometry", "team":"applied after damage and adaptation; no vertex ownership channel", "damage":"presentation parameter only"}
...
connect(one, "", boost, "A")
connect(mul(window,"",active,""), "", boost, "B")
...
actors = {}
for state in STATES:
    imported=json.load(open(os.environ["EBS_IMPORT_REPORT"]))
    records=imported.get("assets",imported.get("meshes",[]))
    stem="SK_EBS_KHA_UNT_002_"+{"baseline":"Baseline","carapace_molt":"Carapace","striker_molt":"Striker"}[state]
    asset_path=os.environ["EBS_MESH_ROOT"]+"/"+stem+"."+stem
    mesh=unreal.load_asset(asset_path)
    if mesh is None:
        report["errors"].append(f"{state}: no SkeletalMesh at {asset_path}"); continue
...
report["readiness"] = {"status":"REQUIRES_PIXEL_REVIEW", "note":"Legacy fixed-coordinate readiness probe is invalid for this geometry; compare captured frames."}
...
for state in STATES:
    c=show(state,0)
    prefix="SK_EBS_KHA_UNT_002_"+{"baseline":"Baseline","carapace_molt":"Carapace","striker_molt":"Striker"}[state]
    sequences=[a for a in import_receipt["animations"] if a["name"].startswith(prefix)]
    for a in sequences:
        action=a["name"][len(prefix):]
        # Full action set on baseline; variant-specific transitions and death on adaptations.
        if state!="baseline" and action not in ("molt","death","fire_on_the_move"):continue
        seq=unreal.load_asset(a["path"])
        for fraction in (.0,.5,1.0):
            t=a["length_s"]*fraction
            c=show(state,int(fraction*80) if action=="molt" else 0)
            c.set_update_animation_in_editor(True)
            # OverrideAnimationData does not reinitialize an unchanged single-node mode.
            # Switching away forces it to install the new sequence and sample time.
            c.set_animation_mode(unreal.AnimationMode.ANIMATION_BLUEPRINT)
            c.override_animation_data(seq,False,False,t,1.0)
            aim(650,-25)
            for _ in range(2):comp.capture_scene()
            file=f"pose_{state}_{action}_{int(fraction*100):03}.png"
            unreal.RenderingLibrary.export_render_target(world,rt,OUT,file)
            bone_positions={}
            for bone in ("body","fl_foot","fr_foot","rl_foot","rr_foot"):
                v=c.get_socket_location(bone);bone_positions[bone]=[v.x,v.y,v.z]
            report["animation_frames"].append({"file":file,"state":state,"action":action,"time_s":t,"bones_cm":bone_positions})
    c.override_animation_data(None,False,False,0,1.0)
death=[f for f in report["animation_frames"] if f["state"]=="baseline" and f["action"]=="death"]
if len(death)!=3 or abs(death[-1]["bones_cm"]["body"][2]-death[0]["bones_cm"]["body"][2]+110)>1:
    report["errors"].append("Engine death body displacement does not match authored -110 cm")
