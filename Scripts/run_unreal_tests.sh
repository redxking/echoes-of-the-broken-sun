...
if [[ "$(read_report_value succeeded)" != "7" ||
  ...
  print -u2 "Unreal automation totals did not match the accepted 7/7 clean result."
...
for expected_test in "${expected_tests[@]}"; do
  matched=false
  for test_index in 0 1 2 3 4 5 6; do
...
print "Unreal automation passed: 7/7 Echoes tests, 0 warnings, 0 errors."
