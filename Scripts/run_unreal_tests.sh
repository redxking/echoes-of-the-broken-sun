...
if [[ "$(read_report_value succeeded)" != "6" ||
  ...
  print -u2 "Unreal automation totals did not match the accepted 6/6 clean result."
...
for expected_test in "${expected_tests[@]}"; do
  matched=false
  for test_index in 0 1 2 3 4 5; do
...
print "Unreal automation passed: 6/6 Echoes tests, 0 warnings, 0 errors."
