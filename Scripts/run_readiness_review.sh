#!/bin/zsh
set -euo pipefail
# SPEC-TUT-008 readiness lessons six to ten: an agent-driven rendered review of the practice route for each lesson (see EchoesPlayerReadinessReview.cpp). Author and owner: Angelis Pseftis.
# SPEC-TUT-008 readiness lessons six to ten: an agent-driven rendered review of the practice route for each lesson (see EchoesPlayerReadinessReview.cpp). Author and owner: Angelis Pseftis.
# SPEC-TUT-008 readiness lessons six to ten: an agent-driven rendered review of the practice route for each lesson (see EchoesPlayerReadinessReview.cpp). Author and owner: Angelis Pseftis.
# SPEC-TUT-008 readiness lessons six to ten: an agent-driven rendered review of the practice route for each lesson (see EchoesPlayerReadinessReview.cpp). Author and owner: Angelis Pseftis.
# SPEC-TUT-008 readiness lessons six to ten: an agent-driven rendered review of the practice route for each lesson (see EchoesPlayerReadinessReview.cpp). Author and owner: Angelis Pseftis.
...
evidence_dir="${ECHOES_READINESS_REVIEW_DIR:-$project_root/BuildArtifacts/Evidence/readiness-review-$stamp}"
log="$evidence_dir/ReadinessReview.log"
captures="$evidence_dir/captures"
scope_root="$evidence_dir/Scope"
budget_seconds="${ECHOES_READINESS_REVIEW_BUDGET:-1500}"
...
"$editor" "$project" /Engine/Maps/Entry \
  ...
  -windowed -ResX="${ECHOES_READINESS_REVIEW_RESX:-1280}" -ResY="${ECHOES_READINESS_REVIEW_RESY:-720}" -ForceRes \
  -UserDir="$scope_root/UserDir" \
  -EchoesSaveGameDirectory="$scope_root/SaveGames" \
  -EchoesReadinessReview \
  -EchoesReadinessReviewOutputDir="$captures" \
  ${=ECHOES_READINESS_REVIEW_EXTRA_ARGS:-} \
  -AbsLog="$log" &
editor_pid=$!
...
while (( elapsed < budget_seconds )); do
  if [[ -f "$log" ]] && /usr/bin/grep -q 'ECHOES_READINESS_REVIEW_COMPLETE' "$log"; then
    completed=1
    break
  fi
  kill -0 "$editor_pid" 2>/dev/null || break
...
if (( completed != 1 )); then
  if [[ -f "$log" ]] && /usr/bin/grep -q 'Engine exit requested' "$log" && ! /usr/bin/grep -q 'ECHOES_READINESS_REVIEW_COMPLETE' "$log"; then
    print -u2 "The engine exited before the review completed; last review markers:"
    /usr/bin/grep -E 'ECHOES_READINESS_REVIEW_' "$log" | tail -5 >&2
  fi
  print -u2 "Readiness review did not reach its completion marker within ${budget_seconds}s."
  print -u2 "Inspect: $log"
  exit 3
fi

print "Readiness review completed. Stage record:"
/usr/bin/grep -E '\[ECHOES_READINESS_REVIEW_(STAGE|CAPTURE|COMPLETE)\]' "$log" | sed 's/^.*LogEchoes: Display: //'
print "Captures:"
ls -1 "$captures"
print "Evidence directory: $evidence_dir"
if /usr/bin/grep -q 'ECHOES_READINESS_REVIEW_COMPLETE\] result=PASSED' "$log"; then
  exit 0
fi
