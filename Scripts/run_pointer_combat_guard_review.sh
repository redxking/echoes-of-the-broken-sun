...
"$editor" "$project" /Engine/Maps/Entry \
  -game -nop4 -nosplash -nosound -windowed -ResX=1600 -ResY=900 \
...
required_markers=(
  ...
  '\[ECHOES_POINTER_COMBAT_GUARD_REVIEW_COMPLETE\].*authoritativeCommands=true.*authoritativeDamage=true.*osInjection=false.*unaidedHuman=false.*controlledNonshipping=true'
)
...
if /usr/bin/grep -Eq '\[ECHOES_POINTER_COMBAT_GUARD_REVIEW_FAILED\]|\[ECHOES_BOOT_INCOMPLETE\]|\[ECHOES_CONTENT_FAILED\]|\[ECHOES_SIM_VIEW_SYNC_FAILED\]|Fatal error:|Assertion failed:' "$log"; then
...
if [[ "$dimensions" != *"pixelWidth: 1600"* ||
      "$dimensions" != *"pixelHeight: 900"* ]]; then
  print -u2 "The pointer combat/Guard capture is not 1600 x 900. Inspect: $capture"
  exit 6
fi

print "Controlled pointer combat/Guard review passed: exact-coordinate LMB selection, J Guard, RMB direct attack, and authoritative damage were observed."
