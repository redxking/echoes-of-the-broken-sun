...
required_markers=(
  ...
  '\[ECHOES_BROKEN_SUN_READY\] planKey=7 founding=1 route=ash_cut recordedProtocol=2 protocol=preserve_hold availability=0x07 voice=[1-9][0-9]* heavy=[1-9][0-9]* neme=[1-9][0-9]* worker=[1-9][0-9]* mara=[1-9][0-9]* oruun=[1-9][0-9]* talar=[1-9][0-9]* approach=\(32,56\) maraSite=\(18,35\) oruunSite=\(18,30\) nemeSite=\(38,43\) talarSite=\(26,43\) restoration=\(32,49\) controlled=\(32,44\) extinguishment=\(26,49\) evolution=\(26,54\) baseHoldTicks=240 inheritedRecords=14 localFaction=HollowChoir localAuthority=HollowChoir namedWitnesses=protectedNeutral mixedFactionCommand=false explicitEndingEligibility=true hiddenMoralityScore=false oldLedgerMigration=true oldCheckpointCompatibility=false broadConsequencesUnmodeled=true campaignBalanceUnproven=true ordinaryHumanCompletionUnproven=true releaseReadinessUnproven=true blocked=223'
  '\[ECHOES_BOOT_READY\]'
