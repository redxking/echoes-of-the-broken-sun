...
int main(int argc,char**argv){
 ...
 auto finalBytes=sim.SaveSnapshot();std::ofstream finalFile(std::string(argv[1])+".final",std::ios::binary);finalFile.write(reinterpret_cast<const char*>(finalBytes.data()),finalBytes.size());finalFile.close();
 std::cout<<"{\"snapshot_version\":"<<kSnapshotVersion<<",\"replay_version\":"<<kReplayVersion<<",\"final_tick\":"<<replay.finalTick<<",\"final_checksum\":"<<replay.finalChecksum<<",\"core\":"<<core<<",\"worker\":"<<worker<<",\"commands\":2}";
 return 0;
}