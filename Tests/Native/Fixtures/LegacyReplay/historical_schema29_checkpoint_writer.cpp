...
int main(int argc, char** argv) {
    ...
    SimulationConfig config{64, 64, 20, 0xE0C0B5A1ULL};
    ...
    if (!sim.AddPlayer(1, Faction::KharuunAssemblies, {1000, 100})) return 3;
    for (int y = 30; y <= 34; ++y) {
        for (int x = 8; x <= 55; ++x) {
            if ((x >= 12 && x <= 15) || (x >= 29 && x <= 35) ||
                (x >= 48 && x <= 51)) continue;
            if (!sim.SetTerrainTile(x, y, Terrain::Blocked)) return 11;
        }
    }
