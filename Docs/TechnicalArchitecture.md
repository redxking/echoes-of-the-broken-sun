...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.48.0 retains schema-20 research, deterministic Tab/Backspace selection, anonymous-contact presentation, and the discoverable Kharuun aliases while adding the bounded screen-reticle destination path. Source commit `9a68d6a6df9995c004b15edc77a0f4564c759f5b` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 22/22 Unreal automation tests without warning or error, both normal faction runtimes, the controlled Kharuun systems fixture, and the 400-unit/four-team active-combat gate. Ordinary rendered application input moved the reticle, issued movement/attack-move, completed a Meridian Bulwark deploy/pack/redeploy cycle, placed and rendered an Array Foundry construction site, delivered deterministic restart, and queued a Lancer with visible progress. Earlier ordinary Kharuun evidence accepts Waystone migration; F4/F6 rejection paths do not establish successful adaptation or cover. Exact coordinate-pointer delivery, new-site construction completion, produced-unit emergence, ordinary Kharuun adaptation/cover success, and a complete player-driven victory/result/restart operation remain unaccepted. Balance, a fully authored vertical slice, networked multiplayer, and game completion also remain open. Version 0.48.0 has not been packaged or profiled because free storage is below the packaging floor; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as test evidence changes.
