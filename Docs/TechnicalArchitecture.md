...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.44.0 presents both schema-20 faction research paths through a modal, pause-safe archive with shared render/hit geometry, explicit keyboard focus, exact focused-tier confirmation, exact-tier pointer targets, stable failure feedback, and authoritative interruption/no-refund state. Clean commit `eb063c6296b7da4a37fe4bc7323256e2535448a3` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 22/22 Unreal automation tests without warning or error, both normal faction boot directions, both controlled completion directions, both controlled interruption directions, and the 400-unit/four-team active-combat runtime gate. At 1600×900, unattended Metal evidence accepts the red interrupted tier, locked prerequisite, concise no-refund field warning, and pause-safe archive without observed clipping or overlap. Coordinate-click delivery and ordinary player-driven interruption/completion remain unaccepted. Version 0.44.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. These artifacts establish deterministic research interruption, Unreal integration, and bounded archive presentation, not a complete pointer-driven match, research balance, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
