...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Current version 0.66.0 at game-source commit `5ef19c26ea71d4b8f3de13183e141cd7f484ccee` retains **The Shape of Silence**, the registered Glass Scar world candidates, and corrected review-mode precedence while adding deterministic selected-producer research cancellation and interaction-specific accepted-order presentation. The exact source passes 27/27 compiler tests, 27/27 native suites in optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations, an arm64 Development Editor build, 34/34 Unreal automation tests without warning or error, and a normal Meridian null-RHI bootstrap. Exact-commit Metal evidence accepts one ordinary technology-archive start and selected-producer `X` interruption with persistent no-refund state. Separate integrated-baseline Metal evidence accepts one exact-coordinate pointer path through selection, movement, construction, production queue/completion, and Future Well Harvest. The public nine-page archive and 0.65.0 environment captures retain their recorded boundaries. Physical-human pointer usability, pointer combat/Guard coverage, adverse camera/UI-scale cases, broad combat/camera readability, final topology/UVs/materials/destruction/VFX/audio, current package performance, campaign-slot/backup UX, the remaining campaign, networked multiplayer, and game completion remain open. Version 0.66.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
