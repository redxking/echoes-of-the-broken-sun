...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.56.0 retains the 0.50.0 interaction/research/AI/victory evidence and the 0.51.0 through 0.55.0 terrain, silhouette, command-feedback, formation, and command-deck layers while adding viewport-safe anonymous vibration cues. Source commit `9389ca8803dbcfc494116ace63e0be63f65881d3` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 26/26 Unreal automation tests without project warning or error, both normal faction runtimes, and the controlled Kharuun systems smoke. A 1600×900 Metal Kharuun fixture accepts one edge-clamped quantized contact in default and high-contrast modes while the player view discloses no hidden entity and the cue supplies no direct target. Exact coordinate-pointer delivery and ordinary player-driven research interruption remain unaccepted. Multiple-contact deconfliction, every camera angle/UI scale/resolution, large or mixed formation behavior, combat cohesion, broad usability, balance, performance of the current presentation layers, final meshes/materials/effects/audio, networked multiplayer, and game completion also remain open. Version 0.56.0 has not been packaged or profiled because free storage is below the packaging floor; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as test evidence changes.
