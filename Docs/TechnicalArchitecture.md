...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Current version 0.71.0 at exact source commit `5368aec5d86a6bf5566c3445890323432f8cba1f` retains **The Shape of Silence**, prior-generation campaign restoration, deterministic research cancellation, the accepted input boundaries, Ash Cut route kit, authored selection/command family, and destruction-state feedback while adding three registered presentation-audio cues. The original deterministic mono PCM sources contain no third-party samples; accepted local commands use one non-spatial confirmation, fair authoritative removals use faction-distinct spatial cues, independent rate limits bound voice bursts, and persistent effects-volume/reduced-dynamic-range controls are exposed before deployment and in the field menu. Audio remains presentation-only and cannot affect simulation, visibility, saves, replays, or checksums. The exact source passes 27/27 compiler tests, 27/27 native suites in optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations, an arm64 Development Editor build, 36/36 Unreal automation tests without warning or error, a normal Meridian null-RHI bootstrap, and separate exact-commit standard/reduced-dynamic-range Metal reviews. Earlier exact-source evidence retains its recorded boundaries. Physical-human pointer usability, pointer combat/Guard coverage, adverse camera/UI-scale cases, broad simultaneous-combat/camera/audio readability, subjective listening and final mix/master quality, music, voice, ambience, complete alerts, final Niagara debris/smoke, transparency/dissolve, production textures and surface response, the remaining route/world kits, current package performance, named or multiple campaign slots, arbitrary campaign history, the remaining campaign, networked multiplayer, and game completion remain open. Version 0.71.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
