...
As recorded on 2026-08-30, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Version 0.79.0 at exact clean game-source commit `fc086dee8eef8e91bdef46c0d53f43a8436c36c5` extends the separate-process transport slice described above while retaining all earlier campaign, art, audio, responsive-HUD, and pointer evidence at its recorded limits. The exact source passes 27/27 content tests, 28/28 native suites in optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations, an arm64 Development Editor build, 37/37 Unreal automation tests without warning or error, a normal Meridian null-RHI bootstrap, and one normal-exit localhost listen-server/client smoke. That smoke traversed compatible, ready, and started states; accepted and acknowledged three 4,537-byte scoped full keyframes at ticks 0, 10, and 20; assigned both a local-host Move and remote Move from tick 0 to tick 3; and verified both at tick 20. Protocol and Unreal tests cover canonical round trips, compatibility mismatch, malformed/corrupt/oversized packets, connection-derived seat identity, actor ownership, sequence/tick bounds, scoped-state construction, hidden-entity exclusion, visible-hostile redaction, keyframe tamper rejection, client lineage advance/rejection/recovery, and accepted execution. They do not establish authentication, encryption, a player-facing lobby, scoped deltas, rendered client world presentation, match completion, reconnect, spectators, separate-machine play, injected latency/loss/reordering, command-rate controls, or abuse resistance. Physical-human pointer usability, arbitrary or dynamically resized resolution/aspect-ratio/camera coverage, broad simultaneous-combat/camera/audio readability, final mix quality, music, voice, ambience, complete alerts, final effects, production textures, current package performance, named or multiple campaign slots, arbitrary campaign history, the remaining campaign, complete multiplayer, and game completion remain open. Version 0.79.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
