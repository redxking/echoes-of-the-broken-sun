...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.36.0 implements the bounded compiler, catalog, complete slice-roster simulation-consumption path, and four authored faction-mechanic profiles. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, deterministic production/work/construction fields, complete four-unit/four-structure role coverage for each playable slice faction, the Meridian deployment and supply profiles, the Kharuun Waystone migration profile, and an adaptation profile restricted to the Kharuun production structure. It atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar. Sixteen adversarial tests exercise canonical stability and missing or misassigned special profiles.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256, validates it, and publishes an immutable catalog. Stable-ID bindings convert all eight named units and eight named structures into sixteen deterministic archetypes. The adaptation profile maps Growth Basin radius, timing, Dawn cost, exposed-state damage, Carapace, and Striker tradeoffs into fixed authoritative rules. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. Rules version 1 and the 32-byte content digest are serialized in schema-15 snapshots and replays and included in checksums. Warform current/pending form and visible molt-site identity are public; opponent health, combat statistics, and exact completion tick remain redacted. Starting a valid molt spends Dawn, resets the prior form without compounding, clears orders, and blocks movement/combat commands. The stationary unit takes authored additional damage. Site loss or field departure cancels without refund; completion applies exactly one form from baseline statistics. Adaptive AI counts only visible public enemy categories before choosing a form at an owned Basin. Unreal automation checks authored mapping, actual scenario actors, public non-color presentation, reason-coded invalid Meridian control, Shift–bracket mappings, and all prior regressions. Direct valid player use remains blocked by the Meridian-only operation. Remaining faction abilities, map packs, technologies, localization, final presentation references, and signed manifests remain open.
