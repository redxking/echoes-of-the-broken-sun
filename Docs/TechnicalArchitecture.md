...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.54.0 retains the 0.50.0 interaction/research/AI/victory evidence, the 0.51.0 non-authoritative Glass Scar composition, the 0.52.0 faction/role silhouettes, and the 0.53.0 transient accepted-order markers while adding Box, Line, and Wedge destination layouts. Source commit `4375b2018e43302b185e4850792d33f465f15b92` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 24/24 Unreal automation tests without project warning or error, and both normal faction runtimes. An ordinary 1600×900 Metal run accepts separate five-unit Line and Wedge moves, the associated visible labels and feedback, and the settled geometry without observed HUD clipping. Exact coordinate-pointer delivery and ordinary player-driven research interruption remain unaccepted. Large or mixed formations, assignment/path deconfliction, combat cohesion, broad tactical-group usability, balance, performance of the current presentation layers, final meshes/materials/effects/audio, networked multiplayer, and game completion also remain open. Version 0.54.0 has not been packaged or profiled because free storage is below the packaging floor; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as test evidence changes.
