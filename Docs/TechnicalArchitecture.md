...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.43.0 presents both schema-19 faction research paths through a modal, pause-safe archive with shared render/hit geometry, explicit keyboard focus, exact focused-tier confirmation, exact-tier pointer targets, and stable failure feedback. Clean commit `b9630affe5d0505500e059207d248e5fc879ad1f` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 22/22 Unreal automation tests without warning or error, both normal faction boot directions, both controlled faction research lifecycles, and the 400-unit/four-team active-combat runtime gate. At 1600×900, Metal evidence accepts controlled active progress, completion, first-incomplete focus, and direct Up/Down focus movement without observed clipping or overlap. Coordinate-click delivery and interruption presentation remain unaccepted because the external control service still does not provide a dependable pointer result. Version 0.43.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. These artifacts establish deterministic research, Unreal integration, and bounded archive presentation, not ordinary player-driven completion, a complete pointer-driven match, research balance, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
