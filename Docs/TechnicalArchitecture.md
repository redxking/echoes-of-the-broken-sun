...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.57.0 retains the prior skirmish/mechanic/presentation evidence and adds the selectable **What the Ledger Keeps** prologue above the unchanged deterministic simulation. Source commit `ce13d8d6e2a9f7032bb5ad4f628ccc736af6520e` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, and 27/27 Unreal automation tests without project warning or error. Campaign automation completes the authored carrier/recovery/Well/withdrawal/result/replay route through ordinary commands; a current-source prologue null-RHI runtime reaches its first fixed tick; and the Metal gate accepts title, brief, recovery-state objectives/navigation, and high contrast after two label corrections. Exact coordinate-pointer delivery and ordinary player-driven research interruption remain unaccepted. The other Well consequences under full mission play, combat-pressure balance, later campaign missions, multiple-contact deconfliction, every camera/UI-scale/resolution, large/mixed formations, broad usability, current presentation performance, final meshes/materials/effects/audio, networked multiplayer, and game completion remain open. Version 0.57.0 has not been packaged or profiled because free storage is below the packaging floor; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
