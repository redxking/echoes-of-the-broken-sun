...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.31.0 implements the bounded compiler, catalog, and foundational simulation-consumption path. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, deterministic production/work/construction fields, and complete four-unit/four-structure role coverage for each playable slice faction, then sorts the records and atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar under `Content/Data/Generated`. Editor builds and package entry points run the compiler first. Eight adversarial tests exercise canonical stability and the principal rejection paths.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256 through a platform-independent implementation, validates the digest, versions, collections, identifiers, references, role coverage, and loaded numeric fields, and publishes an immutable catalog. Stable-ID bindings convert each faction's worker, line combatant, headquarters, logistics structure, and production structure into ten deterministic archetypes. Units convert centimeters per second into Q22.10 movement per tick; sight becomes tiles; attack range becomes fixed-point distance; structures carry construction time, logistics, and authored footprint; Future Well economy, vision, and duration enter the same rules object. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. Rules version 1 and the 32-byte content digest are serialized in schema-10 snapshots and replays and included in state checksums. Unreal automation checks selected mappings and tamper rejection; normal and stress gates require both content-ready and rules-ready markers. The catalog still contains six non-foundational unit/structure types without distinct simulation entity types. Their abilities, map packs, technologies, localization, presentation references, and signed manifests remain open.
