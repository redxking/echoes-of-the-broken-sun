...
The present code-only interface implements and persists HUD scale, high-contrast HUD, reduced motion, reduced flashing, edge pan, pan-speed scale, and zoom-step scale. A responsive predeployment operations brief pauses the interactive deterministic match, blocks battlefield and camera commands, exposes the implemented accessibility toggles, and resumes on Enter. Unattended automation and explicitly profiled runs bypass the brief so their fixtures start deterministically. Escape or P opens a modal field menu that pauses the authoritative tick, blocks battlefield/camera input, exposes only implemented settings, and resumes through Enter, Escape, or P; R rebuilds the deterministic baseline. A local objective snapshot exposes only owned Command Core integrity, currently visible Future Well and hostile-core state, and the public authoritative outcome. The HUD converts that snapshot into a live tracker without consulting hidden hostile health. A terminal outcome presents a modal victory/draw/defeat state, blocks battlefield and camera input, and permits deterministic restart through Enter or R. World proxies add persistent health bars and four non-color team-marker shapes. The tactical overview reads only the local simulation view: it withholds unexplored blocked terrain and non-visible entities, uses the same four shape variants, and keeps the camera crosshair in presentation state. These are prototype implementation boundaries; the intended UMG/CommonUI front-end, settings, and remapping surface has not been built.
