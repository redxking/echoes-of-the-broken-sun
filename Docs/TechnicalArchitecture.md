...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.34.0 implements the bounded compiler, catalog, complete slice-roster simulation-consumption path, and two authored faction-mechanic profiles. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, deterministic production/work/construction fields, complete four-unit/four-structure role coverage for each playable slice faction, a deployment profile restricted to the Meridian heavy screen, and a supply-extension profile restricted to the Meridian utility. It then sorts records and atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar under `Content/Data/Generated`. Editor builds and package entry points run the compiler first. Twelve adversarial tests exercise canonical stability and the principal rejection paths, including missing or misassigned deployment and supply rules.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256 through a platform-independent implementation, validates the digest, versions, collections, identifiers, references, role coverage, and loaded numeric fields, and publishes an immutable catalog. Stable-ID bindings convert all eight named units and eight named structures into sixteen deterministic archetypes. Units convert centimeters per second into Q22.10 movement per tick; sight becomes tiles; attack range becomes fixed-point distance; structures carry construction time, logistics, and authored footprint; Future Well economy, vision, and duration enter the same rules object. The Bulwark profile converts cover depth and half-width from centimeters into fixed-point distance and carries damage-reduction and movement percentages. The Relay profile converts connection radius to fixed-point distance and carries its logistics bonus, active duration, and cooldown. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. Rules version 1 and the 32-byte content digest are serialized in schema-13 snapshots and replays and included in state checksums. Bulwark deployment/facing and Relay active state are public entity state: player views retain them for readable counterplay while continuing to redact opponent health, movement, combat, order, construction, production, and exact ability timers. The Relay bonus is recomputed from a continuously required connection to an owned completed Anchor or Power Link; a disconnect or destruction collapses it in the same tick, while the cooldown remains. Unreal automation checks selected mappings, both ability controls/presentations, and tamper rejection; normal and stress gates require content-ready and rules-ready markers. Remaining faction abilities, map packs, technologies, localization, final presentation references, and signed manifests remain open.
...
The core independently validates the versioned deterministic rules, numeric bounds, foundational costs/statistics, Future Well timing, and snapshot integrity before constructing match state. Runtime never silently substitutes defaults for rejected authoritative content. The current adapter still performs stable-ID resolution from canonical JSON before copying the rules into the core; a fixed-width binary rules pack, full roster sections, technology/ability references, and signed distribution manifest remain future hardening work.
...
The present code-only interface implements and persists HUD scale, high-contrast HUD, reduced motion, reduced flashing, edge pan, pan-speed scale, and zoom-step scale. Interactive launch first presents a feature-gated title state that names only the implemented Glass Scar single-player operation, makes implemented settings available, hides campaign/multiplayer/replay, pauses the authoritative tick, and blocks battlefield and camera commands. Enter advances to a responsive operations brief and then deploys; the simulation remains at tick zero until deployment. Unattended automation and explicitly profiled runs may bypass both states so their fixtures start deterministically. Escape or P opens a modal field menu during play that pauses the authoritative tick, blocks battlefield/camera input, exposes only implemented settings, and resumes through Enter, Escape, or P; R rebuilds the deterministic baseline when title/briefing modal states are absent. A local objective snapshot exposes only owned Command Core integrity, currently visible Future Well and hostile-core state, and the public authoritative outcome. The HUD converts that snapshot into a live tracker without consulting hidden hostile health. A terminal outcome presents a modal victory/draw/defeat state, blocks battlefield and camera input, and permits deterministic restart through Enter or R. World proxies add persistent health bars and four non-color team-marker shapes. The tactical overview reads only the local simulation view: it withholds unexplored blocked terrain and non-visible entities, uses the same four shape variants, and keeps the camera crosshair in presentation state. These are prototype implementation boundaries; a production UMG/CommonUI shell, skirmish setup, complete settings, and remapping surface have not been built.
...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. The adapter currently generates a two-player playable placeholder arena or an explicit four-team scale fixture, and exposes an uncaptured/unlocked visible RTS cursor, feature-gated title, responsive predeployment operations brief, visibility-scoped live objectives, modal victory/draw/defeat and redeployment, a modal field pause/options menu, camera, visibility-scoped entity proxies, selection, context orders, deterministic attack-move, patrol, hold, guard, and stop, construction placement commands, timed one-slot production, logistics capacity, pause/restart, derived Command-Core victory, transactional checkpoints, persistent accessibility settings, a fog-respecting tactical minimap, a HUD, an Adaptive opponent with deterministic expansion/retreat behind the materialized player-view boundary, and Future Well orders. Match creation now applies authored values to ten foundational archetypes and Future Wells. The scale fixture creates 100 owned units per active player and one central Well, queues 396 deterministic broad attack-move orders, enters actual combat, and uses lightweight reduced-motion-aware atmosphere plus reduced-flashing-aware damage feedback. It is still a synthetic performance fixture rather than authored final gameplay. Version 0.31.0 is editor-built and automation/runtime-verified; its presentation is unchanged from the bounded 0.27.0 Metal inspection, and it has not been packaged or profiled. The latest accepted package remains 0.23.1. These artifacts establish foundational data-driven gameplay, not complete faction mechanics, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
