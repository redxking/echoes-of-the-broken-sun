...
The present code-only interface implements and persists HUD scale, high-contrast HUD, reduced motion, reduced flashing, edge pan, pan-speed scale, and zoom-step scale. Interactive launch presents a feature-gated title state with the implemented Glass Scar skirmish and **What the Ledger Keeps** prologue; F9 rebuilds the selected operation at tick zero, while Tab changes faction only for skirmish. Enter advances through an operation-specific brief and deployment. The authoritative tick remains paused and battlefield/camera commands blocked until deployment. Unattended and profiled runs may explicitly bypass those states. Escape or P opens the existing modal field menu; R rebuilds the selected operation. The local objective snapshot retains the fair skirmish Core/visible-Well/visible-hostile/public-outcome fields and adds explicit operation/prologue phase, owned carrier state, and the locally authored Well consequence. The HUD converts those facts into operation-specific trackers without advancing the mission. Skirmish terminal state presents victory/draw/defeat; the prologue presents success/failure only after the subsystem reducer determines its outcome. The tactical overview remains fog-respecting for entities/terrain and adds only the prologue's public authored archive/evacuation sites. These are prototype boundaries; a production UMG/CommonUI shell, broad skirmish setup, complete settings/remapping, and a general campaign presentation/store have not been built.
...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.58.0 retains the prior skirmish, mechanic, presentation, and prologue evidence and adds the first durable campaign decision record. Source commit `fdb241d06b5f2922e2f87e62c6f0bc5ede78978c` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, and 28/28 Unreal automation tests without project warning or error. Automation covers encoding, decode rejection, integrity failure, transactional replacement, backup recovery, two-generation failure, idempotent replay, replay conflict, preservation of real user paths, and the real mission-completion commit path. A controlled null-RHI path completes recovery, legitimate Well discovery, Preserve, withdrawal, result, and an isolated ledger commit through ordinary simulation commands. The Metal gate renders the resulting committed-ledger presentation in default and high-contrast modes. Exact coordinate-pointer delivery and ordinary player-driven research interruption remain unaccepted. The other Well consequences under full rendered mission play, campaign-slot/reset UX, downstream consequence consumption, combat-pressure balance, later campaign missions, multiple-contact deconfliction, every camera/UI-scale/resolution, large/mixed formations, broad usability, current presentation performance, final meshes/materials/effects/audio, networked multiplayer, and game completion remain open. Version 0.58.0 has not been packaged or profiled because free storage is below the packaging floor; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
