...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Current integrated version 0.65.0 adds **The Shape of Silence**, registered Glass Scar shelf, ridge, shard, three-route, Matter-deposit, and world-material candidates, and corrected explicit review-mode precedence at game-source commit `981569b143761835cff9194cebd356ee5774ba57`. The exact source passes 27/27 compiler tests, 27/27 native suites in optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations, an arm64 Development Editor build, and 34/34 Unreal automation tests without test warning or error. Both selectable faction directions and the Preserve **Unburied Road** and **The Shape of Silence** bootstraps pass under null RHI. Four direct Metal environment-review captures establish bounded current-editor rendering of the overview, Ash Cut, Buried Causeway, and Folded Verge; a combined-flag Ash Cut run additionally proves that the explicit route selector is retained. The public nine-page archive passes local link/asset checks, desktop and 390-pixel mobile overflow checks, keyboard skip-link focus inspection, and automated WCAG contrast evaluation with no observed failures. Exact coordinate-pointer delivery, ordinary player-driven research interruption, broad combat/camera readability, final topology/UVs/materials/destruction/VFX/audio, current package performance, campaign-slot/backup UX, the remaining campaign, networked multiplayer, and game completion remain open. Version 0.65.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
