...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.37.0 implements the bounded compiler, catalog, complete slice-roster simulation-consumption path, and five authored faction-mechanic profiles. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, deterministic production/work/construction fields, complete four-unit/four-structure role coverage for each playable slice faction, the Meridian deployment and supply profiles, the Kharuun Waystone migration profile, an adaptation profile restricted to the Kharuun production structure, and a temporary-cover profile required only on the Kharuun Cairnback. It atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar. Eighteen adversarial tests exercise canonical stability and missing or misassigned special profiles.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256, validates it, and publishes an immutable catalog. Stable-ID bindings convert all eight named units and eight named structures into sixteen deterministic archetypes. The adaptation and mineral-cover profiles map their authored ranges, timings, costs, damage modifiers, health, footprint, and tradeoffs into fixed authoritative rules. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. Rules version 1 and the 32-byte content digest are serialized in schema-16 snapshots and replays and included in checksums. Warform current/pending form and visible molt-site identity are public; opponent health, combat statistics, and exact completion tick remain redacted. Mineral cover is a temporary owned battlefield entity whose occupied tile participates in deterministic passability. Attacks against an allied target are redirected to the first qualifying barrier when the fixed-point fire segment crosses its square footprint; direct attacks against the barrier remain valid. Expiry or destruction restores the captured open/scarred terrain and invalidates the path cache. The object remains after its creator dies. Visible opponents receive public presence and geometry but not exact health, expiry, hidden creator identity, or underlying terrain. Adaptive AI uses only materialized visible threats when choosing both warform adaptations and deterministic cardinal cover positions. Unreal automation checks authored mapping, actual scenario actors, public non-color presentation, reason-coded invalid Meridian control, Shift–bracket and Shift–semicolon mappings, and all prior regressions. Direct valid player use remains blocked by the Meridian-only operation. Remaining faction abilities, map packs, technologies, localization, final presentation references, and signed manifests remain open.
...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. The adapter currently generates a two-player playable placeholder arena or an explicit four-team scale fixture, and exposes an uncaptured/unlocked visible RTS cursor, feature-gated title, responsive predeployment operations brief, visibility-scoped live objectives, modal victory/draw/defeat and redeployment, a modal field pause/options menu, camera, visibility-scoped entity proxies, selection, context orders, deterministic attack-move, patrol, hold, guard, and stop, construction placement commands, timed one-slot production, logistics capacity, five bounded faction mechanics, pause/restart, derived Command-Core victory, transactional checkpoints, persistent accessibility settings, a fog-respecting tactical minimap, a HUD, an Adaptive opponent operating behind the materialized player-view boundary, and Future Well orders. Match creation applies authored values to all sixteen named slice archetypes and Future Wells. The scale fixture creates 100 owned units per active player and one central Well, queues 396 deterministic broad attack-move orders, enters actual combat, and uses lightweight reduced-motion-aware atmosphere plus reduced-flashing-aware damage feedback. It is still a synthetic performance fixture rather than authored final gameplay. Version 0.37.0 is editor-built and automation/runtime-verified; its presentation has not received a new Metal acceptance pass, and it has not been packaged or profiled. The latest accepted package remains 0.23.1. These artifacts establish foundational data-driven gameplay and five bounded faction mechanics, not complete faction identity, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
