...
The 0.77.0 command request deliberately carries no player identifier. Its fixed bounded encoding carries one actor and the complete deterministic `Command` payload. Local authority admission supplies the seat, requires the next contiguous per-seat sequence, rejects missing or non-owned actors, derives its execution window from the authoritative simulation tick with overflow checks, and mutates the accepted-sequence cursor only after `Simulation::QueueCommand` succeeds. This establishes a reusable authority boundary, not transport: actor-list batching, rate limiting, authenticated sessions, acknowledgements/execution results, scoped snapshots, sockets/RPCs, lobbies, reconnect, and spectators remain open.
...
As recorded on 2026-08-30, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Current version 0.77.0 at exact game-source commit `0b03f3237518738c03234a6ab15917c10af65d40` adds the canonical compatibility and single-command admission boundary described above while retaining all accepted 0.76.0 campaign, art, audio, responsive-HUD, and seven-case full-bounds pointer evidence at its recorded limits. The exact source passes 27/27 content tests, 28/28 native suites in optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations, an arm64 Development Editor build, 37/37 Unreal automation tests without warning or error, and a normal Meridian null-RHI bootstrap. Protocol tests cover canonical round trips, every compatibility mismatch, malformed/corrupt/oversized packets, server-derived identity, actor ownership, sequence and tick bounds, overflow, simulation rejection, and accepted execution. They do not establish transport, authentication, encryption, scoped snapshots, remote processes, reconnect, spectators, latency/loss behavior, or separate-machine play. Physical-human pointer usability, arbitrary or dynamically resized resolution/aspect-ratio/camera coverage, broad simultaneous-combat/camera/audio readability, subjective listening and final mix/master quality, music, voice, ambience, complete alerts, final Niagara debris/smoke, transparency/dissolve, production textures and surface response, current package performance, named or multiple campaign slots, arbitrary campaign history, the remaining campaign, networked multiplayer, and game completion remain open. Version 0.77.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
