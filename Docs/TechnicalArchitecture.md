...
`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256, validates it, and publishes an immutable catalog. Stable-ID bindings convert all twelve named units, twelve named structures, and six named technologies across the three playable factions into deterministic rules. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. Research definitions and the Hollow Choir rules are copied into the versioned simulation rules and participate in validation, snapshots, replays, and checksums. A player's completed mask and active project contain only deterministic IDs, producer, progress, and required ticks. Research command validation returns explicit failures for invalid players, producers, technology IDs, faction mismatch, completion, prerequisite, occupancy, and resources. Production and research share the one-slot Barracks producer boundary. Completion rebuilds each surviving combat unit's authored/researched combat values before applying faction-specific derived state in a stable order; future units derive the same completed-research state at creation. A destroyed producer cancels the project after costs have been paid. Only the owning `PlayerState` is present in a player's materialized view, allowing fair-view AI policy without exposing opponent research internals.

The selected local faction remains presentation/bootstrap state rather than a simulation rule: the subsystem instantiates player 0 with the chosen Meridian, Kharuun, or Hollow Choir faction, derives a supported opposing faction for player 1, and binds the same role-based spawn layout to faction-specific archetypes. Title or briefing selection performs a fail-safe scenario rebuild and restores the prior faction if reconstruction fails. Restarts retain the selection. Snapshot loading requires the serialized player-0 faction to match the active choice. Snapshot and replay schema 22 retain active, completed, and last-interrupted research state, Future Well activation timing, and the Hollow Choir identity/coherence state required to reconstruct current authoritative behavior. Older supported snapshots migrate at the simulation boundary and are revalidated before re-save; Mission 14 continuity requires native schema 22 rather than treating migrated state as equivalent evidence. `lastInterruptedResearch` is owning-player state, enters the checksum, persists through snapshot/replay, rejects malformed faction/completion combinations on load, and clears when a new valid research command begins or completes. Command-line faction selection supplies a deterministic unattended/runtime test path without replacing the player-visible Tab flow. The HUD derives faction names, objective text, special-control guidance, research status, interruption status, and result language from active subsystem state. `Shift+R` queues the next incomplete faction technology from a selected local Barracks-role structure. The special profiles, anonymous vibration contacts, powered-Aegis graph, Hollow Choir identity/coherence rules, visibility/redaction rules, and simultaneous-damage behavior remain deterministic simulation state. Map packs, a full technology browser, ability schemas, localization, final presentation references, and signed manifests remain open.
...
Revision `world-surface-instancing-v1` at exact implementation commit `dab16f281b9fe3fe84f463b556fe23dd56bd36ca` closes the shared material's instanced-static-mesh compatibility defect without changing that one-way authority boundary. The Unreal generator now repairs an existing `M_EchoesWorldSurface` in place, enables `MATUSAGE_INSTANCED_STATIC_MESHES`, verifies the usage flag, recompiles and saves only when necessary, and fails closed if the usage or save cannot be established. `Echoes.Runtime.Map.GlassScar` independently loads the material and asserts `GetUsageByFlag(MATUSAGE_InstancedStaticMeshes)`. A repeated generation must emit a reuse marker and preserve tracked bytes. The exact clean-source Development package passed cook plus normal/four-team startup, and two local packaged `METAL_SM5` captures contained none of the previous usage/default-material fallback markers. This accepts material compatibility for the named local package. It does not promote the material to simulation authority or establish final texture, lighting, UI, broad visual quality, package performance, or release readiness; the ordinary frame still shows severe overexposure and debug-like prototype overlay debt.
...
Version 0.73.0 extends that generator/runtime boundary to the Folded Verge. Revision-gated generation replaces the legacy mesh, authors high/low displaced-plate topology, two UV channels, four dedicated material instances, and one simple asset collision primitive. Runtime presentation preserves the route material family while disabling collision, overlaps, and navigation. The widened non-shipping review camera affects only evidence framing. `glass_scar_v5` records all three crossings as production-pipeline candidates while deterministic terrain and campaign consequences remain the only route authority.

Version 0.69.0 applies the same authority separation to selected-entity and accepted-order feedback. Version 0.74.0 advances the family to `selection-command-vfx-v2`: the asset generator creates one selection halo, seven command sigils including a direct-Attack asset distinct from Attack-move, one reusable orbit accent, and a shared parameterized emissive material. Each static mesh has two explicit LODs and no simple collision. Entity views attach the selection halo only to visibility-scoped actors; accepted orders spawn transient command-marker actors only after the authoritative controller reports at least one accepted local command. The standard branch updates presentation transforms and a low-frequency emission envelope from frame time, while the reduced-motion branch restores fixed transforms and the reduced-flashing branch uses constant lower emission. All mesh components disable collision, overlaps, navigation influence, decals, and shadowing as appropriate. None of these transforms, material parameters, review fixtures, or actor lifetimes feeds command validation, fog, pathing, save, replay, or checksum state. Automation verifies all asset paths, direct-Attack/Attack-move separation, component isolation, standard motion, steady reduced motion, lower reduced emission, and unchanged simulation checksum.

Version 0.74.0 also adds a Development-only pointer combat/Guard review boundary without creating a second command authority. A mutually exclusive controlled scenario materializes one local Bulwark, one owned Surveyor, one visible hostile Riftstalker, and both Command Cores; normal opponent command production is suppressed only for that fixture. After ordinary view synchronization, the controller resolves those live entities, projects each actor's presentation bounds through `ProjectWorldLocationToScreen`, moves Unreal's own cursor with `SetMouseLocation`, and calls the same LMB selection, `GuardAtCursor`, and `ContextOrderPressed` methods used by runtime input bindings. It then observes the ordinary queued Guard order in authoritative simulation state, the ordinary visible-target Attack order, and subsequent authoritative HP reduction before pausing and requesting a UI-inclusive screenshot. Shipping sets the scenario selector false and never starts the harness. The fixture does not mutate orders or HP directly, inject operating-system events, query hidden hostiles, or establish unaided-human usability.

Version 0.75.0 gives the field HUD and pointer-review gate one shared `FEchoesHudLayout` geometry source. The pure model computes responsive rectangles for the main panel, objective tracker, command deck, status strip, and minimap from viewport size and the clamped HUD scale. The renderer consumes those rectangles directly. When the objective cannot clear the main panel horizontally, it moves below the main panel with a fixed 16-pixel separation; command-deck and minimap visibility also depend on the main-panel boundary, and the status backing scales with HUD text. During the Development-only review, `IsBattlefieldPointClear` rejects projected targets within any visible panel, the viewport edge, or a 12-pixel expanded margin before the ordinary input handler executes. `FEchoesPointerCombatGuardReview` admits only `Default`, `Offset`, `MinHud`, and `MaxHud`, each with a fixed camera center, zoom, and HUD scale; bootstrap fails closed on any other value. The matrix wrapper requires three unobscured live projections, ordinary selection/Guard/Attack markers, authoritative Guard and damage, and a 1,600 × 900 UI-inclusive frame in every case. This is repeatable integration and layout evidence for four configurations, not a second command path, general layout proof, operating-system input, or human-usability evidence.

Version 0.76.0 strengthens that boundary in three places. First, the controller projects all eight corners of each live actor's presentation bounds, constructs one screen-space box, and requires the complete box—not only its center—to remain within the viewport and outside the 12-pixel expansion of every visible panel. Second, each admitted review variant carries an exact expected viewport; the controller fails closed if Unreal produces different dimensions. Third, responsive visibility requires the command deck and minimap to remain separated from every other visible panel, so constrained layouts can suppress a lower-priority panel instead of drawing an overlap. `FEchoesPointerCombatGuardReview` now admits seven fixed cases: the four reframed 1,600 × 900 camera/HUD variants plus compact 1,280 × 720, Mac 16:10 1,440 × 900, and Full HD 1,920 × 1,080. The shell gate requires exact dimensions and `fullBoundsVisible=true` for all three actors in every case. These checks qualify seven controlled layouts over four resolutions and two aspect ratios; they do not prove arbitrary sizes, live window-resize transitions, operating-system input, or human usability.
...
The present code-only interface implements and persists HUD scale, high-contrast HUD, reduced motion, reduced flashing, edge pan, pan-speed scale, and zoom-step scale. Interactive launch presents a feature-gated title state with the Glass Scar skirmish and fifteen campaign missions through **The Broken Sun**; F9 rebuilds the selected operation at tick zero, while Tab changes among supported factions only for skirmish. Enter advances through an operation-specific brief and deployment. The authoritative tick remains paused and battlefield/camera commands blocked until deployment. Unattended and profiled runs may explicitly bypass those states. Escape or P opens the existing modal field menu; R rebuilds the selected operation. The local objective snapshot retains the fair skirmish Core/visible-Well/visible-hostile/public-outcome fields and adds operation-specific campaign phase, protected-entity state, local-Core survival, prior-ledger consumption, and the locally authored consequence. The HUD converts those facts into operation-specific trackers without advancing the mission. Skirmish terminal state presents victory/draw/defeat; campaign missions present success/failure only after their subsystem reducers determine the outcome. The tactical overview remains fog-respecting for entities/terrain and adds only authored sites for the selected operation. Versions 0.65.0 through 0.91.0 retain the previously accepted bounded public markers and claim language. Version 0.92.0 adds Mission 15's approach, Mara/Oruun/Neme/Talar accord and record markers, ending-specific final site, explicit earned-ending list, double-confirmation state, conduit/hold state, final-resolution result language, and campaign-schema commit status. The Metal review rejected three overflowing briefing lines; the shortened copy, live legend, objective tracker, compact minimap, high-contrast state, and restored default then showed no observed clipping or overlap at the recorded 1319x768 app frame. The UI distinguishes an ending selected for the current operation from an ending actually committed to storage and distinguishes replay conflict or storage failure from a successful append. These remain prototype boundaries: the evidence does not establish ordinary human completion, cinematic ending presentation, modeled downstream social consequences, broad balance, a production UMG/CommonUI shell, complete settings/remapping, or a general campaign-slot store.

Mission 08 is reduced by `FEchoesShapeBesideUsMissionModel`, not by HUD state. A validated, choice-consistent seven-record ledger selects one of three fixed geometries. Talar is the first-echo and convergence proxy; a worker completes the UtilityStructure relay; and distinct Worker and Soldier witnesses must each reach their assigned paired-state site. The reducer advances only in order, tolerates reconstruction after Talar leaves the first site once the relay exists, and fails on loss of a protected proxy, loss of the local Core, or either terminal Core outcome. Successful convergence appends an eighth record with all six completion facts. Quick-save load binds the operation to its ledger fingerprint and revalidates distinct entity identity, ownership, faction, and type before reconstructing the phase. This makes reciprocal contact repeatable and actionable within the authored scenario; it does not model a unified Choir identity, assign causation or hidden authorship, or add a Hollow Choir command roster.

Mission 09 is reduced by `FEchoesReserveAuthorityMissionModel`, not by HUD or presentation state. A validated, choice-consistent eight-record ledger selects one of three nearby authority sites and one advisory first-district recommendation. Mara must reach authority before the host-facing worker construction path admits Power Links. Completed locally owned Dropoff-role structures extend the existing deterministic Aegis network; the model reads only the authoritative powered state of the three protected district posts. Exactly two powered posts advance to the deferred-district phase, while three powered posts deterministically fail. Mara must then reach the fixed site of the intact unpowered district. Loss of Mara, any district post, the local Core, or the ongoing match also fails. Successful completion appends a ninth record containing four common facts plus exactly two of the three district bits; the campaign validator rejects missing, extra, one-district, and three-district masks. Identical replay requires the same choice availability and fact mask, while an alternate exact-two allocation is a conflict. Quick-save load binds the operation to its ledger fingerprint and revalidates distinct IDs, local ownership, Meridian faction, expected entity types, and district geometry before reconstructing phase from authoritative entities. Simultaneous orders that over-allocate the finite reserve are allowed to resolve fail-closed rather than being silently rewritten. The current remote network seat is owner 1 and cannot power the owner-0 protected Aegis network; future owner-0 command entry points must retain the pre-authority and finite-reserve gates. This architecture establishes one local allocation decision only, not wider-city restoration or civilian-survival evidence.

Mission 10 is reduced by `FEchoesChoirAtLumeReachMissionModel`. A validated, choice-consistent nine-record ledger derives the public contact from the inherited Well doctrine and the liability site from Mission 09's deferred district. The operation replaces the Glass Scar barrier with a dedicated 223-blocked-tile Lume Reach topology. Oruun must reach contact; the existing Kharuun Waystone must be uprooted, moved, and rooted at the liability; ordinary workers must build the first and then second Listening Spine at fixed sites; a worker must commit a new Harvest, Preserve, or Reshape choice at the neutral Lume Well; and Oruun must reach the choice-specific resolution. Loss of Oruun, the Waystone, the Well, the local Core, or a committed anchor fails. Reshape also fails if its bounded window expires before resolution. Successful completion appends a tenth record with `AvailableWellChoices == 0x07` and all eight required fact bits (`0xFF`); the decision remains separate even when it repeats the prologue doctrine. Mission 10 quick saves retain the existing CRC-fingerprinted filename only as a namespace. The file itself is an operation-specific envelope containing a format version, the operation identifier, the complete encoded active campaign ledger, and the unchanged schema-20 simulation snapshot. Both primary and backup loads compare the embedded ledger byte-for-byte before snapshot deserialization, so a renamed valid checkpoint or CRC collision cannot cross branches. This is deterministic local binding and corruption rejection, not cryptographic authentication against an actor able to forge local files. The opponent is instantiated mechanically as Meridian quarantine proxies; neither the reducer nor presentation establishes Mara's involvement, Compact-wide action, mixed-faction command, a playable Choir, hidden identity, or causation.

Mission 11 is reduced by `FEchoesNoNeutralLedgerMissionModel`, not by HUD state. Admission requires the exact ordered Mission 01–10 ledger. The founding doctrine, Mission 09's exact powered-district pair, and Mission 10's independent Lume protocol produce 27 explicit plans; completion ticks and checksums do not select a plan. Oruun's Kharuun force remains the sole commandable authority. Two neutral powered Meridian district interfaces, one neutral powered Meridian evidence interface, and one neutral Kharuun evidence interface expose public attributable state without attack, vision, construction, or player ownership. Their inert canonical state is validated when restoring a snapshot. The player re-roots the Waystone on the inherited route, completes Kharuun Listening Spine links within three tiles of both district interfaces, places Oruun at the Kharuun evidence endpoint and a distinct Kharuun witness at the Meridian evidence endpoint, applies only the recorded Mission 10 protocol, and rallies both units at the protocol-specific public site. Protected loss, public-interface loss, a conflicting protocol, an expired unresolved Reshape window, local-Core loss, or either terminal Core outcome fails closed. Success appends Mission 11 with the single recorded protocol bit and all eight completion facts. The `ECHOM11Q` quick-save envelope embeds the exact first ten campaign records before the unchanged schema-20 snapshot; operation and byte-for-byte ledger checks apply to primary and backup recovery. This local coalition-assembly result does not establish mixed-faction allied command, a playable Choir, hidden trust, optional survivor composition, wider faction agreement, or causation.

Mission 12 is reduced by `FEchoesFutureThatWonMissionModel`, not by HUD or presentation state. Admission requires the exact ordered Mission 01–11 ledger. The model reuses the 27 doctrine/district-pair/protocol plans; Mission 11's receipt is required but does not select another axis. Oruun and a distinct verifier are Kharuun Scout units owned by the local player. The two recorded Meridian district inputs, Meridian and Kharuun readback interfaces, Rhyse-attributed demonstrator apparatus, and separate Future Well are neutral, non-commandable public entities with canonical identity, type, faction, geometry, and inert combat/vision state revalidated on load. The reducer advances through independent paired readback, two locally built UtilityStructure links near the exact district sites, binding the recorded protocol at the separate Well, holding its activation receipt for 300 ticks, and observing both district readbacks. The controller synchronizes its Future Well choice to the ledger-bound protocol at title, briefing, operation change, restart, and ordinary offline context dispatch. Reshape terrain may expire normally during the 300-tick hold because the receipt covers the apparatus and protocol activation, not permanent transformed terrain. Protected/public-interface loss, a conflicting or premature protocol, local-Core loss, or either terminal Core outcome fails closed. Success appends Mission 12 with one protocol bit and all eight exact completion facts; the activation fact records the protocol transition rather than claiming that the static demonstrator interface changed state. Snapshot/replay schema 21 serializes `wellActivationTick`; schema-20 snapshots migrate with dormant Wells at activation tick zero and non-dormant Wells conservatively assigned the snapshot's current tick, but Mission 12's `ECHOM12Q` envelope rejects that derived value by requiring native schema 21 and embeds the exact first eleven records before the snapshot. Primary and backup loads enforce the operation, exact ledger bytes, native snapshot schema, entity composition, and phase reconstruction. Campaign completion provenance accepts schema 21 through the running build's supported schema, rejecting older and unsupported future values. This is deterministic local binding and corruption rejection, not cryptographic authentication against forged local files. The accepted full playthrough is Preserve plan 07 only; the other 26 plans have plan/reducer coverage rather than complete end-to-end evidence.

Mission 13 is reduced by `FEchoesAssemblyOfTheMissingMissionModel`, not by HUD or presentation state. Admission requires the exact ordered Mission 01–12 ledger. The same three durable axes produce 27 plans: Mission 01 supplies doctrine, Mission 09 supplies the exact district pair, and Mission 10 supplies the recorded Lume protocol; Missions 11 and 12 remain mandatory receipts rather than new branch variables. Oruun and a distinct verifier are the only local commandable Kharuun Scouts. Meridian and Kharuun public-record interfaces plus the Crownfall public index are neutral, non-commandable public entities whose canonical IDs, ownership, faction, type, positions, inert combat state, and zero vision are revalidated on load. The reducer advances through simultaneous paired readback, an ordinary completed local UtilityStructure within three tiles of the Crownfall index, and separate Kharuun-side and Meridian-side assembly observations. Once the index link exists, readback remains durably reconstructable even after the scouts leave their first sites; there is no subsystem-only progress timer and no selectable Well. Protected/public-interface loss, an impossible link without prior readback, local-Core loss, or either terminal Core outcome fails closed. Success appends Mission 13 with the single inherited protocol bit and all eight exact Mission 13 completion facts. The `ECHOM13Q` envelope embeds the exact first twelve records before the unchanged native schema-21 snapshot. Primary and backup loads enforce operation identity, exact ledger bytes, native snapshot schema, exactly two owned Kharuun scouts, and exactly three canonical neutral public interfaces before acceptance. The reducer then reconstructs phase from the loaded authoritative state. This is deterministic local binding and corruption rejection, not cryptographic authentication against forged local files. The accepted complete playthrough is Preserve plan 07 only; the other 26 plans have plan/reducer coverage rather than complete end-to-end evidence.

Mission 14 is reduced by `FEchoesSeveralVoicesOneCommandMissionModel`, not by HUD or render-frame state. Admission requires the exact ordered Mission 01–13 ledger; the same doctrine, district-pair, and Lume-protocol axes produce 27 plans. Local authority is Hollow Choir. A protected Soldier voice must complete Held Alternatives and reconcile as Possible at its inherited site; a protected Heavy voice must reconcile as Manifest at its separate site; Neme, represented by the protected local Scout, must occupy the inherited command site. Shared Resolution cannot begin before all three identity and site facts hold. A worker then builds a completed local UtilityStructure Phase Anchor within three tiles of the inherited crisis site. The crisis hold lasts exactly 160 authoritative fixed ticks. Each fixed step audits both voices, Neme, research loom, Phase Anchor, local Core, all required sites, completed research, and terminal outcome; any post-start breach latches failure permanently even if later state appears repaired. Success appends Mission 14 with the inherited protocol bit and all eight exact facts. The `ECHOM14Q` version-2 envelope places operation, flags including the irreversible failure latch, ledger length/data, snapshot length/data, and a CRC32 under one integrity boundary. It requires the exact thirteen records and native schema 22. Primary and backup loads enforce the envelope, operation, ledger, schema, entity identities, sites, and reconstructed phase. Clearing only the persisted latch bit causes CRC rejection and accepts only a valid prior-generation backup. This is deterministic local integrity and corruption rejection, not cryptographic authentication against an actor able to forge a complete local envelope. The accepted full playthrough is Preserve plan 07 only; the other 26 plans have plan/reducer coverage rather than complete end-to-end evidence.

Mission 15 is reduced by `FEchoesBrokenSunMissionModel`; neither HUD strings nor frame timing advance it. Admission requires the exact ordered Mission 01–14 ledger. The doctrine, exact powered-district pair, and recorded Lume protocol retain 27 stable plan keys and derive an explicit bitmask over Restoration, Controlled Stabilization, Extinguishment, and Open Evolution. Controlled Stabilization is always earned; Restoration requires Preserve plus powered Life Support; Extinguishment requires the founding doctrine or Lume protocol to be Harvest; Open Evolution requires the founding doctrine or Lume protocol to be Reshape. Neme and the local Hollow Choir Soldier, Heavy, and Worker are player-owned; Mara and Talar remain neutral Meridian entities and Oruun remains a neutral Kharuun entity. An ordinary completed UtilityStructure at the inherited approach precedes both Choir technologies and the three-site Possible/Manifest/Neme accord. One eligible resolution must be requested twice. That selects a distinct convergence site and ending-specific hold duration derived from the deferred district and resolution, without yet writing the campaign ending. An ordinary local UtilityStructure becomes the exact Resolution Conduit. Every fixed step audits protected-force, witness, Core, and terminal-outcome validity. The bound Approach Anchor, selected accord and ending, and active-hold Resolution Conduit join the irreversible contract at their respective phase transitions; any applicable breach latches failure permanently. Success is committed transactionally by appending to a candidate ledger and then atomically saving it; only a successful append replaces in-memory progress. Campaign schema 2 adds separate final-resolution, availability-mask, and plan-key fields to each 27-byte record while decoding legacy schema-1 24-byte records. The `ECHOM15Q` version-2 envelope binds the exact Mission 01–14 schema-2 projection, native schema-22 snapshot, pending/selected resolution, hold/failure flags, hold start tick, structure IDs, lengths, and CRC32. Re-encoding an older ledger changes its bytes, so Mission 10–14 schema-1-bound checkpoint compatibility is explicitly false rather than silently inferred. The accepted full playthrough is plan 07 with Controlled Stabilization; the other endings have eligibility, reducer, record-validation, and replay coverage rather than complete end-to-end playthrough evidence. CRC32 provides corruption detection, not cryptographic authentication against an actor able to forge the whole envelope.
...
Version 0.77.0 implements the engine-independent admission subset in `NetworkProtocol`. The fixed compatibility packet canonically encodes the protocol, snapshot, simulation-rules, player-view-schema, build, rules-pack, map-pack, serialization-feature, and match-settings identities. It fails closed on length, magic, envelope-version, packet-kind, reserved-field, and CRC-32 errors before comparing manifests in a fixed diagnostic order. CRC-32 detects accidental corruption only; authentication, confidentiality, and malicious-tamper resistance belong to the future authenticated transport.

Version 0.78.0 carries that exact contract through Unreal reliable RPCs for one bounded localhost listen-server slice. The first remote owning controller is connection-bound to seat 1, a compatibility mismatch prevents activation, and the authority sends one canonical keyframe built solely from the admitted `PlayerView`. The format is capped at 256 KiB, 65,536 tiles, 4,096 entities, and 1,024 anonymous vibration signatures; it carries public resources, population, terrain/visibility/passability, scoped entities, last accepted command sequence, and a scoped consistency digest. Hidden entities, authoritative random state, opponent internals, and the full simulation checksum are structurally unavailable to the builder. The current connection binding is not identity authentication.

Version 0.79.0 adds a transport-level compatible/ready/started state machine. A listen authority remains paused with opponent AI suppressed until the compatible remote controller explicitly reports ready; start then releases the fixed tick, begins a 0.5-second full-keyframe timer, and reports the authority tick and three-tick input delay. Every sent snapshot records its scoped digest until acknowledged. Unknown, repeated, regressive, or digest-mismatched acknowledgements fail closed, and eight outstanding snapshots terminate the stream rather than permit unbounded retention. The client keeps a monotonic visibility-scoped view value: first and contiguous snapshots advance it, stale or duplicate snapshots do not mutate it, and a later full keyframe can recover a gap. Decode corruption requests a fresh full keyframe. Reliable localhost RPCs were used for the accepted test; no network fault was injected, so recovery under actual loss, reordering, or malicious traffic remains unqualified.

Version 0.80.0 adds `ScopedViewDelta` as a bounded engine-independent wire format. A delta names its base and resulting snapshot, carries canonical sorted tile/entity upserts plus removals, includes the complete reconstructed scoped-view digest, and is protected by the same packet CRC boundary. The client applies a delta only when its accepted snapshot exactly matches the named base; invalid removals, corrupt structure, digest mismatch, or unavailable base fail closed without mutating the accepted view. The authority sends a delta only when its encoding is smaller than the equivalent full keyframe. In the accepted normal smoke, the initial 4,537-byte keyframe was followed by 129-byte and 102-byte deltas. Deltas use the unreliable Unreal RPC path while full recovery keyframes remain reliable. Cumulative acknowledgements retire every earlier outstanding snapshot so a lost delta does not leak retained state.

Missing-base recovery is bounded on both peers to one request per second. The accepted `-EchoesNetworkDropFirstDelta` fault deliberately omitted delta 2; delta 3 was rejected with `NET_DELTA_BASE_MISSING`; the client requested recovery; and reliable full keyframe 4 restored the lineage before acknowledgement 4 retired the three outstanding snapshots. This qualifies that one deterministic missing-base path, not arbitrary latency, reordering, burst loss, malicious peers, or internet transport.

The 0.81.0 client retains the ordinary Enter-driven ready lobby and scoped presentation, then adds local selection, drag selection, control groups, formations, and adapters for the implemented tactical, building, production, research, and faction-special commands. These adapters use only the accepted scoped view to construct intent; they do not obtain the authority simulation or hidden state. The HUD exposes the enabled online controls and the server retains the assigned seat. One 1,440 × 900 Metal battlefield composition was inspected, and one controlled direct-Attack path completed a match. This is bounded Development evidence, not unaided-human online usability or complete multiplayer qualification.
...
The 0.77.0 command request deliberately carries no player identifier. Its fixed bounded encoding carries one actor and the complete deterministic `Command` payload. Local authority admission supplies the seat, requires the next contiguous per-seat sequence, rejects missing or non-owned actors, derives its execution window from the authoritative simulation tick with overflow checks, and mutates the accepted-sequence cursor only after `Simulation::QueueCommand` succeeds.

The 0.79.0 authority no longer trusts a client-selected execution tick. After decoding a connection-bound intent, it assigns `CurrentTick + 3`, applies the existing sequence/ownership/semantic admission gates, and reports both the requested and assigned values. The local-host adapter selects the same three-tick delay whenever a network human occupies the opposing seat; offline command timing remains unchanged. The retained separate-process smoke queues one host Move and one remote Move at authority tick 0, assigns both tick 3, and verifies both positions at tick 20.

Version 0.81.0 advances the wire protocol to 2 and adds `CommandBatchRequest`. One gesture carries a nonzero client-batch ID and at most 512 strictly ascending unique actor intents inside a 16 KiB cap; player identity, simulation sequence, and execution tick are absent. A syntactically valid next batch consumes its client-batch ID. The authority applies an eight-request and 1,024-intent fixed window, assigns each semantically accepted intent the next contiguous seat sequence and one common `CurrentTick + 3`, and reports accepted/rejected counts without allowing a rejected actor to advance the simulation sequence. CRC-32 remains accidental-corruption detection, not authentication. Native and Unreal coverage exercises canonical transport and malformed/bounded cases. The complete-match fixture accepts one 24-actor direct-Attack batch and ordinary combat result; this does not qualify every adapter, hostile traffic, abuse resistance, or internet transport.

When the authority observes a terminal deterministic outcome, it sends the final scoped keyframe reliably rather than as an unreliable delta, then sends the result tied to that exact snapshot ID, simulation tick, and scoped digest. The client presents the result only when its already accepted state matches all three values. The accepted fixture produced `Player1Victory` at tick 92, final snapshot 11, and seat-aware victory presentation. Version 0.83.0 adds only the bounded bearer-resume path described below; surrender/draw negotiation over transport, spectator scope, and host-departure policy remain unimplemented.

Version 0.82.0 distinguishes stale or duplicate deltas from forward gaps before base validation. A stale duplicate is ignored without mutating the accepted view or requesting recovery. A forward delta whose named base is unavailable still fails closed and requests a rate-limited reliable keyframe. Non-shipping, mutually exclusive client-delivery fixtures delay the first delta by 250 ms, duplicate it, deliver the first two newer-before-older, drop the first delta, or drop three consecutive deltas. All five cases plus normal delivery pass in separate Unreal processes. Because injection occurs after Unreal RPC delivery, this evidence validates state-machine behavior rather than socket, operating-system, WAN, jitter, congestion, or bandwidth behavior.

Version 0.83.0 adds an authority-owned reconnect reservation around the existing controller protocol. Once a compatible peer has been issued its credential, disconnect stores seat, match-start state, last accepted batch, and authority tick for 120 monotonic seconds; the running match keeps that seat human-owned and does not enable AI. A replacement connection has no seat authority until a reliable resume-credential RPC succeeds. The credential format is fixed at 32 hexadecimal characters, comparison covers the complete fixed-length value, invalid attempts preserve the reservation, and a successful match rotates the credential. Compatibility runs again after seat activation. The authority obtains the next command sequence from the live simulation, restores the connection-local batch ID, resumes the existing match, and begins a new per-connection snapshot lineage with a reliable full keyframe. This creates no serialized deterministic-match state and gives the client no hidden world state. The credential is an opaque Development bearer produced with `FGuid`; it is deliberately excluded from project server logging, but it is not authenticated identity, protected persistence, TLS, anti-replay session security, or hostile-client qualification.

Version 0.84.0 removes a timing assumption from the controlled completion protocol. After the client has accepted exact state and remote execution, it records the requested completion snapshot and sends a reliable server RPC. The authority accepts only that exact acknowledged snapshot after remote and host execution are verified, logs its pass, and returns a reliable client confirmation. The client compares the response with the stored requested snapshot rather than the latest view, because later legitimate deltas may advance the view while the confirmation is in flight. Only then does the client emit its pass marker and exit. This closes the shutdown race exposed by bidirectional NetDriver latency. The harness can apply 75 ms outgoing lag with 25 ms variance or 10% outgoing packet loss independently to both peers through Unreal's non-shipping `FPacketSimulationSettings`. An observed lost unreliable delta followed the ordinary missing-base/full-keyframe recovery path. These settings act within Unreal's NetDriver; they do not shape macOS traffic, impose a bandwidth ceiling, reproduce congestion, or establish WAN behavior.
...
Offline save content includes the complete core snapshot, pending commands, AI memory, scenario objectives, campaign fact ledger, and presentation-independent checkpoint metadata. Online clients do not save authoritative matches; the server retains reconnect state and replay. Unreal writes a temporary file, flushes it through the current platform API, validates it by reopening, then logically replaces the target while retaining the prior autosave generation. A corrupt newest autosave falls back only after the user is told which generation failed. The observed replace/reopen behavior and API-level rename-failure tests do not establish fsync, multiprocess exclusion, filesystem-wide crash consistency, or power-loss durability.
...
The Project Ledger is the authority for budgets and measured results. The architecture is designed around its 20 Hz, 400-unit slice budget: 4.0 ms p95 game-thread simulation, 1.5 ms p95 fog, 6.0 ms path burst, a checksum every 20 ticks within 0.25 ms p95, and 60 FPS at 2560×1440 medium on the inspected M1 Pro. PERF-003 remains the native observation: in an isolated optimized four-team harness with 100 units per team, visibility refresh measured 0.088500 ms p95, 100 cold path requests 0.411375 ms p95, simulation ticks with 100 moving units 0.133583 ms p95, and schema-9 state checksum 0.034102 ms p95. Historical exact-0.23.1 package captures passed their applied short-duration budgets. The earlier 0.93.0 package at clean commit `79d0cf3b45b7eeebed6d59a3939dae5ef0056421` does not pass the current Metal gate: its 32-entity startup measured 12.0798 ms GPU and 12.7095 ms render-thread p95, and its 400-unit/four-team fixture measured 12.9721 ms GPU and 13.6956 ms render-thread p95, all above the 11 ms limits. The corresponding frame, game-thread, and sampled-resident-memory budgets passed. A 180-second memory diagnostic failed its fitted slope limit at 470.201696 MiB/hour. A nominal 600-second report is rejected because the one-shot match became terminal before its 120-second warm-up, leaving no accepted sustained active-workload interval. The later material-qualified package at clean commit `dab16f281b9fe3fe84f463b556fe23dd56bd36ca` has not inherited or rerun those measurements. A valid soak requires a distinct sustained mode with replenishing work, explicit activity heartbeats, exact population/view/faction invariants, terminal/idle rejection, and qualification time counted only after readiness; only the same package that passes a 600-active-second preflight may attempt an uninterrupted 60-minute qualification. None of these observations constitutes clean-machine or release qualification.
...
## Mission 13 historical evidence boundary

As recorded on 2026-08-30, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, thirteen bounded campaign missions, and a runtime Unreal view/controller adapter. Version 0.90.0 at exact game-source commit `18ee3041f1e16d5a8c9c3b8099eef17f835a3499` retains the accepted networking and Missions 01–12, then adds **Assembly of the Missing** without changing native snapshot/replay schema 21. Content passed 27/27 with canonical SHA-256 `100f1fcd184cf94fe9b21d3f591714a2e33cc92b60f018bc6523868675156fa0`; native simulation/protocol coverage passed 29/29 in optimized, debug, and sanitizer configurations; the arm64 Development Editor built; and 43/43 Unreal automation tests passed with zero test warnings or errors. Mission 13 automation covers all 27 plan contracts, strict twelve-record admission, three neutral public interfaces, Kharuun-only authority, ordered readback/link/observation gates, native schema-21 persistence, exact primary/backup save-envelope binding and corrupted-primary recovery, one complete Harvest-founding/Life-Support-plus-Transit/Preserve route, thirteenth-record persistence, idempotent replay, and divergent replay refusal. The automation-index SHA-256 is `0481083ef6d17dc1db3fdf68406b266e2b5a7c578fed1185edbeb87de0bb629d`. A twelve-record Preserve startup smoke passed with log SHA-256 `d0b2c4e3a1546ac33de29fee40f310d61ed4f6aa1537ba85207c6ad87f0aa322`. A requested 1600x900 Metal review produced an observed 1319x768 app frame. The first Mission 13 briefing review exposed one clipped situation line; the line was shortened, the editor rebuilt, and the evidence replaced. The initially retained title file was a premature black frame and was rejected during final assurance. A separate exact-source Metal session recaptured a visible true-PNG title only after `[ECHOES_TITLE_READY]`; that title plus the accepted briefing, live objective/minimap layout, high-contrast view, and restored-default view showed no observed clipping or overlap. High contrast was restored off in the full review, and both Metal sessions exited normally. The Metal-log/title-recapture-log/title/brief/live/high-contrast/restored SHA-256 values are `631eff8911b3c1563e0a81e6cac874af1a134e179007ff2953c0b3c99eeba41f`, `7d95b2527381b2368d671e25f0d11c2f2cff8427af8a29f20a579b6bcf090c2d`, `1bfef0c9b693abbec77edbd4477fab807877e3356e3efa34f24dbaa89fec3049`, `6670d803b27db1881d30c834b05edd8e32be2e8a36f68e868c1cf1798b9d47f9`, `550fd31404c4a3ec638229801a42f931436d3159f725c9ab1b2db60e9248c094`, `110f683ccd3295adb065935982a5aa1f66713dd9bbc342546fc9afeb548cdadf`, and `2c5892ec785d3bd0fe11745075681da9d78336ea498bf87c93416d2e311e8774`. The full-review Metal log contains no project warning/error, fatal, assertion, or failed-boot marker; it retains 23 nonblocking editor/material warnings, so the visual result is not final-material qualification. Mission 13 establishes all 27 plan contracts and one complete automated Preserve route; it does not establish a full Reshape or other alternate playthrough, ordinary human completion, restoration, responsibility or authorship, consent, trust, civilian or survivor state, cryptographic authenticity, mixed-faction command, a playable Rhyse or Choir, broad balance/usability, or final assets. The NetDriver evidence remains non-shipping localhost packet simulation, not macOS traffic shaping, measured WAN behavior, or separate-machine play. The reconnect fixture remains a Development bearer path, not authentication, protected credential storage, encryption, hostile-traffic or abuse resistance, private invitations, spectators, or security qualification. Physical-human pointer usability, arbitrary or dynamically resized layouts, broad simultaneous-combat readability, final art/audio, current package performance, named or multiple campaign slots, Missions 14–15, complete multiplayer, and game completion remain open. Version 0.90.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.

## Current evidence boundary

As recorded on 2026-08-30 local time, version 0.93.0 retains the accepted networking, three source-reachable local skirmish factions, native snapshot/replay schema 22, campaign schema 2, and all fifteen bounded operations through **The Broken Sun**, then adds a campaign-journey controller around the persisted ledger. The resolver admits only an ordered, duplicate-free prefix of the fifteen authored records; `C` opens its exact next operation, failure remains on the current operation, and a complete ledger returns to the title/archive boundary without synthesizing Mission 16. New Campaign and Restore coordinate scenario reconstruction with transactional active/prior ledger generations. Replay conflict pauses the replay scenario and returns to the journey without changing either generation. Mission 02–09 campaign checkpoint containers bind operation, faction, and the exact prerequisite-ledger projection; unbound raw and version-1 bound campaign saves reject fail closed, while a compatible legacy noncampaign raw save remains loadable and is preserved during first container upgrade. The exact-source Unreal report created `2026.08.31-01.18.03` UTC records 47/47 successful tests, zero failed/not-run/in-process tests, and no warning or error entries; report SHA-256 is `bafabf416e726c212703263a1369465539aa14340c43f249a4ae11d2ad37d68f`. Its isolated `FreshJourney` case completes Preserve-to-Restoration and Harvest-to-Extinguishment routes from empty ledgers through Mission 15 and verifies generation swap/restore and replay-conflict immutability. A corrected exact-tree run moved every automation entry point under serial GUID-scoped temporary save storage, retained the suite-level fallback root, passed 47/47 with report SHA-256 `e3f227fec8231ebfb1328d3a48578f55f2d2d6929aa0cef28f953cb4706bba8d`, emptied the suite root, and produced equal real-player-tree manifests with SHA-256 `03d8ae063da0b6d612e1451e7211e441b3d9a2a4f1c92ff439d1076c0303c512`. The directory resolver preserves production and mission-specific filenames; explicit exact-file overrides remain non-shipping compatibility exceptions and take precedence only when deliberately supplied. This establishes serial test-path containment and equality of the sampled before/after tree states, not general concurrent command-line mutation safety, absence of transient writes or reads, an external-race exclusion, or broader persistence durability. The campaign journey itself remains headless controller/simulation evidence, not rendered UI, physical input, an ordinary-human playthrough, every route/ending, or packaged behavior. A one-record Metal title inspection exposes `C: CONTINUE CAMPAIGN`; its log/title SHA-256 values are `4ffb5a16e82626e7cba554c6c6a7abb62cbb2492186a8fe40948465bfe809c46` and `de48532c9f3491a979534b3a56b759f68b1e4493fa5df10a8332c446872ab231`. Those report and render artifacts identify version 0.93.0 but do not embed a Git SHA. Exact package provenance is separate: the retained Mac-arm64 Development package manifest binds clean source commit `79d0cf3b45b7eeebd6d59a3939dae5ef0056421`, version 0.93.0, UE 5.8.2, and Xcode 26.6 build 17F113; manifest/sidecar-file SHA-256 values are `de54024cd33fce495bc86fbadfbfbaa60808956dfa8f6372a6a9f3f151bd8540` and `83a817a2cc0236a5c6c99d048b12fe19174fd85fb6bc010d5507900471be413b`. Normal and 400-unit/four-team packaged null-RHI startup passed on this host, but both Metal profiles failed GPU/render budgets; the 180-second memory diagnostic failed its slope gate; and the nominal 600-second soak result is invalid because the match ended before warm-up. Recovery testing covers logical single-process persistence/reopen and controlled API-level rename failures only, not multiprocess exclusion, filesystem crash consistency, or power-loss durability. The package is ad-hoc signed, not notarized, and has no accepted sustained active-workload soak, clean-machine result, installer, or release qualification. Version 0.93 establishes journey/controller and checkpoint-binding behavior plus two automated routes, not ordinary human full-campaign completion, every alternate route/ending, cinematic endings, modeled downstream social consequences, broad three-faction balance/usability, final assets/audio, complete multiplayer, or game completion. The Project Ledger remains authoritative as evidence changes.

The later `world-surface-instancing-v1` package supersedes the earlier package only for material compatibility and source provenance. Its manifest binds clean commit `dab16f281b9fe3fe84f463b556fe23dd56bd36ca`; its local ad-hoc seal, normal/four-team startup, and two 1600x900 `METAL_SM5` sessions passed. The overview and ordinary-battlefield render logs contain no instancing/default-material fallback, and inspected frames contain no checkerboard or missing surface. The package has not passed a current performance profile or sustained soak. The ordinary frame's blown highlights, dense control overlay, and prototype copy remain explicit presentation debt. The retained acceptance manifest is `BuildArtifacts/Acceptance/WorldSurface-20260831T022650Z/acceptance_manifest.md`, SHA-256 `0aa4f8b758ed12ec36809abb35e9f9535ff1d4779c7b6ca013c151aa8e8924d1`.
...
- an authenticated, reconnectable, spectator-capable, separate-machine, hostile-network-qualified, or dedicated-server multiplayer service exists;
