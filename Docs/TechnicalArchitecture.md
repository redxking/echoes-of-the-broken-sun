...
Version 0.42.0 adds a code-only technology archive without moving authority into the HUD. The controller owns modal lifecycle, selected-producer resolution, exact-tier command translation, rejection feedback, and pause restoration. The HUD reads only catalog definitions and authoritative simulation/player state. `FEchoesTechnologyPanelLayout` is a shared pure screen-space calculation used by both rendering and pointer hit-testing; automation asserts that both tier targets and the close target remain within the accepted viewport and do not overlap. F2 opens the archive, Enter requests the next incomplete project, LMB targets a tier or close region, and F2/Escape/P closes. The UI path does not bypass `IssueResearchCommand` or its deterministic validation. A larger tree will require explicit keyboard focus, navigation, search/filter behavior, localization, and scalable graph layout rather than extending the bounded two-row panel indefinitely.
...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.42.0 presents both schema-19 faction research paths through a modal, pause-safe archive with shared render/hit geometry, keyboard confirmation, exact-tier pointer targets, and stable failure feedback. Clean commit `bf98bfba3b33a05515297bed11aaa5258727bc34` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 22/22 Unreal automation tests without warning or error, both normal faction boot directions, and the 400-unit/four-team active-combat runtime gate. At 1600×900, Metal evidence accepts normal/high-contrast Meridian, no-producer rejection, and Kharuun archive states without observed clipping or overlap. Active research progress and coordinate-click delivery remain unaccepted because the external control service still does not provide a dependable pointer result. Version 0.42.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. These artifacts establish deterministic research, Unreal integration, and bounded archive presentation, not a complete pointer-driven match, research balance, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
