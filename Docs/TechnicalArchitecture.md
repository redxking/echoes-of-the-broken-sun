...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.51.0 retains the 0.50.0 schema-20 research, deterministic selection and tactical controls, bounded Adaptive opening posture, and ordinary victory/research evidence while adding the first non-authoritative Glass Scar composition layer. Source commit `83e53720b1e0357cc972567e141ebfc140831ebd` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 22/22 Unreal automation tests without project warning or error, and both normal faction runtimes. An ordinary Metal run accepts the new magenta/copper fracture seam and shard silhouettes after fair-fog reveal. Exact coordinate-pointer delivery and ordinary player-driven research interruption remain unaccepted. Broad tactical-group usability, balance, performance of the new composition, final terrain/unit/effect/audio art, networked multiplayer, and game completion also remain open. Version 0.51.0 has not been packaged or profiled because free storage is below the packaging floor; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as test evidence changes.
