...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.46.0 retains schema-20 research, deterministic Tab/Backspace selection, and anonymous-contact presentation while adding discoverable F3–F6 Kharuun controls. Clean commit `38e9f70abe9d23d447584f9cb72bcae95c9f9c11` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 22/22 Unreal automation tests without warning or error, the Kharuun normal runtime, controlled Kharuun systems fixture, and 400-unit/four-team active-combat runtime gate. In an ordinary rendered match, direct Enter/Tab/F3 application input selected the Waystone and started migration. F4 correctly rejected a Riftstalker outside the Growth Basin field; F6 correctly required a valid world target. Those rejections establish delivery and validation only, not successful adaptation or cover. Coordinate-click delivery, a complete player-driven match, balance, a fully authored vertical slice, networked multiplayer, and game completion remain unaccepted. Version 0.46.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as test evidence changes.
