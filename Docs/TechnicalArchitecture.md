...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.40.0 retains the bounded compiler, catalog, complete slice-roster simulation-consumption path, and seven bounded faction mechanics. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, deterministic production/work/construction fields, complete four-unit/four-structure role coverage for each playable slice faction, the prior special profiles, vibration detection only on the two Kharuun detector roles, and powered defense only on the Meridian defense role. It atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar. Twenty-four adversarial tests exercise canonical stability and missing or misassigned special profiles.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256, validates it, and publishes an immutable catalog. Stable-ID bindings convert all eight named units and eight named structures into sixteen deterministic archetypes. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. The selected local faction is presentation/bootstrap state rather than a new simulation rule: the subsystem instantiates player 0 with the chosen faction, derives the other playable faction for player 1, and binds the same role-based spawn layout to faction-specific archetypes. Title or briefing selection performs a fail-safe scenario rebuild and restores the prior faction if reconstruction fails. Restarts retain the selection. Snapshot loading additionally requires the serialized player-0 faction to match the active choice; schema-18 and replay version 18 are otherwise unchanged. Command-line faction selection supplies a deterministic unattended/runtime test path without replacing the player-visible Tab flow. The HUD derives faction names, objective text, special-control guidance, and result language from the active subsystem state. The special profiles, anonymous vibration contacts, powered-Aegis graph, visibility/redaction rules, and simultaneous-damage behavior remain as recorded at 0.39.0. Map packs, technologies, localization, final presentation references, and signed manifests remain open.
...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.40.0 makes both slice factions selectable in the title and briefing and rebuilds the real two-player scenario around that choice. Automation proves local Kharuun ownership, Waystone command admission/transition, faction-specific production, local anonymous vibration disclosure, title-to-brief continuity, and the Tab input mapping. Separate null-RHI runs boot both player-faction directions and observe the Adaptive opponent through its materialized player view; the four-team scale fixture remains green. A 1600×900 Metal session visibly accepted the Meridian-to-Kharuun Tab change, dynamic operations brief, and Kharuun battlefield deployment. The content digest, schema-18 simulation state, seven mechanics, and 400-unit workload are unchanged. Coordinate-click delivery remains unaccepted because the external control service closed before returning and the game recorded no selection. Version 0.40.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. These artifacts establish rendered keyboard faction reachability, not a complete pointer-driven match, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
