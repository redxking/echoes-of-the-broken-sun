...
The 0.80.0 client now exposes an ordinary Enter-driven ready lobby and builds a client-only presentation from the accepted scoped view: scoped terrain and fog, scoped entity proxies, authored terrain candidates, lighting, and a camera framed on the assigned Kharuun seat. Its HUD explicitly identifies the view as remote observation and does not expose local selection or order claims. The server transfers the assigned seat to that client. This is rendered observation evidence, not a general network-human controller or complete match presentation.
...
The 0.79.0 authority no longer trusts a client-selected execution tick. After decoding a connection-bound intent, it assigns `CurrentTick + 3`, applies the existing sequence/ownership/semantic admission gates, and reports both the requested and assigned values. The local-host adapter selects the same three-tick delay whenever a network human occupies the opposing seat; offline command timing remains unchanged. The separate-process smoke queued one host Move and one remote Move at authority tick 0, assigned both tick 3, and verified both positions at tick 20. This proves parity for those two bounded Move paths, not every command or a playable online match. Actor-list batching, command-rate controls, authenticated sessions, scoped deltas, rendered client-side world presentation, complete match lifecycle, reconnect, and spectators remain open.

Version 0.80.0 adds a fixed per-connection command budget of eight admitted requests per second. Automated coverage accepts all eight, rejects the ninth, reopens the window after its fixed interval, and rejects commands from a compatible client that has not entered the started state. The existing smoke Move remains a bounded protocol/test adapter; general remote selection, actor-list batching, order construction, command-deck integration, and complete network match/result flow remain unimplemented.
...
As recorded on 2026-08-30, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Version 0.80.0 at exact clean game-source commit `b9c5c711e1ae567ef15ef6e865ed2efa7bfb6036` extends the separate-process transport slice with a player-facing ready lobby, compact base-identified scoped deltas, bounded missing-base recovery, cumulative acknowledgement retirement, per-connection command-rate control, and a rendered scoped remote-observation battlefield while retaining all earlier campaign, art, audio, responsive-HUD, and pointer evidence at its recorded limits. The exact source passes 27/27 content tests, 28/28 native suites in optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations, an arm64 Development Editor build, 37/37 Unreal automation tests without warning or error, a normal Meridian null-RHI bootstrap, normal and deliberately fault-injected localhost listen-server/client smokes, and two visually inspected 1,440 x 900 Metal lobby/battlefield reviews. The normal smoke emitted one 4,537-byte keyframe followed by 129-byte and 102-byte deltas and retired all acknowledgements. The injected smoke dropped the first delta, rejected the next because its base was missing, rate-limited a recovery request, accepted full keyframe 4, and retired all pending snapshots. Protocol and Unreal tests cover canonical keyframe/delta round trips, delta construction/application/rejection, compatibility mismatch, malformed/corrupt/oversized packets, connection-derived seat identity, actor ownership, sequence/tick bounds, scoped-state construction, hidden-entity exclusion, visible-hostile redaction, client lineage advance/rejection/recovery, the eight-request command window, not-started command rejection, and accepted execution. They do not establish general remote order adapters, complete network match/result flow, authentication, encryption, reconnect, spectators, separate-machine play, broader latency/reordering/burst-loss behavior, abuse resistance, or security qualification. Physical-human pointer usability, arbitrary or dynamically resized resolution/aspect-ratio/camera coverage, broad simultaneous-combat/camera/audio readability, final mix quality, music, voice, ambience, complete alerts, final effects, production textures, current package performance, named or multiple campaign slots, arbitrary campaign history, the remaining campaign, complete multiplayer, and game completion remain open. Version 0.80.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
...
- a complete host-authoritative match transport, reconnect, spectators, desync recovery, separate-machine play, or dedicated-server operation exists;
