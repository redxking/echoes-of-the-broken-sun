...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.30.0 implements the first bounded compiler/runtime portion of this design. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, and complete four-unit/four-structure role coverage for each playable slice faction, then sorts the records and atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar under `Content/Data/Generated`. Editor builds and package entry points run the compiler first. Eight adversarial tests exercise canonical stability and the principal rejection paths.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256 through a platform-independent implementation, validates the digest, versions, collections, identifiers, references, role coverage, and loaded numeric fields, and publishes an immutable catalog. `UEchoesSimulationSubsystem` refuses scenario creation unless that catalog is ready. Unreal automation checks the reviewed digest and selected authored values and proves that a tampered pack is rejected. Normal and stress runtime gates require the exact reviewed digest. The catalog now contains the full named roster definitions from the design bible—four units and four structures per slice faction—but this remains a catalog-integrity and match-start boundary. The deterministic simulation still instantiates technical-fixture statistics; faction-specific abilities, production mappings, map packs, technologies, localization, presentation references, signed manifests, and simulation consumption remain open.
...
The core must independently validate the eventual binary pack header, schema version, section lengths, counts, enum ranges, identifier uniqueness, references, numeric bounds, and checksum before constructing match state. Runtime never silently substitutes defaults for rejected authoritative content. The current Unreal catalog gate does not yet satisfy that core-consumption requirement.
...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions, a strict canonical content compiler, an immutable digest-verified Unreal content catalog, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. The adapter currently generates a two-player playable placeholder arena or an explicit four-team scale fixture, and exposes an uncaptured/unlocked visible RTS cursor, feature-gated title, responsive predeployment operations brief, visibility-scoped live objectives, modal victory/draw/defeat and redeployment, a modal field pause/options menu, camera, visibility-scoped entity proxies, selection, context orders, deterministic attack-move, patrol, hold, guard, and stop, construction placement commands, timed one-slot production, logistics capacity, pause/restart, derived Command-Core victory, transactional checkpoints, persistent accessibility settings, a fog-respecting tactical minimap, a HUD, an Adaptive opponent with deterministic expansion/retreat behind the materialized player-view boundary, and Future Well orders. Match creation now requires the reviewed eight-unit/eight-structure catalog, but simulation entity statistics and behaviors still come from technical fixtures rather than that catalog. The scale fixture creates 100 owned units per active player and one central Well, queues 396 deterministic broad attack-move orders, enters actual combat, and uses lightweight reduced-motion-aware atmosphere plus reduced-flashing-aware damage feedback. It is still a synthetic performance fixture rather than authored final gameplay. Version 0.30.0 is editor-built and automation/runtime-verified; its presentation is unchanged from the bounded 0.27.0 Metal inspection, and it has not yet been packaged or profiled. The latest accepted package remains 0.23.1. Those artifacts establish a bounded playable-systems prototype, complete named roster definitions, and content-loading boundary—not complete faction mechanics, data-driven authoritative gameplay, networked multiplayer, a vertical slice, or a completed game. The Project Ledger remains authoritative as test evidence changes.
