...
The current 0.27.0 implementation is a deterministic precursor to that final boundary. `GenerateAiCommands` still lives inside the simulation rather than behind a separately materialized immutable `PlayerView`, so architectural isolation is not yet complete. It limits opponent targeting and Future Well interaction to `IsEntityVisibleTo`, limits expansion sites to `Visibility::Visible` plus the same placement validator, uses only the AI player's resources/population/owned structures for production and expansion, and applies no standard-difficulty bonuses. Expansionist and Adaptive personalities are implemented alongside the prior Balanced, Defensive, Raider, and Economic modes. Personality-specific capacity pressure governs Barracks/drop-off construction, and personality-specific health thresholds govern withdrawal to the owned Command Core. Native tests cover hidden-enemy exclusion, visible-terrain expansion, standard validation, invalid-personality rejection, and low-health retreat overriding an otherwise visible attack target.
