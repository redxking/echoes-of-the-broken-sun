...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Current version 0.67.0 at game-source commit `b4a6ce21c6d170de17c08c64868fa04c94705831` retains **The Shape of Silence**, the registered Glass Scar world candidates, corrected review-mode precedence, deterministic selected-producer research cancellation, and interaction-specific accepted-order presentation while adding validated, reversible activation of one prior campaign generation. The exact source passes 27/27 compiler tests, 27/27 native suites in optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations, an arm64 Development Editor build, 34/34 Unreal automation tests without warning or error, and a normal Meridian null-RHI bootstrap. Exact-commit Metal evidence accepts the title's two-press Page Up path from an empty active ledger and seven-record prior generation to an active seven-record ledger and retained empty backup. Earlier exact-source Metal evidence retains its stated boundaries for research interruption, exact-coordinate pointer interaction, and the public nine-page archive. Physical-human pointer usability, pointer combat/Guard coverage, adverse camera/UI-scale cases, broad combat/camera readability, final topology/UVs/materials/destruction/VFX/audio, current package performance, named or multiple campaign slots, arbitrary campaign history, the remaining campaign, networked multiplayer, and game completion remain open. Version 0.67.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
