...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.35.0 implements the bounded compiler, catalog, complete slice-roster simulation-consumption path, and three authored faction-mechanic profiles. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, deterministic production/work/construction fields, complete four-unit/four-structure role coverage for each playable slice faction, the Meridian deployment and supply profiles, and a migration profile restricted to the Kharuun mobile supply node. It atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar. Fourteen adversarial tests exercise canonical stability and missing or misassigned special profiles.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256, validates it, and publishes an immutable catalog. Stable-ID bindings convert all eight named units and eight named structures into sixteen deterministic archetypes. The Waystone profile converts 120 cm/s into Q22.10 per-tick movement and carries uproot/root timing plus the non-rooted damage multiplier. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. Rules version 1 and the 32-byte content digest are serialized in schema-14 snapshots and replays and included in checksums. Waystone rooted/mobile/transition state is public, while opponent transition ticks and health remain redacted. Only rooted completed Waystones provide Logistics or receive Matter; starting a transition clears movement; only the mobile state accepts move orders; non-rooted states take authored additional damage; rooting checks terrain and structure occupancy at both start and completion and returns to mobile if the footprint becomes invalid. Unreal automation checks catalog mapping, the real scenario Waystone, public presentation, invalid control rejection, Hyphen mapping, and all prior regressions. Remaining faction abilities, map packs, technologies, localization, final presentation references, and signed manifests remain open.
