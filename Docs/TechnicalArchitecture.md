...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.39.0 implements the bounded compiler, catalog, complete slice-roster simulation-consumption path, and seven bounded faction mechanics. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, deterministic production/work/construction fields, complete four-unit/four-structure role coverage for each playable slice faction, the prior special profiles, vibration detection only on the two Kharuun detector roles, and powered defense only on the Meridian defense role. It atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar. Twenty-four adversarial tests exercise canonical stability and missing or misassigned special profiles.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256, validates it, and publishes an immutable catalog. Stable-ID bindings convert all eight named units and eight named structures into sixteen deterministic archetypes. The special profiles map authored ranges, timings, costs, modifiers, footprints, detection resolution, and tradeoffs into fixed authoritative rules. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. Rules version 1 and the 32-byte content digest are serialized in schema-18 snapshots and replays and included in checksums. Vibration detection retains its separate anonymous player-view contact vector. Powered Aegis state is recomputed from completed, surviving Meridian network nodes: Anchors seed the graph, Power Links join only when within the authored 800-cm radius of an already powered node, and an Aegis is powered only when within that radius of the resulting graph. A powered Aegis autonomously selects the nearest ordinarily visible hostile entity within its authored 900-cm range, with entity ID breaking equal-distance ties; it deals 28 damage on a 20-tick cadence. It stores no target and accepts no movement/tactical-order behavior. Simultaneous damage resolves before destroyed links are removed and the network is recomputed, so a shot and link destruction in the same tick both resolve deterministically; no later shot occurs while disconnected. The public `aegisPowered` flag is serialized, checksum-covered, opponent-visible after normal entity visibility, and validated against the reconstructed graph on load, while enemy combat statistics remain redacted. Unreal world and minimap geometry distinguishes powered state without color alone. Automation checks canonical mapping, the powered actual-scenario Post, powered/unpowered geometry, and all prior regressions. Map packs, technologies, localization, final presentation references, and signed manifests remain open.
...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. The adapter currently generates a two-player playable placeholder arena or an explicit four-team scale fixture, and exposes the prior playable-systems controls plus seven bounded faction mechanics, anonymous vibration-contact presentation, powered-Aegis world/minimap state, persistent accessibility settings, a fog-respecting tactical minimap, and an Adaptive opponent operating behind the materialized player-view boundary. Match creation applies authored values to all sixteen named slice archetypes and Future Wells. The scale fixture creates 100 owned units per active player and one central Well, queues 396 deterministic broad attack-move orders, enters actual combat, and uses lightweight reduced-motion-aware atmosphere plus reduced-flashing-aware damage feedback. It remains a synthetic performance fixture rather than authored final gameplay. Version 0.39.0 is editor-built and automation/runtime-verified; the normal null-RHI fixture observed one powered scenario Aegis but not live Aegis fire or link severing. New special-state presentation has not received a Metal acceptance pass, and 0.39.0 has not been packaged or profiled. The latest accepted package remains 0.23.1. These artifacts establish foundational data-driven gameplay and seven bounded faction mechanics, not complete faction identity, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
