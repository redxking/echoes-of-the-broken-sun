...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.53.0 retains the 0.50.0 interaction/research/AI/victory evidence, the 0.51.0 non-authoritative Glass Scar composition, and the 0.52.0 faction/role silhouettes while adding transient accepted-order markers. Source commit `9eb0e935cda38cf00330f94f36ea2dedc9576457` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 23/23 Unreal automation tests without project warning or error, and both normal faction runtimes. Ordinary Metal runs accept visible Attack-move and Patrol markers at the normal RTS camera and confirm the reduced-motion/reduced-flashing branch with settings restored after capture. Exact coordinate-pointer delivery and ordinary player-driven research interruption remain unaccepted. Broad tactical-group usability, balance, performance of the new presentation layers, final meshes/materials/effects/audio, networked multiplayer, and game completion also remain open. Version 0.53.0 has not been packaged or profiled because free storage is below the packaging floor; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as test evidence changes.
