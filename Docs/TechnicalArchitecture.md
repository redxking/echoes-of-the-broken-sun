...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. Version 0.41.0 binds the two-step research path for both selectable factions into schema-19 simulation, player-view AI, save/replay/checksum state, controller feedback, and HUD status. Clean commit `3e77039534f6c449088d00c543f092614ca74ffc` passes 27/27 compiler tests, 27/27 native suites in each of three configurations, an arm64 Development Editor build, 22/22 Unreal automation tests without warning or error, both normal faction boot directions, and the 400-unit/four-team active-combat runtime gate. The accepted 1600×900 Metal evidence remains the earlier Meridian-to-Kharuun Tab change, dynamic operations brief, and Kharuun battlefield deployment; the new research flow has not been visually accepted. Coordinate-click delivery remains unaccepted because the external control service closed before returning and the game recorded no selection. Version 0.41.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. These artifacts establish deterministic and Unreal-integrated research behavior, not a complete pointer-driven match, research balance, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
