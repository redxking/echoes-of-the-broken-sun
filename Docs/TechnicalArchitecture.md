...
The 0.78.0 slice binds the first remote controller to seat 1, suppresses that seat's AI, seeds the network sequence cursor from the authoritative simulation, and uses the architecture's three-tick minimum delay. Its separate-process smoke holds the authority at tick zero until one compatible client submits one Move, then verifies authoritative execution and returns admission/execution results. This is transport evidence for one command, not a lobby or playable online match. Actor-list batching, host/local command parity, rate limiting, authenticated sessions, periodic keyframes/deltas, client-side world presentation, match lifecycle, reconnect, and spectators remain open.
...
As recorded on 2026-08-30, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Version 0.78.0 at exact clean game-source commit `02c0b62d75d16d6f30575d01f0a507e0fd68a136` adds the first separate-process transport slice described above while retaining all earlier campaign, art, audio, responsive-HUD, and pointer evidence at its recorded limits. The exact source passes 27/27 content tests, 28/28 native suites in optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations, an arm64 Development Editor build, 37/37 Unreal automation tests without warning or error, a normal Meridian null-RHI bootstrap, and one normal-exit localhost listen-server/client smoke. Both processes observed the same 4,537-byte scoped keyframe and digest; one remote sequence-1 Move was admitted at authority tick 0 for tick 3 and verified at tick 20. Protocol and Unreal tests cover canonical round trips, compatibility mismatch, malformed/corrupt/oversized packets, connection-derived seat identity, actor ownership, sequence/tick bounds, scoped-state construction, hidden-entity exclusion, visible-hostile redaction, keyframe tamper rejection, and accepted execution. They do not establish authentication, encryption, a lobby, periodic snapshots/deltas, host/local input parity, client world presentation, match completion, reconnect, spectators, latency/loss behavior, abuse controls, or separate-machine play. Physical-human pointer usability, arbitrary or dynamically resized resolution/aspect-ratio/camera coverage, broad simultaneous-combat/camera/audio readability, final mix quality, music, voice, ambience, complete alerts, final effects, production textures, current package performance, named or multiple campaign slots, arbitrary campaign history, the remaining campaign, complete multiplayer, and game completion remain open. Version 0.78.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
