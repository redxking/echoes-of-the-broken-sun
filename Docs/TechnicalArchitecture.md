...
The source-controlled JSON under `Content/Data/Source` is the human-editable balance source. It is not loaded directly into an authoritative match. Version 0.38.0 implements the bounded compiler, catalog, complete slice-roster simulation-consumption path, and six authored faction-mechanic profiles. `Scripts/compile_content.py` accepts only the declared schema and exact fields, rejects malformed types and values, checks stable identifiers, display names, duplicates, references, numeric bounds, structure roles, Future Well rules, deterministic production/work/construction fields, complete four-unit/four-structure role coverage for each playable slice faction, the prior five special profiles, and vibration-detection profiles required only on the Kharuun scout/counter-scout and detection structure. It atomically emits canonical UTF-8 JSON plus a bare SHA-256 sidecar. Twenty-two adversarial tests exercise canonical stability and missing or misassigned special profiles.

`UEchoesContentSubsystem` loads only the generated pack, recomputes SHA-256, validates it, and publishes an immutable catalog. Stable-ID bindings convert all eight named units and eight named structures into sixteen deterministic archetypes. The special profiles map authored ranges, timings, costs, modifiers, footprints, detection resolution, and tradeoffs into fixed authoritative rules. `UEchoesSimulationSubsystem` refuses scenario creation if any binding, role, digest, or conversion is invalid. Rules version 1 and the 32-byte content digest are serialized in schema-17 snapshots and replays and included in checksums. Actual deterministic movement refreshes an internal signature-expiry tick on mobile entities. `CreatePlayerView` emits a separate sorted, de-duplicated `VibrationSignature` vector only when an owned completed Resonant or Listening Spine covers an enemy mobile source that is not ordinarily visible. Contact positions are snapped to the authored 200-cm cell center; the signature type contains no entity identifier or identity/stat/order fields. The enemy entity itself remains absent. Visible sources suppress duplicate signatures, and AI can respond only with attack-move to the approximate point. Snapshot validation bounds signature lifetime and rejects signatures on neutral or immobile entities. The Unreal HUD renders labeled diamond-wave contacts in world and minimap space without spawning selectable entity actors; current Meridian-only operation means standard local presentation does not exercise them live. Unreal automation checks authored mapping, actual detector actors, anonymous view materialization, quantization, AI indirect response, HUD construction, and all prior regressions. Remaining faction abilities, map packs, technologies, localization, final presentation references, and signed manifests remain open.
...
As recorded on 2026-08-29, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, and a runtime Unreal view/controller adapter. The adapter currently generates a two-player playable placeholder arena or an explicit four-team scale fixture, and exposes the prior playable-systems controls plus six bounded faction mechanics, anonymous vibration-contact presentation, persistent accessibility settings, a fog-respecting tactical minimap, and an Adaptive opponent operating behind the materialized player-view boundary. Match creation applies authored values to all sixteen named slice archetypes and Future Wells. The scale fixture creates 100 owned units per active player and one central Well, queues 396 deterministic broad attack-move orders, enters actual combat, and uses lightweight reduced-motion-aware atmosphere plus reduced-flashing-aware damage feedback. It remains a synthetic performance fixture rather than authored final gameplay. Version 0.38.0 is editor-built and automation/runtime-verified; its new contact presentation has not received a Metal acceptance pass, and it has not been packaged or profiled. The latest accepted package remains 0.23.1. These artifacts establish foundational data-driven gameplay and six bounded faction mechanics, not complete faction identity, a fully authored vertical slice, networked multiplayer, or a completed game. The Project Ledger remains authoritative as test evidence changes.
