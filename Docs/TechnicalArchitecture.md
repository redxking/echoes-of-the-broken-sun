...
When the authority observes a terminal deterministic outcome, it sends the final scoped keyframe reliably rather than as an unreliable delta, then sends the result tied to that exact snapshot ID, simulation tick, and scoped digest. The client presents the result only when its already accepted state matches all three values. The accepted fixture produced `Player1Victory` at tick 92, final snapshot 11, and seat-aware victory presentation. Reconnect, surrender/draw negotiation over transport, spectator scope, and host-departure policy remain unimplemented.
...
As recorded on 2026-08-30, the repository contains a UE 5.8 project, Mac configuration, source JSON for both complete named slice-roster definitions and four faction technologies, a strict canonical content compiler, an immutable digest-verified Unreal catalog, versioned deterministic rules, design records, a tested four-player-capable engine-independent simulation, seven bounded campaign missions, and a runtime Unreal view/controller adapter. Version 0.82.0 at exact clean game-source commit `db59fb103f1cc2c33b188e097d6afd293c480961` retains the protocol-2 remote-order and complete-match work and adds bounded deterministic handling for delayed, duplicate, reordered, single-loss, and three-loss-burst scoped deltas. Six independent server/client delivery pairs, the complete controlled 24-actor match, 27/27 content tests, 28/28 native suites in optimized, debug, and sanitizer configurations, the arm64 Development Editor build, 37/37 Unreal automation tests without warning or error, and the normal Meridian bootstrap passed. Duplicate state was ignored without recovery; forward gaps recovered only through an exact reliable keyframe. These are client-delivery injections, not measured network conditions. The 0.81 rendered battlefield review and all earlier campaign, art, audio, responsive-HUD, and pointer evidence remain accepted only at their recorded limits. This does not establish unaided-human online usability, every remote adapter, authentication, encryption, disconnect/reconnect, spectators, separate-machine play, operating-system traffic behavior, hostile-traffic or abuse resistance, private invitations, or security qualification. Physical-human pointer usability, arbitrary or dynamically resized layouts, broad simultaneous-combat readability, final art/audio, current package performance, named or multiple campaign slots, the remaining campaign, complete multiplayer, and game completion remain open. Version 0.82.0 has not been packaged or profiled; the latest accepted package remains 0.23.1. The Project Ledger remains authoritative as evidence changes.
