...
The project pins Unreal Engine 5.8. Epic's version-specific macOS table lists Xcode 26.0 as the minimum and 26.1.1 as recommended, and explicitly marks 26.4 incompatible. The locally installed Xcode 26.6 has passed project generation, arm64 Development Editor compilation, five Unreal automation tests, null-RHI and rendered startup, and a local Development cook/package. That is positive evidence on this host, not a clean-machine, Developer ID signing, notarization, performance, or full compatibility result. Xcode 26.1.1 remains Epic's recommended support baseline.
...
The observed current results are: workspace generation succeeded; `EchoesOfTheBrokenSunEditor Mac Development` compiled and linked for arm64; seven Unreal automation tests passed 7/7 with no test warning or error; and the null-RHI game smoke selected `EchoesGameMode`, created a 25-entity scenario with 10 initially visible entity views, the 165-tile Glass Scar layout, three crossings, and a 4,096-tile fog/shroud surface at 20 Hz, emitted every required readiness marker, and exited cleanly. `Echoes.Runtime.Bootstrap.ClassesAndCore` covers class registration, required input mappings—including `[F]` attack-move, `[X]` stop, `[K]` checkpoint, `[L]` load, `[G]` assignment mode, and `[1-0]` recall—and a one-tick portable-core call. `Echoes.Runtime.Controls.ControlGroups` proves group index validation, local-entity filtering, duplicate removal, hostile rejection, and explicit clearing. `Echoes.Runtime.Gameplay.ProductionPauseRestart` uses the real world subsystem to prove pause, worker and soldier production, logistics accounting, and deterministic restart. `Echoes.Runtime.Gameplay.CompleteSkirmish` proves one composed automated match across the Glass Scar: drop-off construction, legitimate Future Well scouting and capture, reinforcement production, safe-side group rally, one-order attack-move engagement, opposing Command-Core destruction, post-victory simulation freeze, and restart. `Echoes.Runtime.Map.GlassScar` proves the exact blocked count, all three authored crossings, closed-span samples, presentation parity, central Well location, and deterministic routing into the northern basin. `Echoes.Runtime.Persistence.QuickSaveLoad` proves exact tick/checksum restoration, prior-generation backup recovery after intentional primary corruption, view reconstruction, and valid post-load command sequencing while preserving any pre-existing test-host checkpoint. `Echoes.Runtime.Visibility.ActorLifecycle` proves fog tile accounting, explored-area growth, visible-to-shrouded transition, actor creation on reveal, destruction when hidden, single-actor recreation on reentry, and health-bar visibility/fraction behavior for full-health, selected, damaged, and restored entities. The null-RHI smoke covers bootstrap only. These results do not validate a pointer-driven rendered match, sustained simulation, production-scale navigation/fog performance, broader gameplay completeness, or clean-machine installation.

## Rendered prototype boundary

Local Metal SM5 Editor runs and the current 0.7.0 packaged application were visually inspected at the runtime-generated arena. The 0.8.0 source HUD displays resources, logistics, match state, construction/production hotkeys, `[F]` attack-move, `[X]` stop, `[G]` then `[1-0]` assignment, bare `[1-0]` recall, `[K]` checkpoint, `[L]` load, `[Z/C/V]` Well choices, and the `PLAYABLE SYSTEMS BUILD — ACTIVE DEVELOPMENT` boundary without observed clipping. A rendered 0.8.0 Editor run showed `G` then `1` reaching the assignment path and clearing an empty group, followed by bare `1` reaching recall and reporting `[GROUP_EMPTY]`; this proves the bounded keyboard paths, not selected-unit assignment. An earlier rendered Editor run visibly committed a checkpoint at tick 830, advanced beyond it, then restored tick 830 with `[L]`; the packaged 0.7.0 application separately committed tick 338 and restored tick 338. Terrain, fog, entities, HUD, and normal lit rendering remained intact. Bare and modified function-key candidates were rejected during inspection because Unreal diagnostic view modes consumed them, and Ctrl+S/Ctrl+L were rejected because the application layer intercepted them. The tile-instanced fog surface visibly separates the local known area from unrevealed territory without hiding friendly units or the HUD. A prior Editor run showed that unexplored fog fully concealed the terrain presentation; with the simulation paused and only the disposable fog actor removed for visual QA, the two blocked span sections and the wide central crossing were distinct without observed clipping or HUD overlap. That inspection did not alter source or establish what a player has legitimately scouted. The desktop automation bridge still cannot reliably deliver pointer clicks or drag gestures into the Metal window, so exact-build selection, construction placement, context movement, gathering, production selection, attack-move pointer delivery, full control-group assignment with selected units, and combat remain unaccepted as manual interactions. These are rendered startup and bounded visual/keyboard observations, not evidence of a complete rendered playable loop, final terrain or fog art, or performance.
...
./Scripts/run_packaged_smoke.sh \
  BuildArtifacts/Packages/Mac-Development-v0.7.0-checkpoints/EchoesOfTheBrokenSun.app
```

The current observed local artifact is a 748 MB self-contained arm64 Development application with bundle identifier `com.angelispseftis.echoesofthebrokensun`, short version `0.7.0`, and five PAK/IoStore container files. Unreal's Xcode packaging phase seals the app before the final PAK update, so the wrapper reseals the completed archive with the checked-in development entitlements and verifies it afterward. The exact archive passed strict deep-signature verification and emitted the environment, Glass Scar, 4,096-tile fog, simulation, boot-ready, and first-tick markers when its packaged executable was launched. Its adjacent manifest records clean source commit `bcdb6d99dbfa153183461c8bd14c144d28c70d09`, UE 5.8.2, Xcode 26.6, configuration and platform identity, and the SHA-256 of every regular file and symlink in the app. The manifest digest is `9c72ce7242a4062752b6455b37d0c12e217fdaec17c207602dadf18b970516d4`.
