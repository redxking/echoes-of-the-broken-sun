...
The packaged Metal profile is a separate gate and defaults to the accepted 0.22.1 package. Run both the normal playable fixture and the explicit active-combat scale fixture:
...
The observed current results are: workspace generation succeeded; `EchoesOfTheBrokenSunEditor Mac Development` compiled and linked for arm64; twelve Unreal automation tests passed 12/12 with no test warning or error; and the normal null-RHI game smoke selected `EchoesGameMode`, created a 25-entity scenario with 10 initially visible entity views, the 165-tile Glass Scar layout, three crossings, and a 4,096-tile fog/shroud surface at 20 Hz, emitted every required readiness marker, and exited cleanly. The separate stress smoke created 400 owned units, 100 per active player across four teams, one central Future Well, 401 visibility-scoped view actors, and 396 deterministic attack-move orders; it then observed actual damage. Scale automation independently verifies the fixture and ongoing initial outcome. The complete-skirmish test remains green and verifies that the seven-unit strike force rallies before crossing hostile territory. These checks do not validate pointer-delivered orders in a rendered match, final effects/formations, network transport, broader gameplay completeness, or clean-machine installation.

## Rendered prototype boundary

The exact 0.22.1 packaged process was identified by its full executable path and visually inspected through Metal. Terrain edges, fog boundary, entities, selection/platform presentation, HUD, feedback surface, and the fog-respecting tactical overview remained visible without observed clipping. Normal and high-contrast overview treatments were inspected. Comma lowered the camera zoom-step setting and Period restored it without activating Unreal's Development debug camera; the conflicting Semicolon binding discovered in 0.22.0 is therefore resolved. Logs independently recorded `[ECHOES_MINIMAP_READY]` with 10 normal-fixture markers and 401 stress-fixture markers. Actual patrol/routing/combat semantics remain automation-covered rather than manually accepted. The desktop automation bridge still cannot reliably deliver pointer clicks or drag gestures into the Metal window, so selected-unit patrol/hold, pointer-targeted guard, construction, context movement, gathering, production selection, attack-move delivery, control-group assignment with selected units, and combat remain unaccepted as manual interactions. These are bounded presentation observations, not evidence of a complete rendered playable loop or final terrain/fog/UI art.
...
./Scripts/run_packaged_smoke.sh \
  BuildArtifacts/Packages/Mac-Development-v0.22.1-tactical-overview/EchoesOfTheBrokenSun.app
./Scripts/run_packaged_stress_smoke.sh \
  BuildArtifacts/Packages/Mac-Development-v0.22.1-tactical-overview/EchoesOfTheBrokenSun.app
```

The current observed local artifact is a self-contained arm64 Development application with bundle identifier `com.angelispseftis.echoesofthebrokensun`, short version `0.22.1`, and five PAK/IoStore container files. Unreal's Xcode packaging phase seals the app before the final PAK update, so the wrapper reseals the completed archive with the checked-in development entitlements and verifies it afterward. The exact archive passed strict deep-signature verification plus normal and active-combat packaged startup. Its adjacent manifest records clean source commit `b854ef29a107b80889c6e395c91054ae6fdb9898`, UE 5.8.2, Xcode 26.6, configuration and platform identity, and the SHA-256 of every regular file and symlink in the app. The manifest digest is `ded81b77f2c68efe2f75aa21997a77574231dab7888011da891a22834aecff04`; normal and stress smoke logs hash to `22367bc7ba357568834924500f8d3b00a001b4e54bce717a0d379dcccad31c47` and `37b8cf5172cc43ee27fd25b8f54377e8fedd99d40d9efc29c4c4dbe9c9b3a52c`. The exact package's normal 25-entity Metal profile passed at 10.8812 ms frame, 10.8696 ms render-thread, 10.3285 ms GPU, 0.9060 ms game-thread p95, and 612.375 MiB peak sampled RSS. Its separate active-combat 400-unit/four-team profile passed at 10.9302 ms frame, 10.9176 ms render-thread, 10.3787 ms GPU, 0.9985 ms game-thread p95, and 645.578 MiB peak sampled RSS. Summary hashes are `83c500fb51fe53289e2f4472d50310692dc39bbb6a92f0c86688cfbd7111dd20` and `29377465e086fd65ba07ce1c2a125b3bbfa05ccdb4a41bae16506034e51b13cd` respectively. The exact rebuildable staging directory was moved recoverably to the Trash path recorded above after qualification; the accepted archive was preserved.
