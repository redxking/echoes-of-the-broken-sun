...
The observed current 0.26.1 source results are: workspace generation succeeded; `EchoesOfTheBrokenSunEditor Mac Development` compiled and linked for arm64; twelve Unreal automation tests passed 12/12 with no test warning or error; and the normal null-RHI game smoke selected `EchoesGameMode`, created a 25-entity scenario with 10 initially visible entity views, the 165-tile Glass Scar layout, three crossings, and a 4,096-tile fog/shroud surface at 20 Hz, emitted every required readiness marker, and exited cleanly. The separate stress smoke created 400 owned units, 100 per active player across four teams, one central Future Well, 401 visibility-scoped view actors, and 396 deterministic attack-move orders; it then observed actual damage. Scale automation independently verifies the fixture and ongoing initial outcome. Runtime bootstrap automation now also verifies that the visible RTS cursor is not captured on launch, not captured during clicks, and not locked to the viewport. The gameplay test verifies that interactive launch presents the feature-gated title, hides unavailable modes, holds the tick fixed, advances to the operations brief on Enter, deploys on the next Enter, then opens the field menu, verifies its modal boundary and fixed tick, and resumes through Enter while retaining all prior production/pause/restart coverage. The complete-skirmish test reaches authoritative player victory, presents the result modal, restarts through the Enter primary action, and verifies an ongoing deterministic replacement match. Objective tests bind hostile-core status to current local visibility rather than hidden state. The clean evidence files at source commit `34bef3754d0326a371339ded5bdf70dfc246774a` hash to `45572eb0cf78bcdbf0320ee99e9ab7d42b25aa4b05173c18e833147f3a4b6981` for the automation index, `f7a8d76c2bac699b21b607d03956466266eabf63d0cdd3b5c89d36e2b44678c2` for normal runtime, and `873ce897f67c0129fb1a1b690cbf432e941b482542113336926580ee22391eba` for stress runtime. These checks do not validate pointer-delivered orders in a rendered match, final effects/formations, network transport, broader gameplay completeness, or clean-machine installation.

## Rendered prototype boundary

The exact 0.23.1 packaged process was identified by its full executable path and visually inspected through Metal at native 2560×1440. Its responsive Glass Scar operations brief was legible without observed clipping, paused deployment, and transitioned to the battlefield and tactical HUD on Enter. The 0.23.0 package had exposed a real defect: its briefing retained a 920-pixel cap and rendered too small on the Retina display. That package is superseded; the proportional 0.23.1 layout is the accepted packaged result. Current 0.26.0 editor-source inspection at 1600×900 showed the title screen without observed overlap or clipping in normal and high-contrast modes. The title exposed only Glass Scar single-player and implemented settings; unavailable campaign, multiplayer, and replay modes were absent. Enter advanced to the high-contrast operations brief, the preference was restored, and a second Enter deployed; the rendered battlefield showed tick five and `DEPLOYED`. The visual log records title-ready, title-confirmed, briefing-ready, and briefing-dismissed markers. Version 0.26.1 separately changed Unreal's cursor policy to no launch capture, no click capture, and no viewport lock. A live Metal retry still found that the desktop automation bridge closed its native channel on coordinate click and Unreal recorded no selection. That result is an automation-tool limitation or unresolved integration behavior, not evidence that human pointer input succeeds or fails; manual pointer-driven acceptance remains open. Separate 0.25.0 and 0.24.0 inspections cover the field menu, enlarged objective panel, and deliberately labeled editor-only result preview; authoritative result/restart behavior remains automation-covered. Terrain edges, fog boundary, entities, selection/platform presentation, HUD, feedback surface, and the fog-respecting tactical overview remained visible. Actual patrol/routing/combat semantics remain automation-covered rather than manually accepted. Selected-unit patrol/hold, pointer-targeted guard, construction, context movement, gathering, production selection, attack-move delivery, control-group assignment with selected units, and combat remain unaccepted as manual interactions. These are bounded presentation observations, not evidence of a complete rendered playable loop or final terrain/fog/UI art. Version 0.26.1 has not yet passed package, packaged-performance, or exact-package visual gates.
