...
The observed current results are: workspace generation succeeded; `EchoesOfTheBrokenSunEditor Mac Development` compiled and linked for arm64; eight Unreal automation tests passed 8/8 with no test warning or error; and the null-RHI game smoke selected `EchoesGameMode`, created a 25-entity scenario with 10 initially visible entity views, the 165-tile Glass Scar layout, three crossings, and a 4,096-tile fog/shroud surface at 20 Hz, emitted every required readiness marker, and exited cleanly. `Echoes.Runtime.Bootstrap.ClassesAndCore` covers class registration, required input mappings—including `[F]` attack-move, `[H]` hold, `[X]` stop, `[K]` checkpoint, `[L]` load, `[G]` assignment mode, and `[1-0]` recall—and a one-tick portable-core call. `Echoes.Runtime.Controls.ControlGroups` proves group index validation, local-entity filtering, duplicate removal, hostile rejection, and explicit clearing. `Echoes.Runtime.Gameplay.HoldPosition` proves Unreal validation, order application, anchor preservation, and Stop interruption for a real scenario defender. `Echoes.Runtime.Gameplay.ProductionPauseRestart` uses the real world subsystem to prove pause, worker and soldier production, logistics accounting, and deterministic restart. `Echoes.Runtime.Gameplay.CompleteSkirmish` proves one composed automated match across the Glass Scar: drop-off construction, legitimate Future Well scouting and capture, reinforcement production, safe-side group rally, one-order attack-move engagement, opposing Command-Core destruction, post-victory simulation freeze, and restart. `Echoes.Runtime.Map.GlassScar` proves the exact blocked count, all three authored crossings, closed-span samples, presentation parity, central Well location, and deterministic routing into the northern basin. `Echoes.Runtime.Persistence.QuickSaveLoad` proves exact tick/checksum restoration, prior-generation backup recovery after intentional primary corruption, view reconstruction, and valid post-load command sequencing while preserving any pre-existing test-host checkpoint. `Echoes.Runtime.Visibility.ActorLifecycle` proves fog tile accounting, explored-area growth, visible-to-shrouded transition, actor creation on reveal, destruction when hidden, single-actor recreation on reentry, and health-bar visibility/fraction behavior for full-health, selected, damaged, and restored entities. The null-RHI smoke covers bootstrap only. These results do not validate a pointer-driven rendered match, sustained simulation, production-scale navigation/fog performance, broader gameplay completeness, or clean-machine installation.

## Rendered prototype boundary

Local Metal SM5 Editor runs and the current 0.8.0 packaged application were visually inspected at the runtime-generated arena. The 0.9.0 source HUD displays resources, logistics, match state, construction/production hotkeys, `[F]` attack-move, `[H]` hold, `[X]` stop, `[G]` then `[1-0]` assignment, bare `[1-0]` recall, `[K]` checkpoint, `[L]` load, `[Z/C/V]` Well choices, and the `PLAYABLE SYSTEMS BUILD — ACTIVE DEVELOPMENT` boundary without observed clipping. Pressing `[H]` with no selection reached the correct `[NO_SELECTION]` path; actual hold semantics are automation-covered, not manually accepted. Both a rendered 0.8.0 Editor run and the archived 0.8.0 app showed `G` then `1` reaching the assignment path and clearing an empty group, followed by bare `1` reaching recall and reporting `[GROUP_EMPTY]`. An earlier rendered Editor run visibly committed a checkpoint at tick 830, advanced beyond it, then restored tick 830 with `[L]`; the packaged 0.7.0 application separately committed tick 338 and restored tick 338. Terrain, fog, entities, HUD, and normal lit rendering remained intact. The schema-5 command expansion intentionally makes schema-4 development checkpoints unsupported; they fail closed rather than being reinterpreted. The desktop automation bridge still cannot reliably deliver pointer clicks or drag gestures into the Metal window, so exact-build selection, construction placement, context movement, gathering, production selection, attack-move pointer delivery, selected-unit hold, full control-group assignment with selected units, and combat remain unaccepted as manual interactions. These are rendered startup and bounded visual/keyboard observations, not evidence of a complete rendered playable loop, final terrain or fog art, or performance.
