...
The project pins Unreal Engine 5.8. Epic's version-specific macOS table lists Xcode 26.0 as the minimum and 26.1.1 as recommended, and explicitly marks 26.4 incompatible. The locally installed Xcode 26.6 has passed project generation, arm64 Development Editor compilation, ten Unreal automation tests, null-RHI and rendered startup, and a local Development cook/package. That is positive evidence on this host, not a clean-machine, Developer ID signing, notarization, performance, or full compatibility result. Xcode 26.1.1 remains Epic's recommended support baseline.
...
The script compiles with Apple Clang, writes to a temporary directory beneath `${TMPDIR:-/tmp}`, removes that directory on exit, and runs optimized, debug, and AddressSanitizer/UndefinedBehaviorSanitizer configurations. All three current configurations pass 16/16 suites. Coverage includes production, logistics, persistence, victory, attack-move, hold, guard, patrol, deterministic obstacle routing, path-cache invalidation after terrain mutation, visibility, Future Wells, replay, and adversarial input bounds. Those results validate only the named engine-independent paths; they do not validate Unreal rendering, multiplayer transport, packaging, or every hostile input. LeakSanitizer is disabled because it is unavailable in Apple's macOS AddressSanitizer runtime.
...
The observed current results are: workspace generation succeeded; `EchoesOfTheBrokenSunEditor Mac Development` compiled and linked for arm64; ten Unreal automation tests passed 10/10 with no test warning or error; and the null-RHI game smoke selected `EchoesGameMode`, created a 25-entity scenario with 10 initially visible entity views, the 165-tile Glass Scar layout, three crossings, and a 4,096-tile fog/shroud surface at 20 Hz, emitted every required readiness marker, and exited cleanly. The complete-skirmish test remains green after schema-8 route-field optimization and verifies that the seven-unit strike force rallies before crossing hostile territory. The remaining tests cover required input mappings, patrol/hold/guard boundaries, control groups, production/pause/restart, exact map topology/routing, checkpoint/backup recovery, view reconstruction, visibility lifecycle, and health-bar state. The null-RHI smoke covers bootstrap only. These results do not validate pointer-delivered orders in a rendered match, sustained Unreal performance, broader gameplay completeness, or clean-machine installation.

## Rendered prototype boundary

Local Metal SM5 Editor runs and the 0.13.0 packaged application were visually inspected at the runtime-generated arena. After TAA became the Mac default, terrain edges, fog boundary, entities, selection presentation, tactical HUD, and feedback surface remained intact without observed clipping or obvious anti-aliasing regression. The HUD displays `[F]` attack-move, `[T]` patrol, `[H]` hold, `[J]` guard, `[X]` stop, construction/production, control groups, checkpoints, and Well choices. Earlier rendered checks reached the reason-coded patrol empty-selection path and cover hold, guard, control-group assignment/recall, and checkpoint rollback. Actual patrol/routing/combat semantics remain automation-covered rather than manually accepted. The desktop automation bridge still cannot reliably deliver pointer clicks or drag gestures into the Metal window, so selected-unit patrol/hold, pointer-targeted guard, construction, context movement, gathering, production selection, attack-move delivery, control-group assignment with selected units, and combat remain unaccepted as manual interactions. These are bounded presentation observations, not evidence of a complete rendered playable loop or final terrain/fog/UI art.
...
./Scripts/run_packaged_smoke.sh \
  BuildArtifacts/Packages/Mac-Development-v0.13.0-mac-performance/EchoesOfTheBrokenSun.app
```

The current observed local artifact is a 748 MB self-contained arm64 Development application with bundle identifier `com.angelispseftis.echoesofthebrokensun`, short version `0.13.0`, and five PAK/IoStore container files. Unreal's Xcode packaging phase seals the app before the final PAK update, so the wrapper reseals the completed archive with the checked-in development entitlements and verifies it afterward. The exact archive passed strict deep-signature verification and emitted the environment, Glass Scar, 4,096-tile fog, simulation, boot-ready, and first-tick markers when its packaged executable was launched. Its adjacent manifest records clean source commit `c37c9b675c22b6c5fe253447af92e3114194b804`, UE 5.8.2, Xcode 26.6, configuration and platform identity, and the SHA-256 of every regular file and symlink in the app. The manifest digest is `91d98d774a763b44003561f35c8fd261ca2b4f36a831d541b9f10e754dc04559`. Rendered packaged inspection also confirmed the TAA scene, fog, entities, feedback surface, and unclipped tactical HUD. The exact 1.1 GB staging directory was moved to Trash after qualification because it is rebuildable; the accepted archive was preserved, and the staging tree remains recoverable until Trash is emptied.
