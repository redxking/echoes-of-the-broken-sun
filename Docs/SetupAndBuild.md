...
The packaged Metal profile is a separate gate and defaults to the accepted 0.14.0 package:
...
./Scripts/run_packaged_smoke.sh \
  BuildArtifacts/Packages/Mac-Development-v0.14.0-four-player/EchoesOfTheBrokenSun.app
```

The current observed local artifact is a 748 MB self-contained arm64 Development application with bundle identifier `com.angelispseftis.echoesofthebrokensun`, short version `0.14.0`, and five PAK/IoStore container files. Unreal's Xcode packaging phase seals the app before the final PAK update, so the wrapper reseals the completed archive with the checked-in development entitlements and verifies it afterward. The exact archive passed strict deep-signature verification and emitted the environment, Glass Scar, 4,096-tile fog, simulation, boot-ready, and first-tick markers when its packaged executable was launched. Its adjacent manifest records clean source commit `de673591443c0d361a59cfb95cad845d9e586972`, UE 5.8.2, Xcode 26.6, configuration and platform identity, and the SHA-256 of every regular file and symlink in the app. The manifest digest is `346701dc16315f233d0f45e33b7ee122f80c110041689f7d7d99652b10527bd4`. The exact archive's packaged Metal profile passed all placeholder-scene thresholds at 10.469 ms frame, 10.457 ms render-thread, 9.929 ms GPU, 0.899 ms game-thread p95, and 612.312 MiB peak sampled RSS. Rendered packaged inspection confirmed the TAA scene, fog, entities, feedback surface, and unclipped tactical HUD. The exact 1.1 GB staging directory was moved to Trash after qualification because it is rebuildable; the accepted archive was preserved, and the staging tree remains recoverable until Trash is emptied.
