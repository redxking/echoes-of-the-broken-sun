...
The observed current 0.30.0 source results are: `EchoesOfTheBrokenSunEditor Mac Development` compiled and linked for arm64; eight adversarial content-compiler tests passed; and thirteen Unreal automation tests passed 13/13 with no test warning or error. The compiler now requires each playable faction to define a worker, three distinct combat/support roles, a headquarters, logistics, production, and a fourth structure role. The generated catalog contains all eight named slice units and eight named structures with display names and bounded statistics. Reviewed pack SHA-256 is `edfd061c610e096f8df16a4fd8e4dfa2ca67af8d4d77aee414e792003c683d05`. The Unreal content subsystem independently hashes and parses that pack, verifies role coverage, and rejects mismatch before match creation. Both null-RHI runtime fixtures loaded that exact digest and retained the normal redacted-AI/visible-expansion and 400-unit/four-team active-combat gates. Clean evidence at source commit `f13433ca798e1c431b47e33ca1673223900de661` hashes to `cb0fb3a458c89790046bcbaf5f078fc761ac256b268a88918642d080300cb5e5` for the automation index, `906aa7f21b25376fad773da5ab14df45ac7c363cf6fe3894ac08f5ae8b6c7a3c` for normal runtime, and `661017144df88d49775992a5dcb6df3aead6fb43c3675ca11dcf81c8405e0895` for stress runtime. The 17/17 native regression remains established at 0.29.0 because this milestone does not change the native core. The simulation still uses technical-fixture archetypes, and none of the newly defined faction-specific abilities is claimed as implemented. These checks do not validate complete playable faction mechanics, pointer-delivered orders in a rendered match, final effects/formations, network transport, broader gameplay completeness, or clean-machine installation.

## Rendered prototype boundary

The exact 0.23.1 packaged process was identified by its full executable path and visually inspected through Metal at native 2560×1440. Its responsive Glass Scar operations brief was legible without observed clipping, paused deployment, and transitioned to the battlefield and tactical HUD on Enter. The 0.23.0 package had exposed a real defect: its briefing retained a 920-pixel cap and rendered too small on the Retina display. That package is superseded; the proportional 0.23.1 layout is the accepted packaged result. Version 0.27.0 editor-source inspection at 1600×900 showed the revised title without observed overlap or clipping in normal and high-contrast modes. It exposed only Glass Scar single-player and implemented settings, disclosed the standard Adaptive Kharuun opposition and fair vision/economy boundary, and kept campaign, multiplayer, and replay absent. Enter advanced through the operations brief into deployment, and the live log recorded the first Adaptive expansion on visible valid terrain. The visual log SHA-256 is `a93ec23694ad6a86f7b89758bf2d72b6058f7456f4ef4188106744a927a323d5`. Versions 0.28.0 through 0.30.0 change simulation/AI and authored-content boundaries without changing presentation, so no new visual result is claimed. The current Metal input retry successfully delivered both Enter transitions and then reproduced the desktop computer-control failure: the service closed its native pipe before returning from a coordinate click, Unreal emitted no selection/order marker, the game remained live, and a keyboard quit succeeded after reacquiring the service. The log SHA-256 is `555b74083e186bfc1ae0e056c8ed16e2528c9e658003c77527285d9eed7c9475`. This is a tool-integration result, not evidence that human pointer input succeeds or fails; manual pointer-driven acceptance remains open. Version 0.30.0 has not passed package, packaged-performance, or exact-package visual gates.
