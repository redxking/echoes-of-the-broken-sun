...
The observed current results are: workspace generation succeeded; `EchoesOfTheBrokenSunEditor Mac Development` compiled and linked for arm64; nine Unreal automation tests passed 9/9 with no test warning or error; and the null-RHI game smoke selected `EchoesGameMode`, created a 25-entity scenario with 10 initially visible entity views, the 165-tile Glass Scar layout, three crossings, and a 4,096-tile fog/shroud surface at 20 Hz, emitted every required readiness marker, and exited cleanly. `Echoes.Runtime.Bootstrap.ClassesAndCore` covers class registration, required input mappings—including `[F]` attack-move, `[H]` hold, `[J]` guard, `[X]` stop, checkpoints, control groups, and Well choices—and a one-tick portable-core call. `Echoes.Runtime.Gameplay.HoldPosition` proves order application, anchor preservation, and Stop interruption. `Echoes.Runtime.Gameplay.Guard` proves hostile-target rejection, owned-target assignment, exact target retention, and Stop interruption. The remaining tests cover control groups, production/pause/restart, the complete Glass Scar skirmish, exact map topology/routing, checkpoint/backup recovery, view reconstruction, visibility lifecycle, and health-bar state. The null-RHI smoke covers bootstrap only. These results do not validate pointer-targeted guard in a rendered match, sustained simulation, production-scale navigation/fog performance, broader gameplay completeness, or clean-machine installation.

## Rendered prototype boundary

Local Metal SM5 Editor runs and the current 0.8.0 packaged application were visually inspected at the runtime-generated arena. The 0.10.0 source HUD displays `[F]` attack-move, `[H]` hold, `[J]` guard, `[X]` stop, construction/production, control groups, checkpoints, and Well choices without observed clipping. Pressing `[H]` and `[J]` with no selection each reached the correct `[NO_SELECTION]` path; actual stance semantics are automation-covered, not manually accepted. Earlier rendered checks cover control-group assignment/recall and checkpoint rollback. Terrain, fog, entities, HUD, and normal lit rendering remained intact. The schema-6 command expansion intentionally makes earlier development checkpoints unsupported; they fail closed rather than being reinterpreted. The desktop automation bridge still cannot reliably deliver pointer clicks or drag gestures into the Metal window, so selected-unit hold, pointer-targeted guard, construction, context movement, gathering, production selection, attack-move delivery, control-group assignment with selected units, and combat remain unaccepted as manual interactions. These are bounded visual/keyboard observations, not evidence of a complete rendered playable loop, final terrain/fog art, or performance.
