...
Local Metal SM5 Editor runs and the 0.12.0 packaged application were visually inspected at the runtime-generated arena. The tactical HUD displays `[F]` attack-move, `[T]` patrol, `[H]` hold, `[J]` guard, `[X]` stop, construction/production, control groups, checkpoints, and Well choices without observed clipping. Pressing `[T]` with no selection reached the correct `[NO_SELECTION]` path; actual patrol routing and combat semantics are automation-covered, not manually accepted. Earlier rendered checks cover hold, guard, control-group assignment/recall, and checkpoint rollback. Terrain, fog, entities, HUD, feedback bar, and normal lit rendering remained intact. The current schema-8 navigation/checksum change intentionally makes earlier development checkpoints unsupported; they fail closed rather than being reinterpreted. The desktop automation bridge still cannot reliably deliver pointer clicks or drag gestures into the Metal window, so selected-unit patrol/hold, pointer-targeted guard, construction, context movement, gathering, production selection, attack-move delivery, control-group assignment with selected units, and combat remain unaccepted as manual interactions. These are bounded visual/keyboard observations, not evidence of a complete rendered playable loop, final terrain/fog art, or performance.
...
./Scripts/run_packaged_smoke.sh \
  BuildArtifacts/Packages/Mac-Development-v0.12.0-native-performance/EchoesOfTheBrokenSun.app
```

The current observed local artifact is a 748 MB self-contained arm64 Development application with bundle identifier `com.angelispseftis.echoesofthebrokensun`, short version `0.12.0`, and five PAK/IoStore container files. Unreal's Xcode packaging phase seals the app before the final PAK update, so the wrapper reseals the completed archive with the checked-in development entitlements and verifies it afterward. The exact archive passed strict deep-signature verification and emitted the environment, Glass Scar, 4,096-tile fog, simulation, boot-ready, and first-tick markers when its packaged executable was launched. Its adjacent manifest records clean source commit `9e04a02616ccb8f6ce2a115866a9c8bd5262c54b`, UE 5.8.2, Xcode 26.6, configuration and platform identity, and the SHA-256 of every regular file and symlink in the app. The manifest digest is `62a18ad2e0b1be20e02ddaa401a2fdf849de76358901cf4f7476d471ba506b84`. Rendered packaged inspection also confirmed the unclipped tactical HUD and `[T]` reason-coded empty-selection feedback; selected-unit endpoint delivery remains automation-covered rather than manually accepted. The exact 1.1 GB staging directory was deleted after qualification because it is rebuildable; the accepted archive was preserved.
