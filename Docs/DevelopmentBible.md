...
The current 0.27.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. It gathers and scouts, chooses a Future Well protocol by personality, produces workers and soldiers, adds a Barracks when missing, adds logistics capacity under population pressure, attacks currently visible enemies, and withdraws low-health soldiers toward its own Command Core. Expansion placement is restricted to currently visible valid terrain, and no income, construction-speed, combat, cooldown, or vision bonus is applied. Broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored faction technology paths, and the complete personality set remain open.
