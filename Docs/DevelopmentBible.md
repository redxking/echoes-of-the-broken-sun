...
The current 0.31.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. It gathers and scouts, chooses a Future Well protocol by personality, produces workers and soldiers, adds a production structure when missing, adds logistics capacity under population pressure, attacks currently visible enemies, and withdraws low-health soldiers toward its own headquarters. AI decisions consume a separately materialized player-view snapshot containing owned state, currently visible observations, known terrain, and the same versioned authored rules used by authority. Hidden entities are omitted; visible-opponent internal orders, cooldowns, and exact statistics are redacted; unexplored terrain and hidden temporary passability changes are not exposed; and the pure command producer receives no authoritative simulation handle. Expansion placement is restricted to currently visible valid terrain, and no income, construction-speed, combat, cooldown, or vision bonus is applied. Authored Surveyor/Lancer/Anchor/Power Link/Array Foundry and Tender/Riftstalker/Memory Hearth/Waystone/Growth Basin values now drive foundational gameplay, as do authored Future Well economy and timing. Bulwark Team, Relay Skiff, Cairnback, Resonant, Aegis Post, and Listening Spine still require distinct simulation types and mechanics. Bulwark deployment, Relay supply extension, Waystone migration, warform adaptation, temporary cover, vibration detection, broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored technology paths, and balance qualification remain open.
