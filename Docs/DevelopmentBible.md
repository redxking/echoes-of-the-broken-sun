...
The current 0.37.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. It gathers and scouts, chooses a Future Well protocol by personality, produces workers and line units, adds production and logistics capacity from its redacted player view, attacks currently visible enemies with all combat categories, withdraws low-health combatants toward its own headquarters, selects a bounded warform adaptation from visible enemy composition when a valid Growth Basin is in range, and can place Cairnback mineral cover one tile toward a visible nearby threat. All eight named units and eight named structures have distinct authoritative categories and baseline authored values. Meridian Bulwark deployment and Relay supply extension retain their public counterplay. Kharuun Waystones expose rooted, transitioning, and mobile state. Kharuun combat warforms can enter an 80-tick stationary molt within 600 cm of a completed Growth Basin for 25 Dawn. The public molt/form geometry is non-color-distinct, exact enemy completion time and health remain redacted, molting takes 150 percent damage, and destroying the Basin or leaving its field cancels the molt without refund. Carapace trades speed for health; Striker improves damage and attack cadence. A Cairnback can spend 15 Dawn to raise a 180-health mineral barrier within 450 cm for 300 ticks, after which its reserve remains on cooldown until tick 600 from activation. The 150-by-150-cm object blocks routing and intercepts fire that crosses its footprint while protecting an ally; direct attacks, destruction, and expiry provide counterplay. It restores the prior open or scarred tile, does not depend on creator survival, and reveals its current presence without disclosing exact enemy expiry, health, creator identity, or hidden underlying terrain. Direct Kharuun player-side faction selection remains open, so Hyphen, Shift–bracket, and Shift–semicolon controls are mapped and validated but are not reachable in the current Meridian-only player scenario. The deterministic AI adaptation and cover-placement paths are tested, but the short normal runtime gate observed neither a live AI molt nor live cover placement. Vibration detection, powered Aegis behavior, broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored technology paths, and balance qualification remain open.
...
The current 0.27.0 implementation exposes a deliberately bounded subset: a visible, uncaptured and unlocked RTS cursor; a keyboard-operable feature-gated title screen; predeployment operations brief; visibility-scoped live objective tracker; explicit victory/draw/defeat presentation with Enter or R redeployment; a keyboard-operable field menu; persistent HUD scale; high-contrast HUD; reduced motion; reduced flashing; edge-pan enablement; adjustable pan speed; adjustable zoom step; offline pause; transactional quick-save recovery; four distinct ownership-marker shapes; and a fog-respecting tactical minimap. The title presents only the implemented Glass Scar single-player operation, its Meridian Compact/Future Well contest context, and the standard Adaptive Kharuun opponent's fair-vision/fair-economy boundary; campaign, multiplayer, and replay are hidden rather than implied. Implemented accessibility settings are available before deployment. Enter advances from title to the responsive operations brief and then deploys, while the deterministic simulation remains held at tick zero and battlefield/camera input remains blocked. Unattended automation and profiling may explicitly auto-start. Escape or P opens the field menu during play, which pauses the deterministic match, locks battlefield/camera input, exposes only implemented options, permits those settings to change live, and resumes with Enter, Escape, or P. The end-state modal likewise locks battlefield and camera input. The objective tracker exposes owned Command Core integrity, currently visible Future Well protocol state, currently visible hostile-core contact, and the public match result without reading hidden hostile health. The minimap presents only legitimately explored blocked terrain and currently visible entities; squares, crosses, diamonds, and plus signs supplement team color. Skirmish setup, remapping, subtitles, audio-category controls, reduced dynamic range, a complete accessibility menu, tutorial replay, searchable technology information, autosave, and campaign-choice confirmation remain design requirements rather than implemented features.
