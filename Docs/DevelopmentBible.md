...
The current 0.29.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. It gathers and scouts, chooses a Future Well protocol by personality, produces workers and soldiers, adds a Barracks when missing, adds logistics capacity under population pressure, attacks currently visible enemies, and withdraws low-health soldiers toward its own Command Core. AI decisions consume a separately materialized player-view snapshot containing owned state, currently visible observations, and known terrain. Hidden entities are omitted; visible-opponent internal orders, cooldowns, and exact statistics are redacted; unexplored terrain and hidden temporary passability changes are not exposed; and the pure command producer receives no authoritative simulation handle. Expansion placement is restricted to currently visible valid terrain, and no income, construction-speed, combat, cooldown, or vision bonus is applied. Version 0.29.0 also validates and loads the current faction, unit, structure, and Future Well source records into an immutable runtime catalog before match creation. The catalog is not yet wired into simulation statistics, and its four units and four structures are only a starter roster, not the complete Meridian Compact or Kharuun Assemblies slice. Broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored faction technology paths, the complete personality set, and balance qualification remain open.
