...
This is the single authoritative creative, game-design, universe, narrative, campaign, art, audio, interface, accessibility, and player-experience document. It is edited in place. Unless an item is linked to acceptance evidence in the project ledger, it is a design requirement rather than an implemented capability.
...
The map is a narrow impact basin split by a fractured transit span. Each base has a safe but inefficient Matter route. A central Future Well controls the short route and observation ridge. Two side caverns give the Kharuun mobility but readable emergence points; the Compact can establish a powerful relay line along the ridge but must defend exposed links.
...
7. **A Memory Deliberately Lost:** Oruun confronts the Assembly's curated omissions.
8. **The Shape Beside Us:** Neme guides units through overlapping states; the Choir becomes understandable.
9. **Reserve Authority:** Mara chooses which failing districts receive power.
10. **The Choir at Lume Reach:** Three factions contest a Well whose harvest could halt a larger cascade.

### Act III — Crownfall

11. **No Neutral Ledger:** Alliance composition depends on accumulated decisions.
12. **The Future That Won:** Rhyse demonstrates a stable restoration prototype and the lives it can save.
13. **Assembly of the Missing:** Oruun exposes responsibility shared across factions.
14. **Several Voices, One Command:** The player controls the Choir and resolves incompatible capabilities.
15. **The Broken Sun:** A final operation supports restoration, controlled stabilization, extinguishment, or open evolution.

Endings report concrete consequences and unresolved costs rather than a hidden good/evil score.

## Skirmish and AI

Skirmish exposes map, faction, teams, AI personality/difficulty, starting resources, victory conditions, and supported speed. Standard difficulty uses only player-visible information and no hidden income. Any assisted level labels the exact modifier before the match and in replay metadata.
...
The current 0.48.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. The title and operations brief allow the player to cycle between Meridian Compact and Kharuun Assemblies before deployment. Changing faction rebuilds the deterministic scenario from its baseline, places the selected force under player 0, assigns the other force to the Adaptive opponent, and keeps the operation paused. Objectives, control guidance, deployment feedback, and result language follow that authoritative selection. Restart retains the chosen force, and quick-load compatibility includes the local faction so an opposite-faction checkpoint cannot silently replace it. Each playable faction has a two-step authored research path produced at its Barracks-role structure. Meridian progresses through Prismatic Targeting and Horizon Lattice; Kharuun progresses through Echo Cartography and Ancestral Edge. The upgrades deliberately alter only combat damage and combat vision at this stage. Research consumes Matter and Dawn, occupies the producer, requires its prerequisite, updates surviving and future combat units, persists through saves and replays, and is interrupted without refund if the structure is destroyed. The owning player retains that interruption state until another valid project starts; the HUD and archive explain the producer loss and no-refund result. Kharuun damage research composes deterministically with Striker adaptation. The Adaptive opponent may queue the next affordable faction technology using only its scoped player view and an idle producer; the short null-RHI gate does not establish that this occurs in live play or that the policy is tactically strong.

F2 opens the faction technology archive and pauses the offline operation. It presents exact authored names, costs, durations, effects, prerequisite state, resource state, completion/progress state, and interruption consequences. Up/Down moves an explicit visible tier focus, Enter requests that exact tier, Shift+R requests the next incomplete tier, and opening focuses the first incomplete tier. A row target synchronizes focus before requesting its exact tier; F2, Escape, or P closes the archive and restores the prior pause state. Rendering and hit-testing share one layout calculation. During live play, Tab selects the next owned live visible-backed entity and Backspace selects the previous one, with deterministic wraparound, a single selection ring, and exact entity feedback. Home enables a cyan tactical reticle, arrows move it in bounded steps while the archive is closed, and Space issues an ordinary context order at its rendered screen location. Target-dependent command keys use that same fair trace while the reticle is active. Kharuun systems expose visible F3 Waystone, F4/F5 Warform, and F6 Cairnback-cover aliases while retaining the older punctuation bindings. Direct application delivery accepts ordinary Waystone migration through F3, a full Meridian Bulwark deploy/pack/redeploy cycle, an Array Foundry placement order, deterministic restart delivery, and a Lancer production order with visible progress. F4 and F6 have reached their ordinary validation paths, but successful adaptation still requires the selected Warform to be inside a Growth Basin field and cover still requires a valid world target. Exact coordinate-pointer activation, new-site construction completion, produced-unit emergence, successful ordinary adaptation/cover, ordinary player-driven research completion/interruption, a full victory/result/restart operation, and balance remain open. Broader composition response, coordinated tactical groups, repair/recovery, multiple bases, richer searchable technology trees, and balance qualification remain open.

## Interface and accessibility

The battlefield holds visual priority. The command band shows selections, subgroups, orders, abilities, production, and stable failure reasons. The minimap distinguishes known terrain, current vision, last-known contacts, alerts, and Well states without color-only information. Compact ownership uses bracketed rectangular marks; Kharuun uses paired faceted marks; Choir uses offset concentric marks.
...
The current implementation exposes a deliberately bounded subset: a visible, uncaptured and unlocked RTS cursor; deterministic Tab/Backspace cycling through owned live visible-backed entities with wraparound and exact feedback; a keyboard-operable feature-gated title screen; predeployment operations brief; visibility-scoped live objective tracker; explicit victory/draw/defeat presentation with Enter or R redeployment; keyboard-operable field and technology modals; persistent HUD scale; high-contrast HUD; reduced motion; reduced flashing; edge-pan enablement; adjustable pan speed; adjustable zoom step; offline pause; transactional quick-save recovery; four distinct ownership-marker shapes; and a fog-respecting tactical minimap. The title presents only the implemented Glass Scar single-player operation and fair-view/fair-economy opponent boundary; campaign, multiplayer, and replay are hidden rather than implied. Implemented accessibility settings are available before deployment. Enter advances from title to the responsive operations brief and then deploys, while the deterministic simulation remains held at tick zero and battlefield/camera input remains blocked. Unattended automation and profiling may explicitly auto-start. Escape or P opens the field menu during play. Tab selects the next owned live entity and Backspace selects the previous one. F2 opens the two-tier technology archive; Up/Down focuses a tier, Enter activates the focused tier, Shift+R chooses the next incomplete tier, and F2/Escape/P closes it. Both modals pause the deterministic match and lock battlefield/camera input. The end-state modal likewise locks battlefield and camera input. The objective tracker exposes owned Command Core integrity, currently visible Future Well protocol state, currently visible hostile-core contact, and the public match result without reading hidden hostile health. The minimap presents only legitimately explored blocked terrain and currently visible entities; squares, crosses, diamonds, and plus signs supplement team color. Anonymous vibration contacts use an amber approximate-contact alert and a non-color diamond with no entity identity or direct-target implication. Broader skirmish setup, remapping, subtitles, audio-category controls, reduced dynamic range, a complete accessibility menu, tutorial replay, search across a larger technology tree, autosave, and campaign-choice confirmation remain design requirements rather than implemented features.
