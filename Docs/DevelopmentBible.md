...
The current 0.38.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. It gathers and scouts, chooses a Future Well protocol by personality, produces workers and line units, adds production and logistics capacity from its redacted player view, attacks currently visible enemies, withdraws low-health combatants, selects a bounded warform adaptation, places Cairnback mineral cover, and can investigate anonymous movement signatures. Resonants detect moving hostile mobile units within 2,200 cm; completed Listening Spines extend that radius to 2,600 cm. Actual movement refreshes a 40-tick signature. Each contact is quantized to a 200-cm cell, collapses duplicate sources in the same cell, disappears when ordinary vision exposes the source, and carries no targetable entity ID, faction, type, health, order, or exact position. AI response is an attack-move to the approximate cell rather than a hidden direct target. Destroying or leaving detector coverage and stopping long enough for the signature to expire provide counterplay. Labeled diamond-wave markers supply non-color world and minimap presentation when a local Kharuun detector is available. All eight named units and eight named structures retain distinct authoritative categories and baseline values, and the prior five faction mechanics retain their recorded counterplay. Direct Kharuun player-side faction selection remains open, so valid player-side Waystone, warform, Cairnback, and vibration-presentation use is not reachable in the current Meridian-only scenario. Deterministic and Unreal integration tests exercise the detection path, but the short normal runtime observed no live AI molt, cover placement, or vibration response. Powered Aegis behavior, broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored technology paths, and balance qualification remain open.
