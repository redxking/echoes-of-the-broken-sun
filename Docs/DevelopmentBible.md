...
The current 0.41.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. The title and operations brief allow the player to cycle between Meridian Compact and Kharuun Assemblies before deployment. Changing faction rebuilds the deterministic scenario from its baseline, places the selected force under player 0, assigns the other force to the Adaptive opponent, and keeps the operation paused. Objectives, control guidance, deployment feedback, and result language follow that authoritative selection. Restart retains the chosen force, and quick-load compatibility includes the local faction so an opposite-faction checkpoint cannot silently replace it. Each playable faction now has a two-step authored research path produced at its Barracks-role structure. Meridian progresses through Prismatic Targeting and Horizon Lattice; Kharuun progresses through Echo Cartography and Ancestral Edge. The upgrades deliberately alter only combat damage and combat vision at this stage. Research consumes Matter and Dawn, occupies the producer, requires its prerequisite, updates surviving and future combat units, persists through saves and replays, and is interrupted without refund if the structure is destroyed. Kharuun damage research composes deterministically with Striker adaptation. The Adaptive opponent may queue the next affordable faction technology using only its scoped player view and an idle producer; the short null-RHI gate does not establish that this occurs in live play or that the policy is tactically strong. The prior Metal session at 1600×900 visibly accepted the Tab transition, Kharuun brief, and Kharuun deployment without observed clipping or overlap, but predates the research HUD. The desktop-control service failed before delivering the first coordinate click and the HUD stayed at zero selected, so manual pointer control, rendered research use, rendered contact legibility, and balance remain open. Broader composition response, coordinated tactical groups, repair/recovery, multiple bases, a searchable technology panel, and balance qualification remain open.
