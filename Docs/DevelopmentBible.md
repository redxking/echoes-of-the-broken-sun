...
The current 0.22.1 implementation exposes a deliberately bounded subset: persistent HUD scale, high-contrast HUD, reduced motion, reduced flashing, edge-pan enablement, adjustable pan speed, adjustable zoom step, offline pause, transactional quick-save recovery, four distinct ownership-marker shapes, and a fog-respecting tactical minimap. The minimap presents only legitimately explored blocked terrain and currently visible entities; squares, crosses, diamonds, and plus signs supplement team color. Remapping, subtitles, audio-category controls, reduced dynamic range, an accessibility menu, tutorial replay, searchable technology information, autosave, and campaign-choice confirmation remain design requirements rather than implemented features.
