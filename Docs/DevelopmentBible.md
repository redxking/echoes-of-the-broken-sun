...
The current 0.23.1 implementation exposes a deliberately bounded subset: a keyboard-operable predeployment operations brief, persistent HUD scale, high-contrast HUD, reduced motion, reduced flashing, edge-pan enablement, adjustable pan speed, adjustable zoom step, offline pause, transactional quick-save recovery, four distinct ownership-marker shapes, and a fog-respecting tactical minimap. The briefing pauses the interactive match, blocks battlefield commands, presents the situation, objectives, Future Well doctrine, controls, and accessibility settings, and deploys on Enter; unattended automation and profiling may explicitly auto-start. The minimap presents only legitimately explored blocked terrain and currently visible entities; squares, crosses, diamonds, and plus signs supplement team color. Remapping, subtitles, audio-category controls, reduced dynamic range, a complete accessibility menu, tutorial replay, searchable technology information, autosave, and campaign-choice confirmation remain design requirements rather than implemented features.
