...
The current 0.25.0 implementation exposes a deliberately bounded subset: a keyboard-operable predeployment operations brief, a visibility-scoped live objective tracker, explicit victory/draw/defeat presentation with Enter or R redeployment, a keyboard-operable field menu, persistent HUD scale, high-contrast HUD, reduced motion, reduced flashing, edge-pan enablement, adjustable pan speed, adjustable zoom step, offline pause, transactional quick-save recovery, four distinct ownership-marker shapes, and a fog-respecting tactical minimap. The briefing pauses the interactive match, blocks battlefield and camera commands, presents the situation, objectives, Future Well doctrine, controls, and accessibility settings, and deploys on Enter; unattended automation and profiling may explicitly auto-start. Escape or P opens the field menu, which pauses the deterministic match, locks battlefield/camera input, exposes only implemented options, permits those settings to change live, and resumes with Enter, Escape, or P. The end-state modal likewise locks battlefield and camera input. The objective tracker exposes owned Command Core integrity, currently visible Future Well protocol state, currently visible hostile-core contact, and the public match result without reading hidden hostile health. The minimap presents only legitimately explored blocked terrain and currently visible entities; squares, crosses, diamonds, and plus signs supplement team color. A front-end/title shell, remapping, subtitles, audio-category controls, reduced dynamic range, a complete accessibility menu, tutorial replay, searchable technology information, autosave, and campaign-choice confirmation remain design requirements rather than implemented features.
