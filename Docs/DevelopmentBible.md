...
The current 0.40.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. The title and operations brief allow the player to cycle between Meridian Compact and Kharuun Assemblies before deployment. Changing faction rebuilds the deterministic scenario from its baseline, places the selected force under player 0, assigns the other force to the Adaptive opponent, and keeps the operation paused. Objectives, control guidance, deployment feedback, and result language follow that authoritative selection. Restart retains the chosen force, and quick-load compatibility includes the local faction so an opposite-faction checkpoint cannot silently replace it. Automation exercises the actual selected Kharuun roster, a locally owned Waystone uproot, Kharuun production, and an injected moving Meridian source detected as an anonymous local vibration contact. Separate null-RHI boots exercise both faction directions and visible-terrain opponent expansion. A Metal session at 1600×900 visibly accepted the Tab transition, Kharuun brief, and Kharuun deployment without observed clipping or overlap. The desktop-control service failed before delivering the first coordinate click and the HUD stayed at zero selected, so manual pointer control, rendered contact legibility, and balance remain open. The prior seven bounded mechanics, schema-18 state, authored roster values, fair-view AI boundary, and powered-Aegis counterplay remain unchanged. Broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored technology paths, and balance qualification remain open.
