...
The current 0.39.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. It gathers and scouts, chooses a Future Well protocol by personality, produces workers and line units, adds production and logistics capacity from its redacted player view, attacks currently visible enemies, withdraws low-health combatants, selects a bounded warform adaptation, places Cairnback mineral cover, and can investigate anonymous movement signatures. Resonant and Listening Spine detection retains its anonymous, quantized, time-bounded counterplay. Meridian Aegis Posts now trace deterministic power from a completed Anchor through completed Power Links, with each edge limited to 800 cm. A connected Post autonomously attacks only ordinarily visible hostile entities within 900 cm for 28 damage every 20 ticks. A public geometric ring/diamond distinguishes powered state without relying on color. Destroying an intermediate link disconnects every downstream Post after simultaneous damage for the tick resolves; isolated, incomplete, and unpowered Posts do not fire. Schema-18 snapshots and replays persist the authored rule and derived public state and reject a power flag inconsistent with the surviving network. All eight named units and eight named structures retain distinct authoritative categories and baseline values, and the prior six faction mechanics retain their recorded counterplay. Direct Kharuun player-side faction selection remains open, so valid player-side Waystone, warform, Cairnback, and vibration-presentation use is not reachable in the current Meridian-only scenario. The normal runtime observed one connected scenario Aegis after presentation synchronization, but not a live shot or severed-link transition. Those combat/counterplay paths are deterministic and integration tested, not rendered-match accepted. Broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored technology paths, and balance qualification remain open.
