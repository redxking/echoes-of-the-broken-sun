...
The current 0.34.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. It gathers and scouts, chooses a Future Well protocol by personality, produces workers and line units, adds a production structure when missing, adds logistics capacity under population pressure, attacks currently visible enemies with all combat categories, and withdraws low-health combatants toward its own headquarters. AI decisions consume a separately materialized player-view snapshot containing owned state, currently visible observations, known terrain, and the same versioned authored rules used by authority. Hidden entities are omitted; visible-opponent internals are redacted; unexplored terrain and hidden temporary passability changes are not exposed; and the pure command producer receives no authoritative simulation handle. All eight named units and eight named structures have distinct authoritative categories and baseline authored values. Meridian Bulwark deployment is public cardinal-facing state: its authored footprint reduces frontal incoming damage without stacking, flanking bypasses it, and deployed movement is reduced. Relay Skiff supply extension is also public: activation requires continuous range to an owned Anchor or Power Link, grants four temporary Logistics for 400 ticks, collapses on disconnection or destruction, and cannot reactivate until tick 800. Opponents see whether either ability is active without receiving hidden cooldown or health data. Waystone migration, warform adaptation, Cairnback temporary cover, vibration detection, powered Aegis behavior, broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored technology paths, and balance qualification remain open.
