...
The current 0.35.0 Glass Scar opponent is a bounded standard-difficulty Adaptive implementation. It gathers and scouts, chooses a Future Well protocol by personality, produces workers and line units, adds production and logistics capacity from its redacted player view, attacks currently visible enemies with all combat categories, and withdraws low-health combatants toward its own headquarters. All eight named units and eight named structures have distinct authoritative categories and baseline authored values. Meridian Bulwark deployment and Relay supply extension retain their public counterplay. Kharuun Waystones now expose rooted, transitioning, and mobile state: only rooted Waystones provide Logistics and accept Matter delivery; uprooting and rooting take authored time; mobile or transitioning Waystones take additional damage; mobile Waystones move slowly; and rooting fails back to mobile if the footprint becomes blocked. Opponents see the public state without receiving exact transition timing or health. The scenario now includes the authored Waystone. Direct Kharuun player-side faction selection remains open, so the Hyphen control is mapped and validated but is not reachable in the current Meridian-only player scenario. Warform adaptation, Cairnback temporary cover, vibration detection, powered Aegis behavior, broader composition response, coordinated tactical groups, repair/recovery, multiple bases, authored technology paths, and balance qualification remain open.
