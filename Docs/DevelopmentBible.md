...
The current implementation exposes a deliberately bounded subset: a visible, uncaptured and unlocked RTS cursor; a keyboard-operable feature-gated title screen; predeployment operations brief; visibility-scoped live objective tracker; explicit victory/draw/defeat presentation with Enter or R redeployment; keyboard-operable field and technology modals; persistent HUD scale; high-contrast HUD; reduced motion; reduced flashing; edge-pan enablement; adjustable pan speed; adjustable zoom step; offline pause; transactional quick-save recovery; four distinct ownership-marker shapes; and a fog-respecting tactical minimap. The title presents only the implemented Glass Scar single-player operation and fair-view/fair-economy opponent boundary; campaign, multiplayer, and replay are hidden rather than implied. Implemented accessibility settings are available before deployment. Enter advances from title to the responsive operations brief and then deploys, while the deterministic simulation remains held at tick zero and battlefield/camera input remains blocked. Unattended automation and profiling may explicitly auto-start. Escape or P opens the field menu during play. F2 opens the two-tier technology archive; Up/Down focuses a tier, Enter activates the focused tier, Shift+R chooses the next incomplete tier, and F2/Escape/P closes it. Both modals pause the deterministic match and lock battlefield/camera input. The end-state modal likewise locks battlefield and camera input. The objective tracker exposes owned Command Core integrity, currently visible Future Well protocol state, currently visible hostile-core contact, and the public match result without reading hidden hostile health. The minimap presents only legitimately explored blocked terrain and currently visible entities; squares, crosses, diamonds, and plus signs supplement team color. Broader skirmish setup, remapping, subtitles, audio-category controls, reduced dynamic range, a complete accessibility menu, tutorial replay, search across a larger technology tree, autosave, and campaign-choice confirmation remain design requirements rather than implemented features.
