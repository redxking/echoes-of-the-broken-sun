...
- CONCEPT-025: Five Campaign Character Leads Visual Identity & Roster Specification (`SPEC-CANON-009..013`, `DEMO-NAR-010`). Defines canonical appearance, silhouette, and costume parameters for Commander Mara Vey (pale ceramic/graphite armor, brass joints, cyan conduits), Oruun-of-Seven-Stones (layered basalt/strata carapace, seven polished amber memory stones), Talar Venn (archive field webbing, ledger folio canisters, ceramic pauldrons), Chancellor Cael Rhyse (structured civic mantle, ivory/gold trim), and Neme (phase-shifted crystalline lattice, magenta/violet probability offsets).
- CONCEPT-026: Hollow Choir Four-Unit Roster Concept Architecture Specification (`SPEC-CHOIR-001..004`). Defines 2x2 presentation sheet for Threadkeeper (phase-limb worker with luminous thread manipulators), Intervalist (phase skirmisher with split lance and displacement trail), Lacuna Warden (folded obsidian shell with probability dampener), and Afterimage (misdirection scout with trailing temporal echoes).
