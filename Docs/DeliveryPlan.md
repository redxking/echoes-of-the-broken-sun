...
| Saved baseline | `main` = `origin/main` after the second-slice commits (code `a37bacd`: planner keeps Well captures, status band clears the console, fixture arrival/version/opponent-hold repairs; docs commit following it). Earlier today: code `d2992b7`, docs `68653ab`. |
...
| Last retained evidence | `BuildArtifacts/Evidence/d2-foundation-20260911T0050Z`: `test_sim-08.log` 138/138 native tests in three configurations; `build-04.log` Result Succeeded; `automation-03/index.json` 138 tests, 135 passed, 3 failed (`Campaign.FreshJourney` M12 Well reveal, `Campaign.FutureThatWon` second district readback, `Campaign.TheBrokenSun` accord approach). Day sequence: 12 on main → 10 → 7 → 3, none added. Cleared today: `SkirmishDeterminismSmoke`, `OrderQueue`, `TheBrokenSunAlternateResolutionPersistence`, `CompleteSkirmish`, `M01SurveyorRig`, `GameUserSettings`, `StandardLongRunCorefall`, `FactionResearch`, `SeveralVoicesOneCommand`. |
| Next exact action after resume | Root-cause the three remaining campaign failures (M12 Well reveal, M12 second district readback, M15 accord approach) as one family: a unit ordered to an authored mission site under solid footprints. If it is a movement rule, repair it in D2; if it is authored site placement, record it against `SPEC-MSN-012/015` for D6/D7 and move to the D2 exit check: a player gathers, builds, trains, moves, fights, repairs and recovers under 30 on an ordinary map with rendered/physical evidence, which automation cannot supply. No D3 work before that. |
