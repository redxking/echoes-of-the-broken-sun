...
| Completed indexing | D0 preservation (`f8bfd47`), D1 binding (`23e7230`), retained main work (`bdd2d8a`), first D2 code (`120b60c`: 30-cap count, 4×4 Foundry, resource-depletion and unit-abilities salvage integrated). `b0108c1` ("D3") was a reverted deck-card change; later D3 slices are recorded below. |
| Saved baseline | Current HEAD `34906cf` plus this task’s uncommitted save-recovery/test/document changes and preserved unrelated art. Another session committed its existing map/harness work during verification; this task made no commit or push. Baseline and final source hashes: BuildArtifacts/Evidence/reshape-checkpoint-20260912. |
...
| Last retained evidence | Reshape checkpoint defect closed at native/editor level, 2026-09-12. Exact baseline failure at tick2280/tile20,32 retained. Existing native153/153 plus integrated new regression1/1 each pass optimized/debug/ASan+UBSan. Mac UE5.8.2 editor build passed; final full engine140/141 plus stale Mission14 test rebuilt and targeted1/1 pass (all141 cases covered across runs). Exact restore tick/checksum/payload, wrong-map refusal, malformed history and verified legacy replay recovery exercised. Save sandbox cleanup passed. Receipt: BuildArtifacts/Evidence/reshape-checkpoint-20260912/receipt.md. |
| Next exact action after resume | Standalone D2 match/recovery preflight prepared under owner authorization (2026-09-12). Owner authorized the scoped save-repair/test/tracking commit/push and prerequisite repairs; proceed with publishing the candidate: the established procedure requires exact clean pushed main, while the repair remains dirty. Review paths/patch and unchanged 15-file candidate hashes in BuildArtifacts/Evidence/standalone-match-20260912. After publication, reserve heavy resources, package a fresh detached worktree, then perform the visible gather/build/train/move/fight/save/quit/relaunch/load route. No package built or game launched; no retained process. Preserve unrelated art, parked AI/balance TBR-STR-008/009, and the known duplicate SPEC-INFO-004 documentation blocker. |
...
## Current production audit — 2026-09-12

Source: main `0ff2e654d4a872c30fd8afe9928c6eeb42b2bb02` plus preserved dirty
map/harness/native-test/art changes. Mac 26.6.2; installed Unreal 5.8.2. Baseline
editor build passed in this session; historical gameplay suites are not transferred
to this dirty candidate. Read-only inventory is representative, not an asset-by-asset
qualification. The live thread inventory showed no other active project thread;
no editor/compiler was running before this session's reserved build.

| Item | Evidence location | Status | Dependencies | Remaining work | Verification needed |
|---|---|---|---|---|---|
| Editor build baseline | BuildArtifacts/Evidence/checkpoint-cover-20260912/baseline-build.log | Verified complete | Installed UE 5.8.2 / Mac toolchain | Rebuild new test | Baseline compilation only; no player claim |
| D2 integrated foundation | RequirementsState D2 owner findings; active table above | Partially implemented | Human replay of repaired findings | Gather/build/train/move/fight/repair/recover journey | Current packaged and human evidence |
| Mineral-cover checkpoint regression coverage | Source/EchoesOfTheBrokenSun/Private/Tests/EchoesMineralCoverCheckpointTest.cpp; checkpoint-cover-20260912 reports | Verified complete | UE 5.8.2 Mac editor / isolated saves | Parent REL-SAV-005 retains unresolved Reshape case | 1/1 focused and 13/13 persistence; exact tick/checksum/cover/terrain, null renderer only |
| Reshape expiry checkpoint recovery | Source/EchoesSimCore; EchoesSimulationSubsystem; reshape-checkpoint-20260912/receipt.md | Verified complete | Snapshot33/replay38; compatible historical replay writers | Packaged/physical and human acceptance remain separate | Exact native/editor round trip, semantic negatives, wrong-map refusal and replay-proven legacy recovery; no blanket old-save forgiveness |
| D3 Meridian / opponent AI | Source/EchoesSimCore; RequirementsState D3 fifth slice | Partially implemented | D2 exit; TBR-STR-009 | Respect parked AI/balance work | Non-degenerate conditions before balance conclusions |
| Campaign world | Content/World/Source/Campaign; 15 mission JSONs and manifest | Partially implemented | Mission-specific gameplay and production | Integrated campaign qualification | M01–M15 rendered/input and narrative checks |
| Art pipeline | Content/Art/Generated/Meridian/Units/SM_Meridian_Bulwark.uasset; Docs/Archive/AssetRegister.md | Implemented but unverified | Provenance and runtime bindings | Inspect scale/material/motion/collision at runtime | Generated asset presence is not visual acceptance |
| Owner music | Content/Audio/Source/Music/OwnerProvided/manifest.json | Partially implemented | 12 retained MP3 candidates unassigned | Approved assignment and integration | Listening, transitions, mix and rights |
| Multiplayer | EchoesNetworkSession, NetworkSnapshotFlow, NetworkActionDispatch; Scripts/run_network_* | Implemented but unverified | Server/client builds and isolated runs | Current multi-process qualification | Authority, reconnect, complete match; no single-player substitution |
| Existing Mac artifacts | Binaries/Mac | Implemented but unverified | Exact source/package identity | Inspect and qualify candidate | Packaged runtime/signing/physical input |
| Older Unity migration lead | No Unity root found in shallow workspace inspection; nine Unreal worktrees found | Blocked | Locate retained project if needed | Unreal selected by current project instructions | No Unity capability or completion claim |

The previous saved-baseline row described an older push, not current HEAD. The
completed-indexing sentence saying D3 had not started contradicted later D3 receipts;
that sentence is historical, superseded by the first-unfinished and evidence rows.
The installed Blender application and available image-generation tool have not been
exercised here; audio-generation and remote-machine access remain unverified.
