...
| Next exact action after resume | Owner play test of the D2 chain on 2026-09-11: FAILED (owner's ruling; four findings in RequirementsState "D2 owner play test — FAILED"): production must require network power (owner ruling to author and implement), the command deck must show unit names, costs, layout hotkeys and disabled reasons (no "Semicolon"/"Apostrophe" drawn over tiles), a selected deposit must show remaining Matter and a legible exhausted state, and production refusals must name the missing resource and its source. Repair these, prove them through native and Unreal suites and a rendered check, then return the chain to the owner. Carry the open decisions: SPEC-MOV-003 route-field body blindness (the review-run builder stalls were NOT this: they were a frozen worker in the half-tile gap between the Core and a supply node, a SPEC-MOV-006/008 route-mask defect, fixed at replay schema 31 and tested; see RequirementsState), REL-AI-024, REL-AI-031; TBR-SCP-012 is decided (option B, D7). |
