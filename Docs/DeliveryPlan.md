...
| Last retained evidence | D3 AI slice (2026-09-12, RequirementsState "D3 Meridian slice — fifth slice"): `BuildArtifacts/Evidence/d3-meridian-20260911T161144Z` — full Unreal suite 140/140 (`automation-approach-doctrine`), native 150/150 (`test_sim-28.log`). Fourteen planner and doctrine fixes; the two failures that had survived every earlier suite are closed. `CompleteSkirmishDefeat` finishes at tick 9,824 (an opponent that could not see a Well another player had claimed, on a map where only workers cost no Dawn), and `StandardLongRunCorefall` passes with all three scenarios reaching their Well (a Well 31 tiles from either Core against a best authored sight of 15). Balance matrix 807/1000 (`balance-matrix-8.json`), whose win rates are NOT faction evidence: four pairings are one deterministic match replayed 111 times. |
