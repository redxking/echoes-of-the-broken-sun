...
10. **Where the work runs:** builds, Unreal automation, packaging, profiling, and asset generation run on
    ...
    through `echoes-heavy-run-coordination`.
11. **Git:** feature branches named for their track; stage explicit paths, never `git add -A`; never
    rewrite history on `main`; commit messages state what changed and what evidence backs it; no AI is
    credited as author.
12. Keep responses concise. No filler, no restating this brief.
