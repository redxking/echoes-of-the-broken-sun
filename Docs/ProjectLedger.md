...
The project is transitioning from **Milestone 2 — Unreal playable-systems prototype** into the **Milestone 3 Glass Scar vertical slice**. The hardened engine-independent simulation passes 15/15 optimized, debug, and sanitizer suites. Unreal Engine 5.8.2 project generation succeeds; the Mac Development Editor target compiles and links for arm64; nine Unreal automation tests pass, including deterministic hold and guard stances, ten-slot control groups, production, logistics, pause/restart, a complete attack-move skirmish, checkpoint/backup recovery, visibility lifecycle, health bars, fog/shroud, and Glass Scar terrain. Version 0.10.0 adds `[J]` guard with a six-tile defensive leash and two-tile follow distance around a designated owned entity; schema-6 snapshots preserve the relationship. Rendered inspection reached `[J]` without HUD clipping, while actual guard behavior is native- and Unreal-automation covered. The latest accepted artifact remains the self-contained 0.8.0 arm64 Mac Development application traceable to clean source commit `958e1b7`; 0.10.0 is source-qualified but not packaged. This does not yet establish complete manual playability, patrol, a pointer-driven/rendered end-to-end match, navigation/fog performance at production scale, multiplayer, final art/audio, Developer ID signing, notarization, clean-machine compatibility, or distribution readiness.
...
**State:** Substantially complete for the technical spike. Commit `adad96e` records the historical 7/7 baseline; the current integrated hardening passes 13/13 in all three configurations. Content-pack import, authenticated multiplayer-seat binding, replay work limits, and stable execution-time rejection reporting remain open.
...
**State:** In progress. Compilation, deterministic obstacle routing, visibility/fog, production, logistics, pause/restart, hold, guard, ten control groups, rendered startup, current-source packaging, and bounded keyboard controls pass at their recorded boundaries. `[F]` attack-move engages visible hostiles and resumes; `[H]` holds a fixed anchor; `[J]` follows and defends a designated owned entity within a bounded leash; `[X]` interrupts all three. One automated composed skirmish crosses the Glass Scar and reaches victory. Exact pointer-driven construction/production/combat, selected-unit hold, pointer-targeted guard, control-group assignment with selected units, and a full rendered match have not been accepted. Patrol, navigation/fog stress measurement, and final terrain/fog art remain open.
...
Complete pointer-driven selection, placement, production, movement, combat, victory, and restart validation as one rendered match. Then profile Glass Scar routing, terrain synchronization, and fog/shroud at the target unit and map scale before expanding the encounter, faction rosters, and final visual content.
