...
**State:** In progress. Compilation, module loading, class/bootstrap automation, deterministic obstacle routing, world-level visibility actor lifecycle, 4,096-tile fog/shroud with explored-state persistence, timed worker/soldier production, logistics capacity, pause, deterministic restart, null-RHI scenario creation, rendered startup, current-source packaging, and bounded keyboard controls pass at their recorded boundaries. One automated composed skirmish now completes drop-off construction, legitimate Future Well scouting and capture, reinforcement production, force rally, visible-target combat, opposing Command-Core victory, post-result freeze, and restart. Construction hotkeys and production controls are wired, but exact pointer-driven construction/production/combat and a full rendered match have not been accepted. Broader orders, navigation/fog stress measurement, and final fog art remain open.
