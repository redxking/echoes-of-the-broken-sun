...
The project is transitioning from **Milestone 2 — Unreal playable-systems prototype** into the **Milestone 3 Glass Scar vertical slice**. The hardened engine-independent simulation passes 13/13 optimized, debug, and sanitizer suites. Unreal Engine 5.8.2 project generation succeeds; the Mac Development Editor target compiles and links for arm64; six Unreal automation tests pass, including production, logistics, pause/restart, one complete one-order attack-move skirmish, deterministic checkpoint/backup recovery, entity visibility lifecycle, health-bar state presentation, 4,096-tile fog/shroud accounting, and the authored Glass Scar terrain. The current scenario has a deterministic 165-tile fractured span, a central Future Well crossing, and two side-cavern routes; a real unit has routed across it under automation. Version 0.7.0 writes a deterministic checkpoint through a validated temporary file, retains one prior generation, rejects incompatible/corrupt snapshots, falls back to the backup, reconstructs disposable views, and resumes command sequencing. Rendered Editor and packaged-app inspection each visibly proved a checkpoint commit and exact tick rollback without disturbing normal lit rendering. A self-contained 0.7.0 arm64 Mac Development application is traceable to clean source commit `bcdb6d9` and passed local build, cook, stage, package, structural, signature-seal, content-manifest, Glass Scar, fog, packaged-startup, and rendered checkpoint checks. This does not yet establish complete manual playability, a pointer-driven or rendered end-to-end match, navigation/fog performance at production scale, multiplayer, final art/audio, Developer ID signing, notarization, clean-machine compatibility, or distribution readiness.
...
**State:** In progress. Compilation, module loading, class/bootstrap automation, deterministic obstacle routing, world-level visibility actor lifecycle, 4,096-tile fog/shroud with explored-state persistence, timed worker/soldier production, logistics capacity, pause, deterministic restart, null-RHI scenario creation, rendered startup, current-source packaging, and bounded keyboard controls pass at their recorded boundaries. Deterministic attack-move now acquires locally visible hostiles, resumes its destination after contact, and can be interrupted by stop; `[F]` and `[X]` expose those commands. One automated composed skirmish completes drop-off construction, legitimate Future Well scouting and capture, safe-side reinforcement rally, traversal through the Glass Scar, opposing Command-Core victory, post-result freeze, and restart from a single attack-move order per soldier. Exact pointer-driven construction/production/combat and a full rendered match have not been accepted. Hold, patrol, guard, control groups, navigation/fog stress measurement, and final terrain/fog art remain open.
