...
The project is in **Milestone 2 — Unreal playable-systems prototype**. The hardened engine-independent simulation passes 13/13 optimized, debug, and sanitizer suites, including deterministic routing around a blocked terrain wall plus attack-move acquisition, resume, and stop behavior. Unreal Engine 5.8.2 project generation succeeds; the Mac Development Editor target compiles and links for arm64; four Unreal automation tests pass, including production, logistics, pause/restart, one complete one-order attack-move skirmish, entity visibility lifecycle, and 4,096-tile fog/shroud accounting with persistent exploration; and null-RHI bootstrap reaches the first fixed tick with 25 entities and 10 visible views at 20 Hz. The current fog boundary, HUD, and pause feedback have been visually observed. A self-contained 0.4.0 arm64 Mac Development application is traceable to clean source commit `43ec89e` and passed local build, cook, stage, package, structural, signature-seal, content-manifest, fog, and packaged-startup checks. This does not yet establish complete manual playability, a pointer-driven or rendered end-to-end match, navigation/fog performance at production scale, multiplayer, Developer ID signing, notarization, clean-machine compatibility, or distribution readiness.
...
Complete pointer-driven selection, placement, production, movement, combat, victory, and restart validation as one rendered match. Then profile obstacle routing and fog/shroud at the target unit and map scale before expanding the Glass Scar encounter and visual content.
