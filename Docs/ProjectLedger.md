...
**State:** In progress. Compilation, module loading, class/bootstrap automation, deterministic obstacle routing, world-level visibility actor lifecycle, 4,096-tile fog/shroud with explored-state persistence, timed worker/soldier production, logistics capacity, pause, deterministic restart, derived Command-Core victory, null-RHI scenario creation, rendered startup, and bounded keyboard controls pass at their recorded boundaries. Construction hotkeys and production controls are wired, but exact pointer-driven construction/production/combat and a full rendered match have not been accepted. Broader orders, navigation/fog stress measurement, final fog art, and current-source packaging remain open.
