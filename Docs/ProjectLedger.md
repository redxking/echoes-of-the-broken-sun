...
The project is in **Milestone 2 — Unreal playable-systems prototype**. The hardened engine-independent simulation passes 11/11 optimized, debug, and sanitizer suites. Unreal Engine 5.8.2 project generation succeeds; the Mac Development Editor target compiles and links for arm64; three Unreal automation tests pass, including a real visible-hidden-visible actor lifecycle plus production, logistics, pause, and restart through the world subsystem; and null-RHI bootstrap reaches the first fixed tick with 25 entities and 10 visible views at 20 Hz. The rendered current HUD and pause/restart feedback have been observed. A separately traceable self-contained arm64 Mac Development application still represents the earlier technical-prototype commit, not this newer playable-loop work. This does not yet establish complete manual playability, player-visible fog/shroud, pathing, a qualified end-to-end match, performance, multiplayer, current-source packaging, Developer ID signing, notarization, clean-machine compatibility, or distribution readiness.
...
**State:** Substantially complete for the technical spike. Commit `adad96e` records the historical 7/7 baseline; the current integrated hardening passes 11/11 in all three configurations. Content-pack import, authenticated multiplayer-seat binding, explored-state player views, replay work limits, and stable execution-time rejection reporting remain open.
...
**State:** In progress. Compilation, module loading, class/bootstrap automation, world-level visibility actor lifecycle, timed worker/soldier production, logistics capacity, pause, deterministic restart, derived Command-Core victory, null-RHI scenario creation, rendered startup, and bounded keyboard controls pass at their recorded boundaries. Construction hotkeys and production controls are wired, but exact pointer-driven construction/production/combat and a full rendered match have not been accepted. A true fog/shroud and explored-state surface, deterministic obstacle pathing, broader orders, stress measurement, and current-source packaging remain open.
...
| ENV-003 | 2026-08-28 | Current local host | `xcode-select -p`, storage observation after final package, test evidence, and Echoes-only Xcode index cleanup | Full Xcode selected at `/Applications/Xcode.app/Contents/Developer`; about 83 GiB free | Point-in-time local state after final verification; above the 40 GiB prototype stop threshold and 60 GiB packaging headroom, below the 100 GiB sustained-production target; no clean-machine result |
...
- Camera, selection, context-order, construction/production hotkeys, pause/restart, HUD, entity-view, arena, and lighting code exists. Views are scoped to currently visible entities, but no rendered fog/shroud surface or explored-state presentation exists, and exact-build pointer selection, placement, production, and context orders remain unverified.
...
Implement deterministic obstacle pathing and the true fog/shroud/explored-state presentation, then complete pointer-driven selection, placement, production, combat, victory, and restart validation as one rendered match. Profile the accepted target scene before expanding visual content.
