...
The project is transitioning from **Milestone 2 — Unreal playable-systems prototype** into the **Milestone 3 Glass Scar vertical slice**. The hardened engine-independent simulation passes 14/14 optimized, debug, and sanitizer suites. Unreal Engine 5.8.2 project generation succeeds; the Mac Development Editor target compiles and links for arm64; eight Unreal automation tests pass, including deterministic hold position, validated ten-slot control groups, production, logistics, pause/restart, one complete one-order attack-move skirmish, deterministic checkpoint/backup recovery, entity visibility lifecycle, health-bar state presentation, 4,096-tile fog/shroud accounting, and the authored Glass Scar terrain. Version 0.9.0 anchors attack-capable units with `[H]`, limits autonomous fire to visible enemies already in weapon range, never chases, persists the stance through schema-5 checkpoints, and clears it with `[X]`. Rendered source inspection reached the hold key's no-selection path without HUD clipping; actual stance behavior is native- and Unreal-automation covered. The latest accepted artifact remains the self-contained 0.8.0 arm64 Mac Development application traceable to clean source commit `958e1b7`; 0.9.0 is source-qualified but not packaged. This does not yet establish complete manual playability, a pointer-driven or rendered end-to-end match, navigation/fog performance at production scale, multiplayer, final art/audio, Developer ID signing, notarization, clean-machine compatibility, or distribution readiness.
...
**State:** In progress. Compilation, module loading, class/bootstrap automation, deterministic obstacle routing, world-level visibility actor lifecycle, 4,096-tile fog/shroud with explored-state persistence, timed worker/soldier production, logistics capacity, pause, deterministic restart, hold position, ten validated control groups, null-RHI scenario creation, rendered startup, current-source packaging, and bounded keyboard controls pass at their recorded boundaries. Deterministic attack-move acquires locally visible hostiles, resumes its destination after contact, and can be interrupted by stop; `[F]` and `[X]` expose those commands. `[H]` anchors attack-capable defenders, fires only at visible targets already in range, and never chases. `[G]` followed by `[1-0]` assigns the current local selection, while bare `[1-0]` recalls it and prunes stale IDs. One automated composed skirmish completes drop-off construction, legitimate Future Well scouting and capture, safe-side reinforcement rally, traversal through the Glass Scar, opposing Command-Core victory, post-result freeze, and restart from a single attack-move order per soldier. Exact pointer-driven construction/production/combat, selected-unit hold/control-group assignment, and a full rendered match have not been accepted. Patrol, guard, navigation/fog stress measurement, and final terrain/fog art remain open.
