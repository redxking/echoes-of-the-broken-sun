...
assert.equal(nodes['#tabs'].children.length,8);assert.equal(nodes['#asset'].children.length,21);
...
console.log(JSON.stringify({studies:8,samples:96,candidate_records:21,control_checks:4,draw_calls:draws,evidence:'source execution with mocked canvas; NOT browser visual verification'}));
