...
Six preservation tests exercise archive round-trip, inventory drift, overwrite refusal, output recursion, tampered payloads and unmanifested members. The local export is retained under `BuildArtifacts/Evidence/concept-handoff-20260906/concept-pipeline-handoff.zip`; its separately tracked receipt binds the exported bytes. Rebuilding an archive uses a new explicit output path; no source cleanup or replacement is performed.
