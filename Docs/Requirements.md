...
* **SPEC-RES-006 — Deposit depletion lifecycle:** At zero Matter, stop extraction immediately and make the deposit non-interactable. Preserve recognizable exhausted terrain while observed and its last observed state under fog-of-war memory rules; no 200-tick disappearance or hidden-state reveal is permitted. Finish delivery of already extracted cargo through the valid delivery rules, then enter the Idle Worker registry and alert the player. Workers shall not automatically select another deposit; reassignment requires a player order, and unscouted deposits shall never be selected through hidden knowledge.
   * **SPEC-RES-006.INSPECT:** Owner ruling 2026-09-11 (D2 play test: "the Matter graphic doesn't show anything once it's mined out or how much is left when a user clicks on it"). Clicking a visible Matter deposit shall show its known remaining Matter in the selection card and the status line without adding the deposit to the command selection or arming any order; a remembered deposit under fog shows no figure. An exhausted deposit shall keep a reduced dark stub on the field (30% height, 80% footprint of the full deposit) and a muted minimap mark rather than vanishing. Verification: UI (EchoesFieldHudViewTest deposit entries and exhausted marker; EchoesFieldHudWidgetTest selection card); acceptance stays with the owner.
...
* **SPEC-RES-008 — Mobile Entity Limit:** Each player is limited to a hard cap of exactly 30 controllable mobile entities at any time, including workers and command characters. Buildings and static defenses are excluded from this count. This constraint operates concurrently with the 200 Logistics ceiling; if either is exhausted, further production or deployment of mobile units is blocked.
...
* **SPEC-BAL-009 — BAL-STR-1 Blob Versus Frontage (TBR-STR-005):** A 1.3× force (13 attackers against 10; amended 2026-09-11 from 1.6×, which no chokepoint holds under the square law of massed fire) that attack-moves through a two-tile chokepoint into a prepared defender holding two ranks beside the mouth shall lose at least 70% of seeded matches, and the identical recording replayed without `SPEC-CMB-013` firing lanes (schema 32 continuation) shall show a lower defender rate, otherwise the pass is not evidence for the rule.
  ...
  * **SPEC-BAL-009.MEASURE:** 2026-09-11 sweep (RequirementsState "Chokepoint sweep"): the defender holds to 12 attackers on current rules and to 13 with role body radii (30/30), and nothing holds 16. At 13 attackers current rules give 9/30 with lanes and 3/30 without. With `TBR-STR-006` role bodies (schema 35) the defender wins 60/60 at 13 attackers against a schema-32 control of 7/60; the control disables firing lanes and role bodies together, so it measures their combined effect.
...
* **SPEC-UI-007 —** Bottom command console and clear battlefield (owner direction, 2026-09-07). Concentrate the minimap, selection details and command controls in a bottom console using the familiar RTS arrangement: minimap left, selection center, command card right. Move large resource/objective/tutorial information panels out of the upper battlefield; compact global indicators may remain as appropriate to the supplied reference. Preserve legible text, accessible controls, matching pointer-blocking bounds and all existing command information. Use Echoes visual identity. At initial deployment frame the actual player-owned base centrally, maintain a stable tactical camera angle, and provide consistent scrolling without direction-dependent speed or unintended outside-window input. Retain working cursor-centered zoom and map-corner reachability. Verify layout and camera through rendered input as well as source/automation; frame-rate consistency tests alone do not establish perceived smoothness.
...
* **REL-FAC-002 — Meridian Compact Power Grid Topology:** Meridian structures shall operate on an interconnected power network. Power Links and Array Foundries project an 800 cm power distribution radius.
  ...
  * **REL-FAC-002.PROD:** Owner ruling 2026-09-11 (D2 play test: "I can build stuff away from power — that's fine — but then I shouldn't be able to build unless it connected to power"). An Array Foundry may be placed and constructed anywhere legal, but a completed Foundry outside the operational network shall neither start nor advance unit production: a Produce order is refused with `[PRODUCER_UNPOWERED]` naming the Power Link remedy, the active item holds its progress and the waiting queue stays queued without refund, and reconnection resumes from the held progress within one simulation tick. The command tile stays pressable and reads the unpowered reason; the selection card states "produces only while connected". Command Cores and the other factions' producers are not gated. Replay schema 32 carries the gate; older recordings keep their ungated production. The `.UX` clause's ban on *falsely* disabling Foundry production stands: the disabling is the simulation's own connectivity, never a presentation inference. Verification: `SRC` (native "Meridian Foundry produces only while powered"; `Echoes.Runtime.Presentation.FieldHudAuthority` unpowered tile and card); acceptance stays with the owner.
