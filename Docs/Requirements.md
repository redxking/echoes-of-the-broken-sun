...
* .ANIM_RIG: 22-bone kinematic rigging layout for a quadruped chassis: root, body, two-bone forward prow, independently aiming two-bone caster mount, and four four-bone limb chains. Skeleton layout carries fixed named sockets: `VFX_Muzzle_Shard_01`, `VFX_Molt_Origin_Base`, and `Target_Hitbox_Center`. Locomotion requires direction-independent movement speeds and turn rates with instant snaps under Reduced Motion.
