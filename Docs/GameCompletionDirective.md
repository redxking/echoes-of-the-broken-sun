...
2. **Typeface** (blocks A8, J2). A licensed or original typeface for game and site, license
   ...
   **VENDORED 2026-09-04:** both families under `Content/UI/Fonts/` with `OFL.txt` beside them, bound at
   runtime through `EchoesTypeface` (HUD chrome and readouts), staged as UFS for packaging; register
   FONT-001. The dependency on this decision is cleared for gates 10 and 47.
...
| 10 | Interface art system complete at five resolutions, standard and high contrast | A8 | 3 | IN PROGRESS — OD 2 typeface vendored and bound 2026-09-04 (FONT-001). Field HUD rearranged the same day on the owner's direction to the RTS convention (ledger UI-A8-002): full-width instrument bar with minimap left, selection panel centre, command card right (grid, dormant state drawn); ledger top-right in the mono face; command strip and objectives top-left; status line above the bar; the key legend moved into the field menu. Rendered at 1920×1080 and 1280×720 (`BuildArtifacts/Evidence/release-gate10-hud-layout-20260904/`); pointer chrome hit-testing and layout tests carried to the new contract; suite green. Selection panel carries a sigil portrait, integrity bar, and group profile; minimap caption sits inside the bar; captured armed (`-EchoesArtReviewSelectForce`, scouted force at the Well), at 2560×1440 and 1280×720, and in high contrast (`-EchoesArtReviewHighContrast`). Remaining: menu/brief/lobby restyle, packaged capture, the full A8 system, owner review |
...
| 50 | Glass Scar composed frame matches `CONCEPT-004` as one frame: dais Well, chasm and cliffs, unit scale and facing, fractured sun | A10 | 3, 5, 6 | IN PROGRESS — owner delegated all decisions on this gate 2026-09-04. Done: Bulwark facing (`19906f9`); roster readability scale on a presentation pivot (Worker/Scout 1.5, Soldier 1.6, Heavy 1.75) and deposits at 0.62 (`7b068c5`, `0a1e26d`); shelf mesh regenerated flush at `glass-scar-shelf-vitrified-v2` (ART-017, `0ed81b4`); and the chasm moved into live play: `AEchoesTerrainView` derives the scar band from the live terrain rows (30–34, four deep spans, five open) and draws banks, terrace, rim teeth, bed, and fissure light as instanced presentation layers, the collision floor stops rendering there while keeping collision, band-row ridge teeth and dressing records drop to the bed, and the review fixture uses the same layers (ledger WORLD-A4-003). Live-play evidence is earned by a scripted scout (`-EchoesArtReviewScout`, real move orders on the local player's units) that reveals the Well before capture. Evidence set `BuildArtifacts/Evidence/release-gate50-composed-frame-20260904/`: fixture and live-play frames at 1920×1080 and 2560×1440, a HUD-on live frame, and four concept pairing sheets; suite 77/77. Broken Sun regenerated as a fractured sphere with drifting shards (ART-018, `broken-sun-sky-fractured-v3`), core emissive held under clip, fixture camera pitch −19 so the sphere sits in the sky band; Glass Scar height-fog haze deepened toward the concept's indigo. Ash Cut and Folded Verge regenerated with chasm piers (ART-019/020, production-v2); Dormant Well core dimmed to a dark shard with the golden light kept on rings and dais. Exposure measured on the evidence set with `Scripts/measure_capture_exposure.py`: live Well frame clipped 0.00005 / near-clip 0.00011 / mean luma 62.0; live Ash Cut frame 0.00001 / 0.00079 / 56.3; fixture 0.00001 / 0.00004 / 59.5 — all inside the A1 window (clip ≤ 0.005%, luma 50–70), so the pale dais is within the accepted rig and is not changed. Folded Verge verified in a scouted live frame (`live-foldedverge-piers-1920x1080.png`, clipped 0.00000). A sun-ward directional-inscattering term was added to the Glass Scar height fog; it produced no visible change in the fixture sky at the review distance, so the sky remains a flat haze field — an atmosphere or authored sky dome is the remaining route. Sky dome added (ART-021, `sky-dome-banded-v1`): four self-lit inward-facing bands grade the sky from a horizon haze to a near-black zenith, no atmosphere pass; emissive held so the fixture stays inside the luma window. Package `Mac-Development-20260904T171524Z-c3dc5722` built from pushed main with manifest and provenance and passed the packaged smoke (front-door pin repaired the same day); its packaged review runs load the chasm, sky dome, and vendored fonts (`[ECHOES_GLASS_SCAR_CHASM_READY]`, `[ECHOES_TYPEFACE_READY]` in the packaged logs) but the review screenshot request writes no file from the packaged binary, so the packaged frame is BLOCKED on a capture route (ledger PKG-A10-001); editor-build frames stand as authoring previews. Still open: packaged frame route, owner review |
