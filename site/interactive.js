...
(function () {
  ...
  // --- 8. Interactive World Map & Glass Scar Recon Gallery ---
  const WorldGalleryItems = [
    { id: 'world-map', title: 'Soryn Continental Map', desc: 'Six biomes, four continental plates, and fifteen campaign battlefields connected by arterial roads across the shattered surface of Soryn.', img: 'assets/concepts/soryn-world-map.webp', register: 'CONCEPT-006', badge: 'Concept target' },
    { id: 'glass-routes', title: 'Glass Scar Route Architecture', desc: 'Three distinct tactical routes through the first battlefield — the Ash Cut, the Buried Causeway, and the Folded Verge — each rewarding different faction playstyles.', img: 'assets/concepts/glass-scar-routes.webp', register: 'CONCEPT-003', badge: 'Concept target' },
    { id: 'gs-overview', title: 'Glass Scar — Engine Overview', desc: 'Full battlefield overview showing the vitrified chasm, Matter deposits, and three diverging route options in the current Unreal Engine build.', img: 'assets/engine/glass-scar-overview-in-engine.webp', conceptImg: 'assets/concepts/target-render-vertical-slice.webp', register: 'ENGINE-WIP', badge: 'Engine capture · not final art', hasCompare: true },
    { id: 'gs-ashcut', title: 'Ash Cut Route — Engine', desc: 'The aggressive flanking route through volcanic ash flats. Fast traversal, minimal cover, high risk.', img: 'assets/engine/glass-scar-ash-cut-in-engine.webp', register: 'ENGINE-WIP', badge: 'Engine capture · not final art' },
    { id: 'gs-causeway', title: 'Buried Causeway — Engine', desc: 'A subterranean passage beneath the scar. Protected from artillery but vulnerable to ambush at chokepoints.', img: 'assets/engine/glass-scar-buried-causeway-in-engine.webp', register: 'ENGINE-WIP', badge: 'Engine capture · not final art' },
    { id: 'gs-verge', title: 'Folded Verge — Engine', desc: 'The defended high-ground route along the scar edge. Slow advance with excellent firing positions.', img: 'assets/engine/glass-scar-folded-verge-in-engine.webp', register: 'ENGINE-WIP', badge: 'Engine capture · not final art' }
  ];

  function initWorldGallery() {
    const terminal = document.getElementById('world-gallery-terminal');
    if (!terminal) return;

    const strip = terminal.querySelector('.gallery-thumb-strip');
    const displayImg = document.getElementById('gallery-display-img');
    const displayTitle = document.getElementById('gallery-display-title');
    const displayDesc = document.getElementById('gallery-display-desc');
    const displayBadge = document.getElementById('gallery-display-badge');
    const displayRegister = document.getElementById('gallery-display-register');
    const compareBtn = document.getElementById('gallery-compare-toggle');

    let currentItem = 0;
    let showingEngine = true;

    // Generate thumbnail buttons
    WorldGalleryItems.forEach((item, idx) => {
      const btn = document.createElement('button');
      btn.className = 'gallery-thumb-btn' + (idx === 0 ? ' active' : '');
      btn.setAttribute('role', 'tab');
      btn.setAttribute('aria-selected', idx === 0 ? 'true' : 'false');
      btn.innerHTML = `<b>${item.title}</b>`;
      btn.addEventListener('click', () => selectGalleryItem(idx));
    ...
    function selectGalleryItem(idx) {
      currentItem = idx;
      showingEngine = true;
      const item = WorldGalleryItems[idx];

      // Update thumb active states
      strip.querySelectorAll('.gallery-thumb-btn').forEach((btn, i) => {
        btn.classList.toggle('active', i === idx);
        btn.setAttribute('aria-selected', i === idx ? 'true' : 'false');
      });

      if (displayImg) { displayImg.src = item.img; displayImg.alt = item.title; }
      if (displayTitle) displayTitle.textContent = item.title;
      if (displayDesc) displayDesc.textContent = item.desc;
      if (displayBadge) displayBadge.textContent = item.badge;
      if (displayRegister) displayRegister.textContent = item.register;

      if (compareBtn) {
        compareBtn.style.display = item.hasCompare ? 'inline-flex' : 'none';
      }
    ...
    if (compareBtn) {
      compareBtn.addEventListener('click', () => {
        const item = WorldGalleryItems[currentItem];
        if (!item.hasCompare) return;

        showingEngine = !showingEngine;
        if (displayImg) {
          displayImg.src = showingEngine ? item.img : item.conceptImg;
          displayImg.alt = item.title + (showingEngine ? ' (Engine)' : ' (Concept)');
        }
        if (displayBadge) {
          displayBadge.textContent = showingEngine ? 'Engine capture · UI hidden' : 'Concept target';
        }
  ...
  // --- 9. UI Design Concept Art Showcase ---
  const UIShowcaseItems = [
    { id: 'faction-select', title: 'Faction Selection War Room', desc: 'Three faction crests orbit a central strategic hologram. Players choose their doctrine before entering the campaign or skirmish theater.', img: 'assets/concepts/echoes-faction-selection.webp', register: 'CONCEPT-015' },
    { id: 'campaign-map', title: 'Campaign Operations Map', desc: 'Fifteen linked operations across three acts rendered as a planetary dossier with mission thumbnails, act gates, and four explicit ending branches.', img: 'assets/concepts/echoes-campaign-map.webp', register: 'CONCEPT-020' },
    { id: 'pause-ledger', title: "Mara's Field Operations Ledger", desc: 'The in-game pause menu designed as a worn leather field ledger — mission objectives, army composition, and tactical options presented diegetically.', img: 'assets/concepts/echoes-pause-field-ledger.webp', register: 'CONCEPT-021' },
    { id: 'post-match', title: 'Post-Match Combat Debriefing', desc: 'Telemetry curves, resource graphs, unit loss breakdowns, and timeline replay rendered as a military after-action report.', img: 'assets/concepts/echoes-post-match-debriefing.webp', register: 'CONCEPT-022' },
    { id: 'settings', title: 'System Configuration & Accessibility', desc: 'Loudness compliance, motion sensitivity, colorblind modes, and granular input remapping — built into the tactical terminal aesthetic.', img: 'assets/concepts/echoes-settings-accessibility.webp', register: 'CONCEPT-023' },
    { id: 'vfx-grammar', title: 'Combat Visual Effects Grammar', desc: 'Faction-distinct weapon beams, impact bursts, shield flares, and construction effects — cyan kinetic (Meridian), amber seismic (Kharuun), magenta phase (Choir).', img: 'assets/concepts/echoes-combat-vfx-grammar.webp', register: 'CONCEPT-024' }
  ];

  function initUIShowcase() {
    const terminal = document.getElementById('ui-showcase-terminal');
    if (!terminal) return;

    const strip = terminal.querySelector('.ui-showcase-thumb-strip');
    const displayImg = document.getElementById('ui-showcase-display-img');
    const displayTitle = document.getElementById('ui-showcase-display-title');
    const displayDesc = document.getElementById('ui-showcase-display-desc');
    const displayBadge = document.getElementById('ui-showcase-display-badge');
    const displayRegister = document.getElementById('ui-showcase-display-register');

    let currentItem = 0;

    UIShowcaseItems.forEach((item, idx) => {
      const btn = document.createElement('button');
      btn.className = 'gallery-thumb-btn ui-thumb-btn' + (idx === 0 ? ' active' : '');
      btn.setAttribute('role', 'tab');
      btn.setAttribute('aria-selected', idx === 0 ? 'true' : 'false');
      btn.innerHTML = `<b>${item.title}</b>`;
      btn.addEventListener('click', () => selectUIItem(idx));
      strip.appendChild(btn);
    });

    function selectUIItem(idx) {
      currentItem = idx;
      const item = UIShowcaseItems[idx];
      ...
      if (displayTitle) displayTitle.textContent = item.title;
      if (displayDesc) displayDesc.textContent = item.desc;
      if (displayBadge) displayBadge.textContent = 'Concept target';
      if (displayRegister) displayRegister.textContent = item.register;
  ...
  // --- 10. Engine Roster Comparison (Faction War Room Enhancement) ---
  function initEngineRosterComparison() {
    const warRoom = document.getElementById('faction-war-room');
    if (!warRoom) return;

    const rosterFig = document.getElementById('faction-engine-roster-figure');
    const rosterImg = document.getElementById('faction-engine-roster-img');
    if (!rosterFig || !rosterImg) return;

    const engineRosters = {
      meridian: { src: 'assets/engine/meridian-roster-in-engine.png', alt: 'Current in-engine: Meridian Compact unit roster' },
      kharuun: { src: 'assets/engine/kharuun-roster-in-engine.png', alt: 'Current in-engine: Kharuun Assemblies unit roster' },
      choir: null
    };

    // Observe data-active-faction attribute changes
    const observer = new MutationObserver(() => {
      const faction = warRoom.getAttribute('data-active-faction');
      const data = engineRosters[faction];
      if (data) {
        rosterFig.style.display = 'block';
        rosterImg.src = data.src;
        rosterImg.alt = data.alt;
      } else {
        rosterFig.style.display = 'none';
      }
    });

    observer.observe(warRoom, { attributes: true, attributeFilter: ['data-active-faction'] });
  }
