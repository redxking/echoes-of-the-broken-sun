...
**Current state:** This is a verified Unreal playable-systems prototype, not a completed game. UE 5.8.2 is installed; the native simulation passes 12/12 hardened suites in optimized, debug, and sanitizer configurations; the arm64 Development Editor target builds; Unreal automation passes 4/4, covering bootstrap, timed production, logistics, pause/restart, obstacle routing, a complete composed skirmish, and visible-hidden-visible entity plus fog/shroud lifecycle; and the current null-RHI runtime boots a 25-entity scenario with 10 initially visible views and a 4,096-tile visibility surface at 20 Hz. The complete automated skirmish builds a drop-off, captures a Future Well, produces and rallies a seven-soldier force, engages visible opposition, destroys the opposing Command Core, freezes on victory, and restarts. The rendered current build exposes the authoritative fog boundary, construction and production controls, and pause/restart feedback. A self-contained 0.3.0 Mac Development package is traceable to clean source commit `08f1bab` and passed structural, signature-seal, manifest, fog, and packaged-startup checks. Complete pointer interaction, a rendered end-to-end match, navigation-scale performance, clean-machine use, Developer ID signing, notarization, and release qualification remain open.
