...
**Current state:** This is a verified Unreal playable-systems prototype moving into its Glass Scar vertical slice, not a completed game. Version 0.34.0 retains distinct deterministic identities and production for all sixteen named Meridian Compact and Kharuun Assemblies slice archetypes and implements two Meridian faction mechanics. A Bulwark can deploy cardinal-facing public cover, reduce frontal damage within an authored protected footprint, move at an authored reduced rate while deployed, and pack up. A Relay Skiff connected to an owned Anchor or Power Link can activate a public temporary logistics field; the authored bonus expires, enters cooldown, and collapses immediately if the Relay leaves the network, its last link is severed, or the fragile Relay is destroyed. The canonical content SHA-256 is `7fac313d5ba8386fe5377c6b8c17d7c2b0608210fbd182e675f7241006e87f32`; rules, schema-13 snapshots/replays, player views, AI observations, and checksums carry both profiles and public states. The deterministic core passes 21/21 suites in optimized, debug, and sanitizer configurations; the arm64 Development Editor builds; Unreal automation passes 15/15 without warnings or errors; and normal plus 400-unit/four-team runtime gates remain green. `Backslash` controls selected Bulwarks and `Equals` activates selected connected Relays; world proxies expose the directional barrier and active supply field. Waystone migration, warform adaptation, Cairnback temporary cover, Resonant/Listening Spine detection, powered Aegis behavior, and broader faction systems remain unimplemented. Campaign, multiplayer, and replay UI remain hidden. The latest accepted package remains clean-source 0.23.1 at commit `98a9b83`; version 0.34.0 has not been packaged or profiled because storage remains at the packaging floor without safe transient margin. Final effects, formations, complete pointer interaction, a rendered end-to-end player-driven match, networked multiplayer, clean-machine use, Developer ID signing, notarization, final art/audio, complete accessibility/settings implementation, and release qualification remain open.
...
2. Keep full Xcode selected. Xcode 26.6 has passed project generation, arm64 editor compilation, fifteen Unreal automation tests, null-RHI startup for both normal and 400-unit/four-team fixtures, and local Development cooking/packaging; Epic still recommends 26.1.1, and clean-machine use remains unverified.
