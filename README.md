...
**Current state:** This is a verified Unreal playable-systems prototype entering its authored Glass Scar vertical slice, not a completed or publicly playable game. Version 0.86.0 at exact clean game-source commit `83137bfb64cabadc9a080320488cd0e4417f0f3d` retains the accepted networking and Mission 01–08 work, then implements campaign Mission 09, **Reserve Authority**. An eight-record, choice-consistent ledger grants Mara a finite Meridian reserve for Life Support, Transit, and Archive. Mara must first reach the branch-specific authority site; ordinary workers then build Power Links that power exactly two intact districts; Mara must reach the surviving deferred district to complete the operation. A third powered district, any protected loss, local-Core loss, or terminal Core outcome fails. Success appends one of the three valid exact-two allocation masks as the ninth durable record. Content passed 27/27; native simulation/protocol coverage passed 28/28 in optimized, debug, and sanitizer configurations; the arm64 editor built; and 39/39 Unreal automation tests passed with zero warnings or errors. A Preserve null-RHI startup smoke and a live Metal title, briefing, battlefield/HUD/minimap, high-contrast, and restored-default review also passed at their recorded boundaries. This establishes one repeatable local reserve allocation only. It does not establish wider-city restoration, unmodeled civilian survival, ordinary human completion, alternate full-route playthroughs, broad combat balance/usability, final assets, current packaging/performance, or full campaign completion. The latest accepted package remains clean-source 0.23.1 at commit `98a9b83`; version 0.86.0 has not been packaged, profiled, soaked, notarized, or clean-machine qualified.
...
2. Keep full Xcode selected. Xcode 26.6 has passed project generation, arm64 editor compilation, thirty-nine Unreal automation tests, null-RHI startup for both selectable faction directions and all nine implemented campaign missions, the controlled prologue-completion path, and the 400-unit/four-team fixture, plus local Development cooking/packaging; Epic still recommends 26.1.1, and clean-machine use remains unverified.
