...
**Current state:** This is a verified Unreal playable-systems prototype moving into its Glass Scar vertical slice, not a completed game. UE 5.8.2 is installed; the native simulation passes 14/14 hardened suites in optimized, debug, and sanitizer configurations; the arm64 Development Editor target builds; and Unreal automation passes 8/8 with no warnings or errors. The current scenario contains the authored Glass Scar topology: a 165-tile fractured transit band, a central Future Well crossing, and two side-cavern routes. A map-specific test proves the exact terrain layout, presentation parity, and deterministic unit routing from the southern basin to the northern basin. The complete automated skirmish still builds a drop-off, captures the Well, produces and rallies seven soldiers, crosses the Scar, reaches victory from one attack-move order per soldier, freezes after the result, and restarts. Version 0.9.0 adds deterministic `[H]` hold position: attack-capable units anchor in place, acquire only visible enemies already in weapon range, never chase, persist the stance through schema-5 checkpoints, and release it on `[X]` stop. Ten validated RTS control groups remain on `[G]` then `[1-0]` assignment and bare `[1-0]` recall; Future Well choices use `[Z]`, `[C]`, and `[V]`. Transactional checkpoints remain on `[K]` and `[L]`. Rendered inspection confirms the hold hotkey and unclipped HUD, while automation covers an actual held defender. The latest accepted self-contained package remains the clean-source 0.8.0 control-group build at commit `958e1b7`; 0.9.0 has not been packaged. Complete pointer interaction, a rendered end-to-end match, navigation-scale performance, clean-machine use, Developer ID signing, notarization, final art/audio, and release qualification remain open.
