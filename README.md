...
**Current state:** This is a verified Unreal playable-systems prototype moving into its Glass Scar vertical slice, not a completed game. UE 5.8.2 is installed; the native simulation passes 13/13 hardened suites in optimized, debug, and sanitizer configurations; the arm64 Development Editor target builds; and Unreal automation passes 5/5. The current scenario contains the authored Glass Scar topology: a 165-tile fractured transit band, a central Future Well crossing, and two side-cavern routes. A map-specific test proves the exact terrain layout, presentation parity, and deterministic unit routing from the southern basin to the northern basin. The complete automated skirmish still builds a drop-off, captures the Well, produces and rallies seven soldiers, crosses the Scar, reaches victory from one attack-move order per soldier, freezes after the result, and restarts. The 0.6.0 source adds authoritative health-bar presentation: bars appear for selected or damaged entities, scale to the exact hit-point fraction, and remain hidden on unselected full-health entities. It also retains `[F]` attack-move, `[X]` stop, fog/shroud, construction, production, context orders, pause, and restart. Rendered inspection confirms that the placeholder span is visually distinct, its central break is readable, unexplored fog conceals it without leaking terrain information, and the health components do not introduce stray full-health bars. The latest accepted self-contained package remains 0.5.0, traceable to clean source commit `20fa98e`; the 0.6.0 health-bar source is not yet packaged. Complete pointer interaction, a rendered end-to-end match, navigation-scale performance, clean-machine use, Developer ID signing, notarization, final art/audio, and release qualification remain open.
...
Unreal Engine 5.8.2 is installed at `/Users/Shared/Epic Games/UE_5.8` and its completed launcher manifest has been inspected. The separately delivered Apple Metal Toolchain is installed. The host is a 16 GB M1 Pro Mac with about 80 GiB free after retaining the current 0.5.0 package and removing its exact rebuildable staging directory. That is above the project's 40 GiB prototype-build stop threshold and its preferred 60 GiB packaging headroom, but below the safer 100 GiB sustained-production target. A verified official launcher-installer backup exists at `~/Downloads/EpicInstaller-20.1.4.dmg`, but it should not replace the newer installed launcher.
