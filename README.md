...
**Current state:** This is a verified Unreal playable-systems prototype moving into its Glass Scar vertical slice, not a completed game. UE 5.8.2 is installed; the native simulation passes 16/16 hardened suites in optimized, debug, and sanitizer configurations; the arm64 Development Editor target builds; and Unreal automation passes 10/10 with no warnings or errors. The current scenario contains the authored Glass Scar topology: a 165-tile fractured transit band, a central Future Well crossing, and two side-cavern routes. Version 0.11.0 adds deterministic `[T]` patrol: a mobile combat unit reverses indefinitely between its original position and a selected endpoint, engages only visible hostiles inside a six-tile route envelope, preserves both endpoints through schema-7 checkpoints and replays, and releases the order on `[X]` stop. `[H]` hold and `[J]` guard remain available with ten validated control groups, attack-move, construction, production, fog/shroud, pause, restart, and transactional checkpoints. The complete automated skirmish still crosses the Scar and reaches victory. Rendered inspection confirms the patrol hotkey, reason-coded empty-selection path, and unclipped HUD; actual route and combat behavior are automation-covered. The latest accepted self-contained package remains the clean-source 0.8.0 control-group build at commit `958e1b7`; 0.11.0 is source-qualified but has not yet been packaged. Complete pointer interaction, a rendered end-to-end match, navigation-scale performance, clean-machine use, Developer ID signing, notarization, final art/audio, and release qualification remain open.
