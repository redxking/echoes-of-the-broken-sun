...
**Current state:** This is a verified Unreal playable-systems prototype moving into its Glass Scar vertical slice, not a completed game. UE 5.8.2 is installed; the native simulation passes 16/16 hardened suites in optimized, debug, and sanitizer configurations; the arm64 Development Editor target builds; and Unreal automation passes 10/10 with no warnings or errors. Version 0.13.0 retains the schema-8 deterministic destination-field and non-allocating checksum redesign and makes TAA the Mac rendering default. In the accepted isolated native 400-unit/two-team profile, visibility, path burst, moving-unit tick, and checksum costs pass their implemented budgets. In a separate packaged Metal profile of the current 25-entity placeholder scene at native 2560×1440, medium groups, TAA, and exclusive fullscreen, 480 post-warm-up frames measured 10.458 ms frame, 10.457 ms render-thread, 9.948 ms GPU, and 0.895 ms game-thread p95; sampled resident memory peaked at 610.9 MiB. Those packaged results clear the recorded thresholds only for that placeholder scene—not a representative 400-unit combat/weather workload, four-team fog, a soak, or release qualification. Deterministic patrol, hold, guard, ten control groups, attack-move, construction, production, fog/shroud, pause, restart, and transactional checkpoints remain intact, and the complete automated skirmish still reaches victory. The latest accepted self-contained package is the clean-source 0.13.0 Mac-performance build at commit `c37c9b6`, with verified arm64 identity, version, local ad-hoc signature seal, content manifest, packaged startup, bounded performance evidence, and rendered presentation inspection. Complete pointer interaction, a rendered end-to-end match, representative stress/soak qualification, clean-machine use, Developer ID signing, notarization, final art/audio, and release qualification remain open.
...
3. Build the representative 400-unit combat/weather stress scene, add four-team visibility, run the 60-minute soak, and complete pointer-driven construction, production, movement, combat, victory, and restart validation as one rendered match before content expansion.
