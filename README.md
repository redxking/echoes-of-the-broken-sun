...
**Current state:** This is a verified Unreal playable-systems prototype moving into its Glass Scar vertical slice, not a completed game. UE 5.8.2 is installed; the native simulation passes 13/13 hardened suites in optimized, debug, and sanitizer configurations; the arm64 Development Editor target builds; and Unreal automation passes 7/7 with no warnings or errors. The current scenario contains the authored Glass Scar topology: a 165-tile fractured transit band, a central Future Well crossing, and two side-cavern routes. A map-specific test proves the exact terrain layout, presentation parity, and deterministic unit routing from the southern basin to the northern basin. The complete automated skirmish still builds a drop-off, captures the Well, produces and rallies seven soldiers, crosses the Scar, reaches victory from one attack-move order per soldier, freezes after the result, and restarts. Version 0.8.0 adds ten validated RTS control groups: `[G]` then `[1-0]` assigns the current local selection, while bare `[1-0]` recalls the group and prunes stale entity IDs. Future Well choices move to `[Z]`, `[C]`, and `[V]`. Transactional checkpoint persistence remains on `[K]` and `[L]`, reconstructing disposable views and preserving valid group identities after load. It retains authoritative health bars, `[F]` attack-move, `[X]` stop, fog/shroud, construction, production, context orders, pause, and restart. Rendered Editor inspection confirms the assignment-mode and empty-recall paths without disturbing normal lit rendering. The latest accepted self-contained package remains the clean-source 0.7.0 checkpoint build at commit `bcdb6d9`; a 0.8.0 package has not yet been qualified. Complete pointer interaction, a rendered end-to-end match, navigation-scale performance, clean-machine use, Developer ID signing, notarization, final art/audio, and release qualification remain open.
