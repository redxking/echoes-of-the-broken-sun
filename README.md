...
**Current state:** This is a verified Unreal playable-systems prototype moving into its Glass Scar vertical slice, not a completed game. UE 5.8.2 is installed; the deterministic core supports four player slots and passes 17/17 hardened suites in optimized, debug, and sanitizer configurations; the arm64 Development Editor target builds; and Unreal automation passes 11/11 with no warnings or errors. Version 0.15.0 adds a deterministic Unreal scale fixture with 400 visible unit proxies split evenly across four active teams, plus one Future Well. In the accepted packaged Metal profile of that scale fixture at native 2560×1440, medium groups, TAA, and exclusive fullscreen, 480 post-warm-up frames measured 10.490 ms frame, 10.484 ms render-thread, 9.963 ms GPU, and 1.205 ms game-thread p95; sampled resident memory peaked at 632.906 MiB, and every applied threshold passed. A separate same-build 25-entity baseline also passed. The scale fixture does not contain authored weather, final effects, formations, or representative broad combat orders, so it does not qualify a representative combat/weather workload, soak stability, or release readiness. Deterministic patrol, hold, guard, control groups, attack-move, construction, production, fog/shroud, pause, restart, and transactional checkpoints remain intact, and the complete automated skirmish still reaches victory. The latest accepted self-contained package is the clean-source 0.15.0 four-team-scale build at commit `8841176`, with verified arm64 identity, version, local ad-hoc signature seal, content manifest, packaged startup, and bounded baseline/scale performance evidence. Complete pointer interaction, a rendered end-to-end match, the 60-minute soak, authored representative combat/weather stress, networked multiplayer, clean-machine use, Developer ID signing, notarization, final art/audio, accessibility/settings implementation, and release qualification remain open.
...
Unreal Engine 5.8.2 is installed at `/Users/Shared/Epic Games/UE_5.8` and its completed launcher manifest has been inspected. The separately delivered Apple Metal Toolchain is installed. The host is a 16 GB M1 Pro Mac with about 72 GiB free after retaining the accepted development packages. The exact 0.13.0, 0.14.0, and 0.15.0 rebuildable staging trees are recoverable in Trash and each still occupies about 1.1 GB until Trash is emptied. Current free space is above the project's 40 GiB prototype-build stop threshold and its preferred 60 GiB packaging headroom, but below the safer 100 GiB sustained-production target. A verified official launcher-installer backup exists at `~/Downloads/EpicInstaller-20.1.4.dmg`, but it should not replace the newer installed launcher.
...
2. Keep full Xcode selected. Xcode 26.6 has passed project generation, arm64 editor compilation, eleven Unreal automation tests, null-RHI startup for both normal and 400-unit/four-team fixtures, and local Development cooking/packaging; Epic still recommends 26.1.1, and clean-machine use plus the authored representative workload remain unverified.
3. Add authored weather/effects and representative broad combat orders to the 400-unit/four-team fixture, run the 60-minute cache/memory soak, and complete pointer-driven construction, production, movement, combat, victory, and restart validation as one rendered match before content expansion.
