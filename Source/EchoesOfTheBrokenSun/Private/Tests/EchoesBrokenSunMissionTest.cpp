...
using FPreservedBrokenSunFile = FEchoesPreservedTestFile;
...
bool FEchoesBrokenSunMissionTest::RunTest(const FString& Parameters)
{
    ...
    TestTrue(
        ...
        FindBuildSite(RuntimePlan.CrownfallApproachSite, 3,
                      ApproachBuildSite));
    // Schema 29 appends production state after schema 28 hostility masks.
    // Replay versioning is independent; every schema named below is the native
    // snapshot schema this build saves.
    const auto NativeBuildSequence =
        Bridge->GetSimulation()->NextCommandSequence(
            UEchoesSimulationSubsystem::LocalPlayerId);
    TestTrue(
        TEXT("The schema-29 approach build has a stable receipt sequence"),
    ...
    TestTrue(
        ...
        Objective.bBrokenSunApproachSecured &&
            Objective.BrokenSunApproachAnchorId != 0);
    TestTrue(
        TEXT("The native schema-29 source retains the approach-build receipt"),
        NativeBuildSequence.has_value() &&
            Bridge->GetSimulation()->FindCommandResolutionReceipt(
                UEchoesSimulationSubsystem::LocalPlayerId,
                *NativeBuildSequence)
    ...
    EchoesSnapshotMigrationTestHelpers::FEmbeddedSnapshotLayout
        NativeLayout;
    // The checkpoint saved just above is native schema 32. The shared helpers
    ...
    TestTrue(
        TEXT("The Mission 15 schema-32 checkpoint exposes bounded receipt, lifecycle, hostility, and production blocks"),
    ...
    // opponent change fails here, readably, instead of as a converter refusal.
    {
        ...
        TestTrue(TEXT("Mission 15 checkpoint carries no opponent production queue or unfinished site"),
    ...
    const int32 Mission15MapTiles =
        Bridge->GetSimulation()->Config().mapWidthTiles *
        Bridge->GetSimulation()->Config().mapHeightTiles;
    TestTrue(
        TEXT("The Mission 15 checkpoint carries one terrain memory per player and a bounded object memory"),
        Mission15MapTiles > 0 &&
            NativeLayout.RememberedTileCount ==
                static_cast<uint32>(Mission15MapTiles) &&
    ...
    TestTrue(
        TEXT("The Mission 15 checkpoint converts through every schema from 32 to its synthetic schema-22 shape"),
        EchoesSnapshotMigrationTestHelpers::
            ...
            FMemory::Memcmp(
                NativeCheckpoint.GetData(), V22Checkpoint.GetData(), 34) == 0 &&
            FMemory::Memcmp(
    ...
    TestTrue(
        ...
        Objective.BrokenSunPendingFinalResolution ==
            ...
            Objective.BrokenSunFinalResolution ==
                EEchoesFinalResolution::None);
    // Resaving writes native schema 32; the replay envelope is untouched here
    // and stays at its independent replay schema.
    TestTrue(
        TEXT("The legacy-loaded Mission 15 state resaves as native schema 32"),
        Bridge->QuickSaveScenario(Feedback));
    TArray<uint8> ResavedNativePrimary;
    EchoesSnapshotMigrationTestHelpers::FEmbeddedSnapshotLayout
        ResavedNativeLayout;
    TestTrue(
        TEXT("The Mission 15 primary records native schema 32 after legacy load"),
    ...
    // Only the resave moved to native schema 32. The retained backup is
    ...
    TestTrue(
        TEXT("The first schema-32 resave retains the valid schema-22 Mission 15 generation"),
        FFileHelper::LoadFileToArray(
            NativeMapEnvelope,
            *(QuickSavePath + TEXT(".bak"))) &&
    ...
    // record; replay versioning remains independent.
    TestTrue(
