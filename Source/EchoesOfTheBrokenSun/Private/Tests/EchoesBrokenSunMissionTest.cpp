...
bool FEchoesBrokenSunMissionTest::RunTest(const FString& Parameters)
{
    ...
    EchoesSnapshotMigrationTestHelpers::FEmbeddedSnapshotLayout
        NativeLayout;
    // The checkpoint saved just above is native schema 33. The shared helpers
    ...
    TestTrue(
        TEXT("The Mission 15 schema-33 checkpoint exposes bounded receipt, lifecycle, hostility, and production blocks"),
    ...
    TestTrue(
        TEXT("The Mission 15 checkpoint converts through every schema from 33 to its synthetic schema-22 shape"),
        EchoesSnapshotMigrationTestHelpers::
    ...
    TestTrue(
        ...
        Objective.BrokenSunPendingFinalResolution ==
            ...
            Objective.BrokenSunFinalResolution ==
                EEchoesFinalResolution::None);
    // Resaving writes native schema 33; the replay envelope is untouched here
    // and stays at its independent replay schema.
    TestTrue(
        TEXT("The legacy-loaded Mission 15 state resaves as native schema 33"),
        Bridge->QuickSaveScenario(Feedback));
    TArray<uint8> ResavedNativePrimary;
    EchoesSnapshotMigrationTestHelpers::FEmbeddedSnapshotLayout
        ResavedNativeLayout;
    TestTrue(
        TEXT("The Mission 15 primary records native schema 33 after legacy load"),
    ...
    // Only the resave moved to native schema 33. The retained backup is
    ...
    TestTrue(
        TEXT("The first schema-33 resave retains the valid schema-22 Mission 15 generation"),
        FFileHelper::LoadFileToArray(
            NativeMapEnvelope,
            *(QuickSavePath + TEXT(".bak"))) &&
