...
Read [echoes-heavy-run-coordination](../echoes-heavy-run-coordination/SKILL.md) before any GUI action, confirm an exclusive resource reservation, and record and release the reservation afterward. Do not reuse or self-invent one. Bind package identity with `echoes-package-provenance` and closure review with `echoes-evidence-gate-review`.
