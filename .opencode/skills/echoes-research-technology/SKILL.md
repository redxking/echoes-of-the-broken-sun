...
Route production interactions to `echoes-construction-production`, balance to `echoes-balance-analysis`, Unreal/UI work to `echoes-unreal-runtime-integration` and `echoes-gui-control-readiness`, then `echoes-evidence-gate-review` and `echoes-human-acceptance-session`. Stop on canon ambiguity, tech tree redesign, lease conflict, or unproven effect scope.
