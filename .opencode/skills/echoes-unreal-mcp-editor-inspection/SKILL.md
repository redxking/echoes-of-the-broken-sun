...
Observed UE 5.8.2 capture compatibility: `CaptureViewport` required explicit optional inputs during setup. For the current-camera capture workaround, and verified tool-call evidence, read [Installed setup and external storage](../../../Docs/UnrealEditorIntegrationResearch.md#installed-setup-and-external-storage). `CaptureEditorImage` worked with empty arguments. Recheck after engine updates.
